...
async function uploadToYouTube(videoPath, script, topic, accessToken) {
  ...
  // Step 1: Initiate resumable upload — capture Location header
  const uploadUrl = await new Promise((resolve, reject) => {
    const req = https.request({
      ...
      res.on('end', () => {
        ...
        else reject(new Error(`No Location header. Status: ${res.statusCode}. Body: ${body}`))
      })
    })
    req.on('error', reject)
    req.write(metadata)
  ...
  if (!result.id) throw new Error(`Upload failed (${uploadRes.status}): ${uploadRes.body}`)
  return result.id
}
