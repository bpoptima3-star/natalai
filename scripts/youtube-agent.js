...
async function createVideo(script, audioPath, outputPath) {
  const lines = [
    script.hook,
    '',
    ...script.content.split('. ').filter(Boolean).map(s => s.trim() + '.'),
    '',
    '✦ natalai.live',
  ]

  // Create text overlay file for ffmpeg
  const drawtextFilters = lines.map((line, i) => {
    const y = 300 + (i * 80)
    const fontSize = i === 0 ? 52 : i === lines.length - 1 ? 48 : 38
    const color = i === lines.length - 1 ? 'E6B568' : 'FFFFFF'
    const escaped = line.replace(/'/g, "\\'").replace(/:/g, '\\:')
    return `drawtext=fontsize=${fontSize}:fontcolor=${color}:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:text='${escaped}':x=(w-text_w)/2:y=${y}:enable=1`
  }).filter((_, i) => lines[i] !== '').join(',')
