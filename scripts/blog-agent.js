...
async function writeArticle(topic) {
  ...
  // Smart CTA based on article type — matches NatalAI features
  const ctaMap = {
    birth_star:    { text: 'Find your Vedic birth star free at NatalAI.live → enter your birth details and see your exact constellation sector, planetary periods, and active combinations.', link: 'https://natalai.live', tab: '' },
    yoga:          { text: 'See if you have this combination in your birth chart — free at NatalAI.live. The Combinations tab shows every planetary combination in your chart with plain English explanations.', link: 'https://natalai.live', tab: '#combinations' },
    compatibility: { text: 'Try the free Cosmic Chemistry reading at NatalAI.live — enter both birth details and see your full Vedic compatibility score, planetary harmony, and honest assessment.', link: 'https://natalai.live', tab: '' },
    ai:            { text: `Get your free Vedic birth chart at NatalAI.live — then use Ask the Stars to ask what this means specifically for you.`, link: 'https://natalai.live', tab: '#ask' },
  }
  const smartCta = ctaMap[topic.series] || ctaMap.ai

  // Related articles context for internal linking
  const allArticles = loadArticles()
  const related = allArticles
    .filter(a => a.slug !== topic.keyword && a.series !== topic.series)
    .slice(-6)
    .map(a => `"${a.title}" at /blog/${a.slug}.html`)
    .join(', ')
...
- ## How to find this in your birth chart (180 words) — mention the free chart at natalai.live
- ## Key variations and what changes things (150 words)
- CTA section: "${smartCta.text}" Also mention the ${report.label} (${report.price}) for a full personalised reading.
...
- Naturally link to 1-2 related articles if relevant: ${related || 'none yet'}
...
function buildArticleHtml(article, topic, date) {
  ...
  // Smart CTA config per series
  const ctaConfig = {
    birth_star:    { headline: 'Find Your Birth Star Free', btn: 'See My Birth Star → natalai.live', link: 'https://natalai.live', sub: 'Free chart · No signup needed' },
    yoga:          { headline: 'Check Your Chart for This Combination', btn: 'See My Combinations → natalai.live', link: 'https://natalai.live', sub: 'Free · Combinations tab shows all your planetary combinations' },
    compatibility: { headline: 'Get Your Free Cosmic Chemistry Reading', btn: 'Try Cosmic Chemistry → natalai.live', link: 'https://natalai.live', sub: 'Free compatibility reading · Enter both birth details' },
    ai:            { headline: 'Ask the Stars What This Means for You', btn: 'Ask the Stars → natalai.live', link: 'https://natalai.live', sub: 'Free · AI answers questions about your specific chart' },
  }
  const cta = ctaConfig[topic.series] || ctaConfig.ai
...
<head>
  ...
  <meta property="og:image" content="https://natalai.live/pin-cover.jpg">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:site" content="@NatalAI">
  ...
  <style>
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .nav-back{color:var(--muted);font-size:13px;text-decoration:none;font-family:system-ui}
    .nav-cta{background:var(--purple);color:#fff;font-size:12px;padding:8px 16px;border-radius:8px;text-decoration:none;font-family:system-ui;font-weight:500}
    ...
    .byline{color:var(--muted);font-size:13px;font-family:system-ui;margin-bottom:32px}
    .try-free{background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.25);border-radius:12px;padding:16px 20px;margin-bottom:40px;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap}
    .try-free p{color:var(--cream);font-size:14px;font-family:system-ui;margin:0}
    .try-free a{background:var(--purple);color:#fff;font-size:13px;padding:8px 18px;border-radius:8px;text-decoration:none;font-family:system-ui;font-weight:500;white-space:nowrap}
    ...
    .mid-cta{background:rgba(230,181,104,.08);border:1px solid rgba(230,181,104,.2);border-radius:12px;padding:20px 24px;margin:48px 0;text-align:center}
    .mid-cta p{color:var(--cream);font-size:15px;margin:0 0 12px;font-family:system-ui}
    .mid-cta a{color:var(--gold-l);font-weight:600;font-family:system-ui}
    ...
    .cta h3{font-size:22px;color:var(--white);margin:0 0 8px;font-family:Georgia,serif}
    .cta .sub-head{color:var(--purple-l);font-size:13px;font-family:system-ui;margin-bottom:16px}
    .cta p{color:rgba(255,255,255,.6);margin-bottom:24px;font-size:15px;font-family:system-ui}
    .cta-btn-primary{display:inline-block;background:var(--gold-l);color:#0d0820;padding:14px 32px;border-radius:10px;text-decoration:none;font-family:system-ui;font-size:15px;font-weight:700;margin-bottom:12px}
    .cta-btn-secondary{display:block;color:var(--purple-l);font-size:13px;text-decoration:none;font-family:system-ui;margin-top:8px}
...
<body>
  <nav>
    ...
    <div class="nav-right">
      <a href="/blog" class="nav-back">← Articles</a>
      <a href="https://natalai.live" class="nav-cta">Free Chart</a>
    </div>
  </nav>
  ...
  <main>
    ...
    <!-- Try it free widget — top of every article -->
    <div class="try-free">
      <p>✦ See this in your own chart — free Vedic birth chart at NatalAI</p>
      <a href="https://natalai.live">Try Free →</a>
    </div>

    ${htmlBody}

    <!-- Mid-article CTA (appears naturally in content flow) -->
    <div class="mid-cta">
      <p>${cta.headline}</p>
      <a href="${cta.link}">${cta.btn}</a>
    </div>

    <!-- Bottom CTA -->
    <div class="cta">
      <h3>Get Your Personalised Vedic Reading</h3>
      <p class="sub-head">${cta.sub}</p>
      <p>The <strong style="color:#fff">${report.label}</strong> gives you a full personalised Vedic report — calculated from your exact birth data using traditional methods.</p>
      <a href="https://natalai.live/#reports" class="cta-btn-primary">Get ${report.label} — ${report.price}</a>
      <a href="https://natalai.live" class="cta-btn-secondary">Or start with your free chart →</a>
      <p class="cta-sub">Instant delivery · Pay once · No subscription</p>
    </div>
  </main>

  <footer>
    <a href="https://natalai.live">✦ Free Vedic birth chart at natalai.live</a>
    &nbsp;·&nbsp;
