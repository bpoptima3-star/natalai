...
// Sends report HTML to user via Resend with light theme + clean formatting
...
function cleanForEmail(html) {
  ...
  // 1. Remove the chart grid tables (house/planet concat text bleeding)
  h = h.replace(/<div[^>]*class="[^"]*rep-covgrid[^"]*"[^>]*>[\s\S]*?<\/div>\s*<\/div>/g, '');

  // 2. Replace dark backgrounds with light
  ...
  // 3. Fix dark text that's now invisible on white
  ...
  // 4. Fix borders that were white/invisible
...
<style>
  ...
  .body{background:#ffffff;padding:0;font-size:15px;line-height:1.7;color:#1d1d1f}
  /* Override dark rep-cover to keep it dark — that's intentional */
  .rep-cover{background:#1d1d1f!important;color:#ffffff!important}
  .rep-cover *{color:inherit}
  .rep-h1{color:#ffffff!important}
  .rep-h1 em{color:#bf9a30!important}
  .rep-brand,.rep-csub{color:rgba(255,255,255,.6)!important}
  .rep-body{padding:32px 40px!important;background:#ffffff!important;font-size:15px!important;color:#1d1d1f!important}
  .rep-body *{font-size:inherit}
  .rep-sec{margin-bottom:36px}
