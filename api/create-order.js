...
// Upstash Redis — used to stash report params at order-creation so the webhook
// can deliver the report server-side even if the client closes mid-generation.
// Reads KV_REST_API_URL / KV_REST_API_TOKEN from env (injected by Vercel).
// All Redis use is wrapped in try/catch — a storage failure NEVER blocks payment.
let _redis = null;
function getRedis() {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Stash report params so the webhook can deliver server-side if the client
    // closes before submitReport() runs. 24h TTL = auto-cleanup. Never blocks payment.
    if (data.id) {
      const redis = getRedis();
      if (redis) {
        try {
          await redis.set('order:' + data.id, JSON.stringify({
            reportType: body.reportType || 'natal',
            name:   body.name   || '',
            dob:    body.dob    || '',
            tob:    body.tob    || '',
            pob:    body.pob    || '',
            ...
            email:  body.email  || '',     // optional on-site email if provided
            createdAt: new Date().toISOString()
          }), { ex: 60 * 60 * 24 });
        } catch (e) {
          console.warn('[ORDER] Redis stash failed (non-fatal):', e.message);
        }
      }
    }
