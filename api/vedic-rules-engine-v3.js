...
// ── PHASE 2: SENSITIVITY DETECTION ──────────────────────────────────────
// Downstream consumers (chart.js askRulesCtx, persona rules) can detect
// sensitive findings by rule ID pattern. No API change to rule() needed.
// Anchored prefixes prevent false positives on positive rules that happen
// to contain common words (e.g. a future INTELLECTUAL-CHALLENGING rule).
const SENSITIVE_RULE_PATTERNS = [
  /^MARR-DENIAL/i,          // new Phase 2 denial rules
  /^DENIAL-/i,              // any DENIAL- prefixed rule
  /WIDOW/i,                 // FEM-WIDOW and similar
  /MARITAL.LONGEVITY/i,     // marital longevity caution rules
  /SIGNIFICANT\.CHALLENGE/i, // SIGNIFICANT-CHALLENGE compound verdicts
  /SIGNIFICANT\.CONCERN/i,
  /SERIOUS\.CHALLENGE/i,
  /CHRONIC\.(ILLNESS|VULNERAB)/i,
  /MULTIPLE.MARRIAGE/i,
];

function isFindingSensitive(finding) {
  if (!finding || !finding.id) return false;
  return SENSITIVE_RULE_PATTERNS.some(p => p.test(finding.id));
}

// ── PHASE 2: SIGN QUALITY HELPERS ────────────────────────────────────────
// Sign indices: 0=Aries, 1=Taurus, 2=Gemini, ... 11=Pisces
function isFixedSign(signIdx)   { return [1, 4, 7, 10].includes(signIdx); }
function isMovableSign(signIdx) { return [0, 3, 6, 9].includes(signIdx); }
function isDualSign(signIdx)    { return [2, 5, 8, 11].includes(signIdx); }

// ── PHASE 2: KARTARI YOGA HELPERS ────────────────────────────────────────
// Papakartari: planet flanked by malefics in BOTH adjacent houses
// Subhakartari: planet flanked by benefics in BOTH adjacent houses
// CRITICAL: 1-indexed houses. Prev of house 1 = 12. Next of house 12 = 1.
// Formula: prev = ((house - 2 + 12) % 12) + 1 ; next = (house % 12) + 1
// Spot-check: house 1 → prev=(( -1+12)%12)+1 = 12, next=(1%12)+1 = 2 ✓
// Spot-check: house 12 → prev=((10)%12)+1 = 11, next=(12%12)+1 = 1 ✓
function isInPapakartari(planet, chart) {
  const house = houseOfPlanet(planet, chart);
  if (!house) return false;
  const prev = ((house - 2 + 12) % 12) + 1;
  const next = (house % 12) + 1;
  return planetsInHouse(prev, chart).some(p => isMalefic(p)) &&
         planetsInHouse(next, chart).some(p => isMalefic(p));
}

function isInSubhakartari(planet, chart) {
  const house = houseOfPlanet(planet, chart);
  if (!house) return false;
  const prev = ((house - 2 + 12) % 12) + 1;
  const next = (house % 12) + 1;
  return planetsInHouse(prev, chart).some(p => isBenefic(p)) &&
         planetsInHouse(next, chart).some(p => isBenefic(p));
}
...
// PHASE 4: getAtmakaraka + getAmatyakaraka are now THIN WRAPPERS over computeJaiminiKarakas.
// computeJaiminiKarakas (engine.js:4140) is the single source of truth — it uses 8 karakas
// with Rahu's degree REVERSED (30 − longitude%30) per classical Jaimini convention.
// The old independent implementations used raw Rahu degree (no reversal), which silently
// produced wrong AK/AmK for charts where Rahu had a high raw degree. This fixes that bug.
// All call sites receive the same planet-name string return type — no callers need updating.
// Cached on chart._jaiminiKarakas to avoid recomputing across multiple call sites.

function getAtmakaraka(chart) {
  // Use cached karakas if available (set by evaluateJaiminiKarakas or prior call)
  const k = chart._jaiminiKarakas || computeJaiminiKarakas(chart);
  if (!chart._jaiminiKarakas) chart._jaiminiKarakas = k;
  return k.AK?.planet || null;
}

// Amatyakaraka — second highest degree (Jaimini — career minister)
// NOTE: engine.js uses this cached c.amatyakaraka. chart.js Part 2a enrichment uses
// longitude%30 which can differ for Rahu edge cases. AmK convergence = Sprint 2 cleanup.
function getAmatyakaraka(chart) {
  const k = chart._jaiminiKarakas || computeJaiminiKarakas(chart);
  if (!chart._jaiminiKarakas) chart._jaiminiKarakas = k;
  return k.AmK?.planet || null;
}
...
function evaluateMarriage(chart) {
  ...
  // PHASE 2: Age gate — classical marriage analysis meaningful from age 15+
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 13) {
    findings.push(rule('AGE-TOO-EARLY-MARRIAGE', 'Classical age threshold',
      `AGE-GATED: Chart subject is in early childhood (age ${chart.subjectAge}). Marriage themes activate with 7th house maturity around teenage years. Classical marriage analysis becomes meaningful from age 15+. Chart structure described for reference.`,
      'HIGH', 0));
    return { findings, marriageScore: 0, delayScore: 0, denialScore: 0, ageGated: true, ageThreshold: 13,
      verdict: 'AGE-GATED: Chart subject too young for meaningful marriage timing analysis. Marriage themes activate with 7th house maturity in teenage years. Classical analysis becomes meaningful from age 15+.' };
  }
  ...
  // ── PHASE 2: SASHA YOGA GRADATION FOR SATURN IN 7TH ──────────────────
  // BPHS Ch.36 — Saturn in own/exalted sign in kendra = Sasha Mahapurusha Yoga.
  // The planetIn7th loop fires Saturn-IN7 with score -2 / delay +3.
  // When Saturn is exalted (Libra) or own (Capricorn/Aquarius) in 7th, the
  // delay is constructive — partnership is late but exceptionally stable.
  // Offset the generic scores with a correction: score -1 (not -2), delay +1 (not +3).
  if (h7planets.includes('Saturn')) {
    const satSign7 = signOfPlanet('Saturn', chart);
    const satDig7  = getDignity('Saturn', satSign7);
    if (satDig7 === 'EXALTED' || satDig7 === 'OWN' || satDig7 === 'MOOLATRIKONA') {
      // The Saturn-IN7 rule already fired with -2 / +3. Apply offset to net to -1 / +1.
      marriageScore += 1;    // net: -2 + 1 = -1
      delayScore    -= 2;    // net: +3 - 2 = +1
      findings.push(rule('SAT-SASHA-7TH', 'BPHS Ch.36 + PD Ch.9',
        `Saturn ${satDig7.toLowerCase()} in 7th creates Sasha Yoga quality in the partnership house. Marriage is delayed but exceptionally stable and enduring once formed. Spouse is highly capable, dignified, often in service, government, or disciplined profession. The delay is constructive — both people arrive more formed. Classical texts note this as "late but lasting."`,
        'HIGH', 1));
    }
  }
  ...
  // ── PHASE 2: PAPAKARTARI / SUBHAKARTARI YOGA FOR VENUS ──────────────
  // PD Ch.12 v.18 + BPHS Ch.18 — Venus flanked by malefics / benefics
  if (isInPapakartari('Venus', chart)) {
    findings.push(rule('MARR-DENIAL-VENUS-PAPAKARTARI', 'PD Ch.12 v.18 + BPHS Ch.18',
      `Venus is in Papakartari Yoga — flanked by malefics in adjacent houses. Classical condition affecting marriage timing. Partnership in this configuration benefits from awareness, right dasha activation, and conscious work on the patterns that surround Venus. Marriage can and does occur in this configuration, typically with more deliberate effort than charts without this pattern.`,
      'HIGH', -3));
    marriageScore -= 3;
    delayScore += 2;
    denialScore += 1;
  ...
  if (isInSubhakartari('Venus', chart)) {
    findings.push(rule('VENUS-SUBHAKARTARI', 'SAR Ch.28',
      `Venus is in Subhakartari Yoga — flanked by benefics in adjacent houses. Classical marriage-blessing. Marriage and partnerships are supported by surrounding life conditions. Favorable for harmony and happiness in partnership.`,
      'HIGH', 3));
    marriageScore += 3;
  }
  ...
  // ══════════════════════════════════════════════════════════════════════
  // PHASE 2 — MARRIAGE RULE ADDITIONS: Sprint 1 net-new + extensions +
  // user's classical rules (early/normal/late/delayed categories).
  // Denial rules are added after all positive/mixed rules — see below.
  // ══════════════════════════════════════════════════════════════════════

  // ── A3/A4: SPRINT 1 NET-NEW MARRIAGE RULES ────────────────────────────

  // RULE: Jupiter strong as marriage significator (gender-variant)
  // BPHS Ch 80 v.12 + Saravali Stree Jataka
  // Engine previously had no standalone Jupiter-for-marriage rule (only JUP-VENUS conjunction).
  const jupSign = signOfPlanet('Jupiter', chart);
  const jupDig  = getDignity('Jupiter', jupSign);
  const jupHouse = houseOfPlanet('Jupiter', chart);
  if (chart.gender === 'M') {
    if (jupDig === 'EXALTED' || jupDig === 'OWN' || jupDig === 'MOOLATRIKONA' || jupDig === 'FRIEND') {
      let jupBonus = 0;
      if (jupHouse && [1,5,7,9,10,11].includes(jupHouse)) jupBonus = 1;
      findings.push(rule('JUP-STRONG-M', 'BPHS Ch 80 v.12',
        `Jupiter strong (${jupDig.toLowerCase()}) — beneficial marriage indicator for male charts. Jupiter as 9th-house-karaka supports fortune in partnership. Benefic placement at house ${jupHouse||'?'} adds to this support.`,
        'MEDIUM', 2));
      marriageScore += 2 + jupBonus;
    } else if (jupDig === 'DEBILITATED') {
      findings.push(rule('JUP-WEAK-M', 'BPHS Ch 80 v.12',
        `Jupiter debilitated — weakened fortune and wisdom signals for marriage in male chart. Marriage may require more deliberate effort in building trust and long-term vision.`,
        'MEDIUM', -1));
      marriageScore -= 1;
    }
  } else {
    // Female: Jupiter is primary husband karaka (Saravali Stree Jataka)
    if (jupDig === 'EXALTED' || jupDig === 'OWN' || jupDig === 'MOOLATRIKONA' || jupDig === 'FRIEND') {
      let jupBonus = 0;
      if (jupHouse && [1,5,7,9,10,11].includes(jupHouse)) jupBonus = 1;
      findings.push(rule('JUP-STRONG-F', 'BPHS Ch 80 v.12 + Saravali Stree Jataka',
        `Jupiter strong (${jupDig.toLowerCase()}) — primary husband karaka for female chart is well-placed. Saravali Stree Jataka identifies Jupiter as the dominant marriage significator for women. Placement at house ${jupHouse||'?'} adds specific expression.`,
        'HIGH', 5));
      marriageScore += 5 + jupBonus;
    } else if (jupDig === 'DEBILITATED') {
      findings.push(rule('MARR-DENIAL-JUP-DEBIL-F', 'Saravali Stree Jataka',
        `Jupiter debilitated in female chart — husband karaka under classical stress. Partnership in this configuration benefits from conscious selection of a partner whose own Jupiter placement is strong, and from Jupiter-strengthening remedies if the person is open. Non-traditional or later-in-life partnerships are a classical response to this pattern.`,
        'HIGH', -3));
      marriageScore -= 3;
      denialScore += 1;
    }
  ...
  // RULE: Multiple benefics aspect 7th house (aggregate — not per-planet)
  // Phaladeepika Ch 9 v.7 — multiple benefic aspects = strong marriage opening
  // Engine previously only used Jupiter→7 aspect as cancellation condition for Mangal Dosha
  const beneficsAspecting7 = ['Jupiter','Venus','Mercury','Moon'].filter(p =>
    planetAspectsHouse(p, chart, 7)
  );
  if (beneficsAspecting7.length >= 2) {
    findings.push(rule('MULTI-BEN-ASPECT-7', 'PD Ch.9 v.7',
      `Multiple benefics (${beneficsAspecting7.join(', ')}) aspect the 7th house — Phaladeepika Ch.9 v.7 identifies this as a strong marriage opening. Benefic aspects collectively protect and enrich the partnership house.`,
      'HIGH', 4));
    marriageScore += 4;
  }

  // RULE: Classical-age + dasha karaka active (age-gated)
  // BPHS Ch 80 — dasha of karaka in classical age window
  // Requires subjectAge on chart object (set by VedicRulesEngine.evaluate())
  if (chart.subjectAge !== null && chart.subjectAge !== undefined) {
    const inClassicalAge = chart.subjectAge >= 26 && chart.subjectAge <= 32;
    if (inClassicalAge && chart.dasha) {
      const { maha, antar, pratyantar } = chart.dasha;
      const classicalTriggers = [l7, 'Venus', 'Jupiter'];
      const activated = classicalTriggers.some(t => t && (maha === t || antar === t || (pratyantar && pratyantar === t)));
      if (activated) {
        findings.push(rule('CLASSICAL-AGE-DASHA', 'BPHS Ch 80',
          `Chart subject is in the classical marriage window (age ${chart.subjectAge}) AND the active dasha activates a marriage karaka (${[maha,antar,pratyantar].filter(Boolean).join('/')}). Classical texts specifically identify this combination as high-probability for marriage events.`,
          'HIGH', 3));
        marriageScore += 3;
      }
    }
  ...
  // RULE: 7th has malefic(s) with NO benefic in 7th and no Jupiter aspect
  // BPHS Ch 80 v.16 — aggregate affliction check
  // NOTE: waning Moon as malefic deferred to Sprint 1.5 (Shadbala work).
  // Using engine's static isMalefic/isBenefic sets here.
  {
    const maleficsIn7 = h7planets.filter(p => isMalefic(p));
    const beneficsIn7 = h7planets.filter(p => isBenefic(p));
    if (maleficsIn7.length >= 2 && beneficsIn7.length === 0 &&
        !planetAspectsHouse('Jupiter', chart, 7)) {
      findings.push(rule('7TH-MULTI-MAL-NO-BEN', 'BPHS Ch 80 v.16',
        `Multiple malefics (${maleficsIn7.join(', ')}) in 7th house with no benefic present and no Jupiter aspect. Classical condition indicating partnership challenges. Marriage in this configuration is possible but requires conscious navigation — finding a partner whose chart provides the benefic support this placement lacks, and activating the right dasha windows.`,
        'HIGH', -3));
      marriageScore -= 3;
      denialScore += 1;
    } else if (maleficsIn7.length >= 1 && beneficsIn7.length === 0 &&
               !planetAspectsHouse('Jupiter', chart, 7)) {
      // Single malefic, no protection — moderate friction
      findings.push(rule('7TH-MAL-NO-BEN', 'BPHS Ch 80 v.16',
        `${maleficsIn7[0]} in 7th house with no benefic presence or Jupiter aspect. Partial classical affliction pattern. Marriage timing and partner quality require more conscious attention than charts with benefic protection on the partnership house.`,
        'MEDIUM', -1));
      marriageScore -= 1;
    }
  }

  // RULE: 8th lord afflicted — female marriage longevity (Saravali Stree Jataka)
  if (chart.gender === 'F') {
    const l8 = lordOf(8, chart);
    const l8sign = signOfPlanet(l8, chart);
    const l8house = houseOfPlanet(l8, chart);
    const l8dig  = getDignity(l8, l8sign);
    if (l8dig === 'DEBILITATED' || l8dig === 'ENEMY') {
      const severity = l8dig === 'DEBILITATED' ? 1.0 : 0.5;
      const h8malefics = planetsInHouse(8, chart).filter(p => isMalefic(p));
      const h8benefics = planetsInHouse(8, chart).filter(p => isBenefic(p));
      const extraStress = (h8malefics.length > 0 && h8benefics.length === 0) ? 0.3 : 0;
      const totalFriction = Math.min(1.0, severity + extraStress);
      if (totalFriction >= 0.5) {
        const scoreAdj = Math.round(-3 * totalFriction);
        findings.push(rule('MARR-DENIAL-8TH-LORD-F', 'Saravali Stree Jataka',
          `8th lord (${l8}) ${l8dig.toLowerCase()} in female chart — classical condition affecting marriage longevity. The 8th house represents the life-of-marriage in the Stree Jataka tradition. Partnership in this configuration benefits from grounding in stability, partner vetting, and conscious long-term commitment patterns. This does not exclude marriage — it indicates the longevity channel requires attention.`,
          'HIGH', scoreAdj));
        marriageScore += scoreAdj;
        denialScore += 1;
      }
    }
  }

  // ── A4/A5: CLASSICAL MARRIAGE RULES — Early Marriage Indicators ───────
  // User's classical list, Early Marriage #1-7

  // Early #1: Venus in Lagna (already in Venus-house loop; supplement with score)
  const vSignE = signOfPlanet('Venus', chart);
  const vHouseE = houseOfPlanet('Venus', chart);
  if (vHouseE === 1) {
    // Venus in 1st is covered by Venus-house loop narrative. Add marriage-opening score here.
    findings.push(rule('VENUS-LAGNA-MARRY', 'BPHS Ch 80 v.6',
      `Venus in lagna — strong marriage indicator. The marriage karaka in the self house creates strong desire for partnership and magnetic personal appeal. Classical early-marriage indicator when combined with favorable dasha.`,
      'MEDIUM', 2));
    marriageScore += 2;
  }

  // Early #2: Venus in own/exalted + 7th lord in benefic sign — conjunctive check
  const vDigE2 = getDignity('Venus', vSignE);
  const l7SignBenefic = isBenefic(l7); // 7th lord being a benefic planet (not sign-quality check)
  const l7SignIdx = typeof l7sign === 'number' ? l7sign : null;
  if ((vDigE2 === 'OWN' || vDigE2 === 'EXALTED') && l7 && !isMalefic(l7)) {
    findings.push(rule('VENUS-OWN-L7-BENEFIC', 'BPHS Ch 80',
      `Venus in own/exalted state AND 7th lord is a benefic planet (${l7}) — combined classical early-marriage indicator. Both the marriage karaka and the partnership-house lord are well-configured.`,
      'HIGH', 3));
    marriageScore += 3;
  }

  // Early #3: Venus + Mars combination in 7th — passionate early partnership
  if (h7planets.includes('Venus') && h7planets.includes('Mars')) {
    findings.push(rule('VENUS-MARS-7TH', 'BPHS Ch 80 + PD Ch 12 v.22',
      `Venus and Mars conjunct in 7th house — intense romantic energy, passionate partnership drive. Classical early-marriage indicator. The relationship may begin with strong physical attraction. Requires Mangal Dosha assessment if Mars-in-7th conditions apply.`,
      'MEDIUM', 2));
    marriageScore += 2;
  }

  // Early #4: Lagna Lord in 7th + 7th Lord in kendra
  const l1 = lordOf(1, chart);
  const l1house = houseOfPlanet(l1, chart);
  const l7inKendra = l7house && [1,4,7,10].includes(l7house);
  if (l1house === 7 && l7inKendra) {
    findings.push(rule('L1-7TH-L7-KENDRA', 'BPHS Ch 80',
      `Lagna lord in 7th AND 7th lord in kendra — double partnership activation. Classical early-marriage indicator. The native's identity and partnership house are strongly interlocked.`,
      'HIGH', 3));
    marriageScore += 3;
  }

  // Early #5: Lagna Lord + 7th Lord conjunct in Lagna
  if (l1 && l7 && l1 !== l7 && houseOfPlanet(l1, chart) === 1 && houseOfPlanet(l7, chart) === 1) {
    findings.push(rule('L1-L7-IN-LAGNA', 'BPHS Ch 80',
      `Lagna lord and 7th lord conjunct in the lagna — classical early marriage yoga. Both self and partnership rulers unite in the house of self, making partnership a central life theme.`,
      'HIGH', 4));
    marriageScore += 4;
  }

  // Early #6: Venus in 6th with Sun or Mars
  if (vHouseE === 6) {
    const h6 = planetsInHouse(6, chart);
    if (h6.includes('Sun') || h6.includes('Mars')) {
      findings.push(rule('VENUS-6TH-SUN-MARS', 'BPHS Ch 80',
        `Venus in 6th house with Sun or Mars — classical early-marriage indicator despite the dusthana placement. The combination activates the desire for partnership through competition or service. Marriage through work environment or competitive contexts is a classical reading.`,
        'MEDIUM', 1));
      marriageScore += 1;
    }
  }

  // ── A5: CLASSICAL — Normal Marriage Indicators ────────────────────────
  // User's classical list, Normal Marriage #1-2

  // Normal #1: Marriage axis clean of malefic influence — aggregate check
  {
    const l1sig = signOfPlanet(l1, chart);
    const l1dig = getDignity(l1, l1sig);
    const l7dig = getDignity(l7, l7sign);
    const vDig  = getDignity('Venus', vSignE);
    const moonSig = signOfPlanet('Moon', chart);
    const moonDig = getDignity('Moon', moonSig);
    const allClean = !_isDignityWeak(l1dig) && !_isDignityWeak(l7dig) &&
                     !_isDignityWeak(vDig) && !_isDignityWeak(moonDig) &&
                     !h7planets.some(p => isMalefic(p)) &&
                     !planetAspectsHouse('Saturn', chart, 1) &&
                     !planetAspectsHouse('Saturn', chart, 7);
    if (allClean) {
      findings.push(rule('MARRIAGE-AXIS-CLEAN', 'BPHS Ch 80',
        `Marriage axis is clear of major malefic influence — lagna/7th lord, Venus, and Moon all in acceptable dignity with no malefics in 7th or Saturn aspecting key points. Classical indicator of standard, unobstructed marriage timing.`,
        'MEDIUM', 2));
      marriageScore += 2;
    }
  }

  // Normal #2: Venus + 7th lord both favorable in D9 — combined navamsha check
  if (chart.navamsha) {
    const vNavH = chart.navamsha['Venus']?.house;
    const l7NavSign = chart.navamsha[l7]?.sign;
    const l7NavDig = l7NavSign !== undefined ? getDignity(l7, l7NavSign) : null;
    const vNavGood = vNavH && [1,4,5,7,9,10].includes(vNavH);
    const l7NavGood = l7NavDig && (l7NavDig === 'EXALTED' || l7NavDig === 'OWN' || l7NavDig === 'MOOLATRIKONA');
    if (vNavGood && l7NavGood) {
      findings.push(rule('D9-VENUS-L7-BOTH', 'BPHS Ch 27 + JM Ch 2',
        `Both Venus and 7th lord are well-placed in Navamsha (D9) — combined D9 confirmation of marriage quality. At the soul-level chart, both the karaka and the partnership lord are strong. Deep marriage fulfillment indicated.`,
        'HIGH', 3));
      marriageScore += 3;
    }
  }

  // ── A5: CLASSICAL — Late Marriage Indicators ─────────────────────────
  // User's classical list, Late Marriage #1-4

  // Late #1: Saturn in Lagna (BPHS — not just Saturn-in-7)
  const satLagna = chart.planets['Saturn'];
  if (satLagna && houseOfPlanet('Saturn', chart) === 1) {
    const satLagnaDig = getDignity('Saturn', signOfPlanet('Saturn', chart));
    if (satLagnaDig !== 'EXALTED' && satLagnaDig !== 'OWN' && satLagnaDig !== 'MOOLATRIKONA') {
      findings.push(rule('SAT-IN-LAGNA-DELAY', 'BPHS Ch 80 + Late Marriage',
        `Saturn in lagna (not exalted/own) — classical late-marriage indicator. Saturn's restraining nature in the self-house delays the native's readiness for partnership. Marriage typically arrives after Saturn has been "processed" through maturity.`,
        'MEDIUM', -1));
      marriageScore -= 1;
      delayScore += 2;
    }
  ...
  // Late #2+3: Saturn aspecting marriage-axis planets (l1, l7, Venus, Moon, Rahu)
  {
    const satAspects1 = planetAspectsHouse('Saturn', chart, 1);
    const satAspects7 = planetAspectsHouse('Saturn', chart, 7);
    const satConjVenus = conjunct('Saturn','Venus', chart);
    const satConjMoon  = conjunct('Saturn','Moon',  chart);
    const satConjRahu  = conjunct('Saturn','Rahu',  chart);
    const satAspectsL1 = l1 && planetAspectsHouse('Saturn', chart, houseOfPlanet(l1, chart));
    const satAspectsL7 = l7 && planetAspectsHouse('Saturn', chart, houseOfPlanet(l7, chart));

    const satAxisCount = [satAspects1, satAspects7, satAspectsL1, satAspectsL7].filter(Boolean).length;
    const satKarakaCount = [satConjVenus, satConjMoon, satConjRahu].filter(Boolean).length;

    if (satAxisCount >= 2) {
      findings.push(rule('SAT-MULTI-ASPECT-AXIS', 'BPHS Ch 80 + Late Marriage',
        `Saturn aspects multiple marriage-axis points — 2+ of {lagna, 7th, lagna lord, 7th lord}. Compounded Saturn delay pattern. Marriage arrives with deliberate patience; typically after Saturn transits through its own conditioning cycle.`,
        'HIGH', -2));
      marriageScore -= 2;
      delayScore += 2;
    } else if (satAxisCount === 1) {
      findings.push(rule('SAT-AXIS-ASPECT', 'BPHS Ch 80',
        `Saturn aspects a marriage-axis point (lagna, 7th, or their lords). Single Saturn delay indicator. Mild delay pattern.`,
        'MEDIUM', -1));
      marriageScore -= 1;
      delayScore += 1;
    }
    ...
    if (satKarakaCount >= 2) {
      findings.push(rule('SAT-MULTI-KARAKA', 'BPHS Ch 80 + Late Marriage',
        `Saturn influences multiple marriage karakas (Venus, Moon, Rahu) — compound influence on the planets that "want" marriage. Marriage desire is under Saturnine constraint; conscious work on opening to partnership is classical advice.`,
        'HIGH', -2));
      marriageScore -= 2;
      delayScore += 2;
    } else if (satKarakaCount === 1) {
      const k = satConjVenus ? 'Venus' : satConjMoon ? 'Moon' : 'Rahu';
      findings.push(rule('SAT-KARAKA-ASPECT', 'BPHS Ch 80',
        `Saturn influences marriage karaka ${k} — single karaka constraint. Moderate delay pattern.`,
        'MEDIUM', -1));
      marriageScore -= 1;
      delayScore += 1;
    }
  ...
  // Late #4: Venus in Mars/Saturn Navamsha sign (deferred in Chart.js Sprint 1, encoding here)
  // Mars-ruled signs in D9: Aries (0) and Scorpio (7). Saturn-ruled: Capricorn (9) and Aquarius (10).
  if (chart.navamsha) {
    const vNav = chart.navamsha['Venus'];
    if (vNav && vNav.sign !== undefined) {
      if ([0, 7].includes(vNav.sign)) {
        findings.push(rule('VENUS-MARS-NAV', 'BPHS Ch 27 + Classical',
          `Venus in Mars-ruled Navamsha (${SIGN_PROPS[vNav.sign]?.name}) — classical late-marriage indicator. Mars energy in the soul-level placement of Venus creates tension between desire and fulfillment. Partnership requires working through Mars themes (courage, directness, independence) at a soul level.`,
          'MEDIUM', -1));
        marriageScore -= 1;
        delayScore += 1;
      } else if ([9, 10].includes(vNav.sign)) {
        findings.push(rule('VENUS-SAT-NAV', 'BPHS Ch 27 + Classical',
          `Venus in Saturn-ruled Navamsha (${SIGN_PROPS[vNav.sign]?.name}) — classical late-marriage indicator. Saturn energy in Venus's D9 placement creates the pattern of delayed but enduring love. Partnership arrives after patience; tends to be serious and long-lasting once formed.`,
          'MEDIUM', -1));
        marriageScore -= 1;
        delayScore += 1;
      }
  ...
  // ── A5: CLASSICAL — Delayed Marriage Indicators ───────────────────────
  // User's classical list, Delayed #1-3 (Delayed #4 = Papakartari, done in A1; #5 = Venus-in-8 in Venus loop)

  // Delayed #1: Saturn + Mars in Lagna or 7th
  {
    const h1 = planetsInHouse(1, chart);
    const satMarsBoth7 = h7planets.includes('Saturn') && h7planets.includes('Mars');
    const satMarsBoth1 = h1.includes('Saturn') && h1.includes('Mars');
    if (satMarsBoth7 || satMarsBoth1) {
      const where = satMarsBoth7 ? '7th' : 'lagna';
      findings.push(rule('SAT-MARS-CONJ-AXIS', 'BPHS Ch 80 + PD Ch 9 + Delayed',
        `Saturn and Mars conjunct in ${where} — combined delay indicators in a critical marriage house. Classical delayed-marriage pattern. Partnership is possible but arrives after processing both Saturnine restraint and Martian aggression. Strong remedies classically advised.`,
        'HIGH', -3));
      marriageScore -= 3;
      delayScore += 3;
    }
  ...
  // Delayed #2: Venus AND Moon both afflicted by Saturn and Mars
  {
    const vAffSat = conjunct('Saturn','Venus', chart) || planetAspectsHouse('Saturn', chart, vHouseE);
    const vAffMars = conjunct('Mars','Venus', chart) || planetAspectsHouse('Mars', chart, vHouseE);
    const moonHouse = houseOfPlanet('Moon', chart);
    const mAffSat = conjunct('Saturn','Moon', chart) || (moonHouse && planetAspectsHouse('Saturn', chart, moonHouse));
    const mAffMars = conjunct('Mars','Moon', chart) || (moonHouse && planetAspectsHouse('Mars', chart, moonHouse));
    if ((vAffSat || vAffMars) && (mAffSat || mAffMars)) {
      findings.push(rule('VENUS-MOON-AFFLICTED', 'BPHS Ch 80 + Delayed',
        `Both Venus and Moon are afflicted by Saturn or Mars — both primary marriage-nature planets under dual malefic pressure. Classical delayed-marriage pattern. Partnership requires working through this compounded constraint; right timing and partner alignment matter significantly.`,
        'HIGH', -2));
      marriageScore -= 2;
      delayScore += 2;
    }
  ...
  // Delayed #3: 7th house/lord afflicted with malefics AND no benefic aspect
  // (Aggregate without-benefic-protection check for 7th axis)
  {
    const has7Malefic = h7planets.some(p => isMalefic(p));
    const hasL7InDust = l7house && [6,8,12].includes(l7house);
    const hasBenProtect = h7planets.some(p => isBenefic(p)) ||
                          ['Jupiter','Venus','Mercury','Moon'].some(p => planetAspectsHouse(p, chart, 7));
    if ((has7Malefic || hasL7InDust) && !hasBenProtect) {
      findings.push(rule('7TH-AXIS-AFFLICTED-NO-BEN', 'BPHS Ch 80 + Delayed',
        `7th house or lord afflicted by malefics with no benefic protection on the partnership axis. Classical delayed-marriage pattern. The absence of benefic support means the partnership house must work harder for positive expression.`,
        'HIGH', -2));
      marriageScore -= 2;
      delayScore += 2;
    }
  ...
  // ── A5: DENIAL RULES (9 classical) — use sensitivity-framework framing ──
  // Rule IDs contain MARR-DENIAL prefix (triggers isFindingSensitive detection).
  // Verdict text NEVER uses "denied/denial/no marriage/will not marry/rejection".
  // All 9 rules increment denialScore per classical weight.
  // Forbidden words lint: verified in smoke tests below.

  // Denial #1: Venus in 8th + Mercury or Saturn also in 8th
  {
    const h8 = planetsInHouse(8, chart);
    if (vHouseE === 8 && (h8.includes('Mercury') || h8.includes('Saturn'))) {
      const companion = h8.includes('Mercury') && h8.includes('Saturn') ? 'Mercury and Saturn' :
                        h8.includes('Mercury') ? 'Mercury' : 'Saturn';
      findings.push(rule('MARR-DENIAL-VENUS-8-MAL', 'Classical Denial List #1 + BPHS Ch.18',
        `Venus in 8th house with ${companion} — classical condition affecting marriage expression. Partnership in this configuration typically requires alignment around depth, shared resources, or transformational work. The 8th house as partnership channel often produces deeply bonded but unconventional relationships. Conscious navigation and right partner alignment matter more in this configuration than in general.`,
        'HIGH', -2));
      marriageScore -= 2;
      denialScore += 2;
    }
  ...
  // Denial #2: Venus debilitated in BOTH D1 and D9
  {
    const vDigD1 = getDignity('Venus', vSignE);
    if (vDigD1 === 'DEBILITATED' && chart.navamsha) {
      const vNavSign = chart.navamsha['Venus']?.sign;
      const vDigD9 = vNavSign !== undefined ? getDignity('Venus', vNavSign) : null;
      if (vDigD9 === 'DEBILITATED') {
        findings.push(rule('MARR-DENIAL-VENUS-D1D9-DEBIL', 'Classical Denial List #2 + BPHS + UK',
          `Venus debilitated in both D1 (${SIGN_PROPS[vSignE]?.name}) and D9 (${SIGN_PROPS[vNavSign]?.name}) — dual-level karaka weakness. Both in the natal chart and the soul chart, the marriage karaka faces challenging expression. Partnership in this configuration benefits from Venus-strengthening remedies and patient conscious work on relationship patterns. Later-in-life partnership or non-traditional partnership forms are classical responses.`,
          'HIGH', -3));
        marriageScore -= 3;
        denialScore += 2;
      }
  ...
  // Denial #3: Lagna/7th and their lords in fixed signs + Moon in movable sign
  {
    const l1SignIdx = chart.lagna;
    const l1LordSignIdx = typeof signOfPlanet(l1, chart) === 'number' ? signOfPlanet(l1, chart) : null;
    const l7SignIdx2 = typeof l7sign === 'number' ? l7sign : null;
    const l7LordSignIdx = typeof signOfPlanet(l7, chart) === 'number' ? signOfPlanet(l7, chart) : null;
    const moonSignIdx = typeof signOfPlanet('Moon', chart) === 'number' ? signOfPlanet('Moon', chart) : null;

    const lagnaFixed = isFixedSign(l1SignIdx);
    const l7Fixed = l7SignIdx2 !== null && isFixedSign(l7SignIdx2);
    const l1LordFixed = l1LordSignIdx !== null && isFixedSign(l1LordSignIdx);
    const l7LordFixed = l7LordSignIdx !== null && isFixedSign(l7LordSignIdx);
    const moonMovable = moonSignIdx !== null && isMovableSign(moonSignIdx);

    if ((lagnaFixed && l7Fixed) && (l1LordFixed || l7LordFixed) && moonMovable) {
      findings.push(rule('MARR-DENIAL-FIXED-SIGNS', 'Classical Denial List #3 + BPHS',
        `Lagna and 7th house in fixed signs, their lords in fixed signs, and Moon in movable sign — classical pattern of rigidity and restlessness in partnership. The native's self-structure is fixed while emotional nature is changeable, creating friction in sustained relationship. Partnership benefits from partners with compatible modality blend and from conscious work on flexibility.`,
        'HIGH', -2));
      marriageScore -= 2;
      denialScore += 2;
    }
  ...
  // Denial #4: Venus combust + debilitated + in Papakartari (triple)
  {
    const vDigD4 = getDignity('Venus', vSignE);
    const vCombust = isCombust('Venus', chart);
    const vPapakartari = isInPapakartari('Venus', chart);
    // At least 2 of 3 conditions for moderate detection; all 3 for severe
    const conditions = [vCombust, vDigD4 === 'DEBILITATED', vPapakartari].filter(Boolean).length;
    if (conditions === 3) {
      findings.push(rule('MARR-DENIAL-VENUS-TRIPLE', 'Classical Denial List #4 + PD Ch.12',
        `Venus is simultaneously combust, debilitated, and in Papakartari Yoga — triple classical condition on the marriage karaka. This configuration describes the most demanding partnership path in this chart. Partnership is possible through conscious remediation, right dasha activation, and finding a partner whose chart provides the Venus support this one lacks. Remedies specifically focused on Venus are classically important here.`,
        'HIGH', -4));
      marriageScore -= 4;
      denialScore += 3;
    } else if (conditions === 2) {
      const which = [vCombust ? 'combust' : null, vDigD4 === 'DEBILITATED' ? 'debilitated' : null, vPapakartari ? 'Papakartari' : null].filter(Boolean).join(' + ');
      findings.push(rule('MARR-DENIAL-VENUS-DOUBLE', 'Classical Denial List #4 partial + PD Ch.12',
        `Venus carries two classical conditions simultaneously (${which}). Partnership in this configuration requires more awareness and deliberate effort than charts with unencumbered Venus. Right timing and remedies if open are important.`,
        'MEDIUM', -2));
      marriageScore -= 2;
      denialScore += 1;
    }
  ...
  // Denial #5: Malefics in lagna + 7th + 12th with Moon in 5th (waning Moon structural note)
  // NOTE: waning Moon as malefic deferred to Sprint 1.5.
  // Encoding structural condition: malefics in 1/7/12 + Moon in 5th.
  {
    const h12 = planetsInHouse(12, chart);
    const h1c = planetsInHouse(1, chart);
    const malIn1 = h1c.filter(p => isMalefic(p)).length > 0;
    const malIn7 = h7planets.filter(p => isMalefic(p)).length > 0;
    const malIn12 = h12.filter(p => isMalefic(p)).length > 0;
    const moonIn5 = houseOfPlanet('Moon', chart) === 5;
    if (malIn1 && malIn7 && malIn12 && moonIn5) {
      findings.push(rule('MARR-DENIAL-MAL-TRIANGLE', 'Classical Denial List #5 + BPHS',
        `Malefics occupy the 1st, 7th, and 12th houses simultaneously with Moon in 5th — classical multi-house affliction pattern. The partnership channel (7th), self-expression (1st), and release (12th) are all under malefic pressure while emotional nature (Moon/5th) seeks creative rather than partnership expression. This configuration is unusual; partnership typically requires exceptional dasha timing and remedies.`,
        'HIGH', -3));
      marriageScore -= 3;
      denialScore += 2;
    }
  ...
  // Denial #6: Affliction of 2nd + 7th house with planets in fixed signs
  {
    const h2 = planetsInHouse(2, chart);
    const mal2 = h2.filter(p => isMalefic(p)).length > 0;
    const mal7 = h7planets.filter(p => isMalefic(p)).length > 0;
    const h2sign = (chart.lagna + 1) % 12; // sign of 2nd house
    const h2Fixed = isFixedSign(h2sign);
    const h7sign2 = typeof l7sign === 'number' ? l7sign : null;
    const h7Fixed = h7sign2 !== null && isFixedSign(h7sign2);
    if (mal2 && mal7 && (h2Fixed || h7Fixed)) {
      findings.push(rule('MARR-DENIAL-2ND-7TH-FIXED', 'Classical Denial List #6 + BPHS',
        `2nd and 7th houses both afflicted by malefics, with one or both in fixed signs — combined family-wealth and partnership pressure in a rigid sign configuration. Classical pattern associated with partnership arriving with difficulty. Conscious effort on both family-dynamics and personal-commitment patterns is classically advised.`,
        'HIGH', -2));
      marriageScore -= 2;
      denialScore += 2;
    }
  ...
  // Denial #7: Lagna lord + 7th lord + Venus all in Saturn Navamsha
  // Saturn-ruled D9 signs: Capricorn (9) and Aquarius (10)
  if (chart.navamsha) {
    const l1NavSign = chart.navamsha[l1]?.sign;
    const l7NavSign2 = chart.navamsha[l7]?.sign;
    const vNavSign2 = chart.navamsha['Venus']?.sign;
    const l1SatNav = l1NavSign !== undefined && [9,10].includes(l1NavSign);
    const l7SatNav = l7NavSign2 !== undefined && [9,10].includes(l7NavSign2);
    const vSatNav  = vNavSign2  !== undefined && [9,10].includes(vNavSign2);
    if (l1SatNav && l7SatNav && vSatNav) {
      findings.push(rule('MARR-DENIAL-SAT-NAVAMSHA', 'Classical Denial List #7 + BPHS Ch 27',
        `Lagna lord, 7th lord, and Venus are all in Saturn-ruled Navamsha signs — at the soul level, all three key marriage factors carry Saturn's restricting, delaying energy. This configuration typically produces very late marriage or partnership with heavy karmic themes. Conscious engagement with Saturn's themes (discipline, responsibility, long-term commitment) is the classical path through.`,
        'HIGH', -3));
      marriageScore -= 3;
      denialScore += 2;
    }
  ...
  // Denial #8: All three natural marriage karakas (Venus, Moon, Rahu) highly afflicted
  {
    const vDig8 = getDignity('Venus', vSignE);
    const moonSig8 = signOfPlanet('Moon', chart);
    const moonDig8 = getDignity('Moon', moonSig8);
    const rahuHouse = houseOfPlanet('Rahu', chart);
    const vAfflicted = vDig8 === 'DEBILITATED' || vDig8 === 'ENEMY' || isCombust('Venus', chart);
    const moonAfflicted = moonDig8 === 'DEBILITATED' || moonDig8 === 'ENEMY' ||
                          conjunct('Saturn','Moon', chart) || conjunct('Mars','Moon', chart);
    const rahuAfflicted = rahuHouse && [6,8,12].includes(rahuHouse);
    if (vAfflicted && moonAfflicted && rahuAfflicted) {
      findings.push(rule('MARR-DENIAL-THREE-KARAKAS', 'Classical Denial List #8 + BPHS',
        `All three natural marriage karakas — Venus (desire and beauty), Moon (emotional bonding), and Rahu (karmic desire) — are simultaneously afflicted. This configuration describes the most challenging marriage pattern in classical texts. Partnership is not excluded, but the path is highly non-standard. External conditions (dasha, partner chart, remedies) matter enormously in this configuration.`,
        'HIGH', -4));
      marriageScore -= 4;
      denialScore += 3;
    }
  ...
  // Denial #9: Venus in 7th OR 7th lord OR Navamsha lagna lord — all afflicted
  {
    const venusIn7th = h7planets.includes('Venus');
    const vDig9 = getDignity('Venus', vSignE);
    const l7Dig9 = getDignity(l7, l7sign);
    const navLagna = chart.navamsha?.['_lagnaLord'] || null; // navamsha lagna lord if computed
    // Composite: require at least 2 of 3 to be afflicted (strict condition = all 3)
    const vAff9 = venusIn7th && (vDig9 === 'DEBILITATED' || vDig9 === 'ENEMY' || isCombust('Venus', chart));
    const l7Aff9 = l7Dig9 === 'DEBILITATED' || l7Dig9 === 'ENEMY' || (l7house && [6,8,12].includes(l7house));
    // For navamsha lagna lord: if not available, skip this prong
    const navLordAff = navLagna ? (getDignity(navLagna, signOfPlanet(navLagna, chart)) === 'DEBILITATED') : false;

    const afflictCount = [vAff9, l7Aff9, navLordAff].filter(Boolean).length;
    if (afflictCount >= 2) {
      const severity = afflictCount === 3 ? 3 : 2;
      const scoreAdj = afflictCount === 3 ? -3 : -2;
      findings.push(rule('MARR-DENIAL-TRIPLE-AFFLICT', 'Classical Denial List #9 + BPHS',
        `${afflictCount} of 3 key marriage indicators simultaneously afflicted (Venus in 7th, 7th lord, Navamsha lagna lord). Classical compounded affliction on the partnership axis. Marriage in this configuration requires specific conditions: right dasha alignment, conscious partner vetting, and often some form of spiritual or remedial work. The chart indicates a more demanding path to partnership, not impossibility.`,
        afflictCount === 3 ? 'HIGH' : 'MEDIUM', scoreAdj));
      marriageScore += scoreAdj;
      denialScore += severity;
    }
  }

  // ── HELPER for A5 — dignity weakness check ────────────────────────────
  // Used in MARRIAGE-AXIS-CLEAN check above
  function _isDignityWeak(dig) { return dig === 'DEBILITATED' || dig === 'ENEMY'; }
  ...
  // ── PHASE 2: 5-TIER marriageStatus CASCADE ───────────────────────────
  // Tier ordering: denial-severity takes priority over raw score.
  // Highest-severity tiers use MORE careful language, not less.
  // Comparison framing ("worse than other charts") replaced with
  // "in this configuration than in general" throughout.
  if (denialScore >= 5 && marriageScore <= -3) {
    // Severe: multiple denial conditions, minimal positive support
    marriageStatus = 'UNUSUAL CONFIGURATION: Multiple classical conditions affecting marriage timing and form converge in this chart. Partnership in this lifetime takes non-standard paths and benefits from conscious navigation, remedies if the person is open, right dasha activation, and partner whose chart aligns. The chart shows the path requires more awareness in this configuration than in general — not impossibility. Late marriage, partnership without formal marriage, or marriage after significant inner work are classical responses to this configuration.';
  } else if (denialScore >= 5 && marriageScore > 0) {
    // Severe denial WITH partial positive factors — names both sides at highest care level
    marriageStatus = 'UNUSUAL CONFIGURATION WITH SUPPORTING FACTORS: Chart shows multiple significant classical conditions that affect marriage timing or form. The chart also carries supportive indicators — typically benefic conjunctions or aspects that provide partial relief. Both layers are real and neither cancels the other. Partnership in this configuration benefits from awareness that supportive factors are partial, not complete protection. Right timing, remedies if open, and partner whose chart provides what this one lacks all matter in this configuration than in general. Classical Vedic astrology recognizes mixed configurations often produce non-traditional partnership paths.';
  } else if (denialScore >= 3 && marriageScore <= 0) {
    // Moderate denial — existing careful message preserved
    ...
    // Mixed signals at moderate level: denial conditions AND positive factors both present
    marriageStatus = 'MIXED CONFIGURATION: Chart shows both supportive marriage indicators AND classical conditions that affect marriage timing or form. The supportive factors do not cancel the conditions — both are real. Partnership in this chart benefits from awareness of both: the natural support that makes marriage possible, and the patterns that require navigation. Right timing and conscious alignment matter more in this configuration than in general.';
  ...
  } else if (marriageScore >= 3) {
    marriageStatus = 'POSITIVE: Marriage is indicated with generally favorable conditions.';
  } else if (marriageScore >= 0) {
    marriageStatus = 'MIXED: Marriage is possible but conditional. Quality and timing depend on dasha activation and partner selection.';
  } else {
    marriageStatus = 'CONDITIONAL: Marriage requires conscious effort and right timing. Multiple delay factors present.';
  }
...
function evaluateChildren(chart) {
  const findings = [];
  // PHASE 2: Age gate
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 18) {
    findings.push(rule('AGE-TOO-EARLY-CHILDREN', 'Classical age threshold', `AGE-GATED: Chart subject age ${chart.subjectAge} — children timing themes are not meaningful at this life stage.`, 'HIGH', 0));
    return { findings, childScore: 0, delayScore: 0, ageGated: true,
      verdict: 'AGE-GATED: Children timing analysis becomes meaningful in adulthood.' };
  }
  const l5 = lordOf(5, chart);
...
function evaluateCareer(chart) {
  const findings = [];
  // PHASE 2: Age gate
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 16) {
    findings.push(rule('AGE-TOO-EARLY-CAREER', 'Classical age threshold', `AGE-GATED: Chart subject age ${chart.subjectAge} — career themes activate through teenage dasha/transit maturity.`, 'HIGH', 0));
    return { findings, careerScore: 0, fields: [], ageGated: true,
      verdict: 'AGE-GATED: Career timing analysis becomes meaningful from age 16+. Chart career structure described for reference.' };
  }
  ...
  if (chart.dasha) {
    const { maha, antar } = chart.dasha;
    // PHASE 2: Extend maha triggers to include AmK (Jaimini — primary career activation)
    const amkPlanet = chart.amatyakaraka; // pre-cached by VedicRulesEngine.evaluate()
    // AmK source: engine.getAmatyakaraka() via cached c.amatyakaraka.
    // Note: chart.js Part 2a enrichment computes AmK from longitude%30 which can differ
    // for Rahu edge cases. Engine maintains internal consistency using its own karaka
    // calculation. AmK convergence between engine and enrichment is separate Sprint 2 cleanup.
    const careerMahaTriggers = new Set([l10, 'Sun', 'Saturn']);
    const careerAntarTriggers = new Set([l10, 'Sun']);
    if (amkPlanet) {
      careerMahaTriggers.add(amkPlanet);
      careerAntarTriggers.add(amkPlanet);
    }
    const isCareerMaha = careerMahaTriggers.has(maha);
    const isCareerAntar = careerAntarTriggers.has(antar);
    if (isCareerMaha || isCareerAntar) {
    ...
    // PHASE 2: Separate explicit AmK rule when AmK dasha active
    // Jaimini Sutras + BPHS Ch 47 — AmK dasha = primary Jaimini career-activation period
    if (amkPlanet && (maha === amkPlanet || antar === amkPlanet)) {
      findings.push(rule('DASHA-AMK-CAREER', 'JM (Jaimini Sutras) Ch.4 + BPHS Ch.47',
        `TIMING: Running ${maha}-${antar} dasha activates Amatyakaraka ${amkPlanet} — Jaimini's career significator. Classical Jaimini timing identifies this as a primary career-activation period. Professional recognition, expansion, or significant career events are classically active under AmK dasha.`,
        'HIGH', 3));
      careerScore += 3;
    }
    // Age-gated Saturn dasha check
    if (chart.subjectAge !== null && chart.subjectAge !== undefined) {
      const inCareerAgeRange = chart.subjectAge >= 18 && chart.subjectAge <= 50;
      if (inCareerAgeRange && maha === 'Saturn') {
        findings.push(rule('DASHA-SAT-CAREER-AGE', 'BPHS Ch.47 + Age-gated',
          `TIMING: Saturn mahadasha active during career-active age (${chart.subjectAge}) — Saturn activates sustained work and long-term professional discipline. Classical texts note Saturn dasha in career-active years as the "grind that pays" period: slower than other career periods but exceptionally durable.`,
          'MEDIUM', 2));
        careerScore += 2;
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════════
  // PHASE 2 — CAREER RULE ADDITIONS
  // B1: AmK dasha (done above). B2-B4: 9 net-new + 5 extensions + l2/l11 helpers.
  // ══════════════════════════════════════════════════════════════════════

  // ── B2: SUN STRONG FOR AUTHORITY (net-new) ────────────────────────────
  // BPHS Ch 34 — Sun as karaka of 10th house (authority). Previously only Sun-IN10 fired.
  {
    const sunSign = signOfPlanet('Sun', chart);
    const sunDig  = getDignity('Sun', sunSign);
    if (sunDig === 'EXALTED' || sunDig === 'OWN' || sunDig === 'MOOLATRIKONA' || sunDig === 'FRIEND') {
      findings.push(rule('SUN-AUTHORITY-STRONG', 'BPHS Ch 34 — Sun rules authority',
        `Sun strong (${sunDig.toLowerCase()}) — authority karaka well-placed. BPHS Ch 34 identifies Sun as one of four career karakas. Strong natal Sun supports visibility, leadership, and command in career. Authority-based professions (government, administration, medicine) are classically favored.`,
        'MEDIUM', 2));
      careerScore += 2;
    } else if (sunDig === 'DEBILITATED' || sunDig === 'ENEMY') {
      findings.push(rule('SUN-AUTHORITY-WEAK', 'BPHS Ch 34',
        `Sun weak (${sunDig.toLowerCase()}) — authority karaka under stress. Visibility and command in career require more conscious effort. Adversarial relationships with authority figures are a classical reading.`,
        'MEDIUM', -1));
      careerScore -= 1;
    }
  }

  // ── B3: SATURN STRONG FOR SUSTAINED WORK (EXTENSION of Sasha Yoga) ───
  // BPHS Ch 34 — Saturn rules sustained work. Sasha Yoga (kendra + own/exalt) covered in
  // evaluateYogas. This rule covers Saturn dignity in ANY placement for career.
  {
    const satSignC = signOfPlanet('Saturn', chart);
    const satDigC  = getDignity('Saturn', satSignC);
    const satHouseC = houseOfPlanet('Saturn', chart);
    if (satDigC === 'EXALTED' || satDigC === 'OWN' || satDigC === 'MOOLATRIKONA') {
      // Sasha Yoga in kendra = evaluateYogas fires +4. Here: add career-specific signal.
      const isKendra = satHouseC && [1,4,7,10].includes(satHouseC);
      if (!isKendra) {
        // Non-kendra strong Saturn — career signal, no Sasha Yoga
        findings.push(rule('SAT-WORK-STRONG', 'BPHS Ch 34 — Saturn rules sustained work',
          `Saturn strong (${satDigC.toLowerCase()}) in non-kendra house ${satHouseC} — sustained-work capacity confirmed outside Sasha Yoga. Career endurance, discipline, and persistence are structurally supported. Long-term professional staying power is classical.`,
          'MEDIUM', 2));
        careerScore += 2;
      }
      // Kendra case: Sasha Yoga fires in evaluateYogas (+4) — no double-fire here.
    } else if (satDigC === 'DEBILITATED' || satDigC === 'ENEMY') {
      findings.push(rule('SAT-WORK-WEAK', 'BPHS Ch 34 — Saturn debilitated career',
        `Saturn weak (${satDigC.toLowerCase()}) — sustained-work capacity under stress. Career in this chart benefits from structured discipline practice, as natural endurance for long-grind professions is lower. Industries requiring sustained routine (government, large organizations, agriculture) may produce more friction.`,
        'MEDIUM', -2));
      careerScore -= 2;
    }
  }

  // ── B4: AMATYAKARAKA FAVORABLE HOUSE (net-new) ────────────────────────
  // Jaimini Sutras — AmK placement determines career manifestation channel
  {
    const amkC = chart.amatyakaraka;
    if (amkC) {
      const amkH = houseOfPlanet(amkC, chart);
      if (amkH) {
        let amkHScore = 0;
        if ([1,4,5,7,9,10].includes(amkH)) amkHScore = 3;
        else if (amkH === 11) amkHScore = 2;
        else if ([6,8,12].includes(amkH)) amkHScore = -1;
        else amkHScore = 1;
        if (amkHScore !== 0) {
          const qualifier = amkHScore >= 3 ? 'excellently' : amkHScore === 2 ? 'favorably' :
                            amkHScore === 1 ? 'acceptably' : 'in a challenging house';
          findings.push(rule('AMK-HOUSE-CAREER', 'JM (Jaimini Sutras) Ch.1 + Ch.2',
            `Amatyakaraka ${amkC} in house ${amkH} — career significator placed ${qualifier}. Jaimini tradition reads AmK placement as the primary channel through which career manifests. House ${amkH} indicates this channel.`,
            amkHScore >= 3 ? 'HIGH' : amkHScore < 0 ? 'MEDIUM' : 'MEDIUM', amkHScore));
          careerScore += amkHScore;
        }
      }
    }
  }

  // ── B5: D10 LAGNA LORD STRONG (net-new) ───────────────────────────────
  // BPHS Ch 6 Dashamsha + Phaladeepika Ch 10
  // Engine has D10-L10 (10th lord in D10) but not D10 lagna lord.
  if (chart.navamsha && chart.lagna !== undefined) {
    // D10 lagna sign is computed by buildVargaChart which isn't accessible here directly.
    // Sprint 1 stores d10 in enrichedChart. We use the heuristic approach: if chart has
    // a d10 object (from Part 2a enrichment), use it; else skip.
    // This rule only fires when d10 data is available on chart object.
    const d10data = chart.d10;
    if (d10data && d10data.lagna !== undefined) {
      const d10lagnaLord = lordOf(d10data.lagna + 1, chart); // +1 to convert sign to house
      // Actually d10.lagna is a sign index (0-11). We need the lord of that sign.
      const SIGN_LORDS_C = ['Mars','Venus','Mercury','Moon','Sun','Mercury','Venus','Mars','Jupiter','Saturn','Saturn','Jupiter'];
      const d10LagnaLordP = SIGN_LORDS_C[d10data.lagna];
      if (d10LagnaLordP) {
        const d10LagnaLordDig = getDignity(d10LagnaLordP, signOfPlanet(d10LagnaLordP, chart));
        let score = 0;
        if (d10LagnaLordDig === 'EXALTED' || d10LagnaLordDig === 'OWN' || d10LagnaLordDig === 'MOOLATRIKONA') score = 3;
        else if (d10LagnaLordDig === 'FRIEND') score = 2;
        else if (d10LagnaLordDig === 'DEBILITATED') score = -2;
        if (d10data.d10LagnaApproximate) score = Math.round(score * 0.5); // half-weight if approximate
        if (score !== 0) {
          findings.push(rule('D10-LAGNA-LORD', 'BPHS Ch.6 (D10) + PD Ch.10',
            `D10 lagna lord ${d10LagnaLordP} in ${d10LagnaLordDig?.toLowerCase()||'?'} dignity — career significator in Dashamsha ascendant. The planet ruling D10 lagna represents the inherent direction of professional life as seen in the career-specific divisional chart.${d10data.d10LagnaApproximate ? ' (Half-weight: D10 lagna is approximate — precise birth time needed for full confirmation.)' : ''}`,
            score >= 3 ? 'HIGH' : 'MEDIUM', score));
          careerScore += score;
        }
      }
    }
  ...
  // ── B6: 10TH FROM KARAKAMSHA OCCUPIED (net-new) ───────────────────────
  // Jaimini Sutras Ch 2-3 — 10th from Karakamsha = deepest-level career indicator
  {
    const kkSign = chart._karakamsha; // set by evaluateJaiminiKarakas via computeKarakamsha
    if (kkSign !== undefined && kkSign !== null) {
      const tenthFromKK = (kkSign + 9) % 12;
      const CAREER_SIGS = new Set(['Jupiter','Venus','Mercury','Moon','Sun','Saturn']);
      const occupants = Object.entries(chart.planets || {})
        .filter(([p, pd]) => pd.sign === tenthFromKK && CAREER_SIGS.has(p))
        .map(([p]) => p);
      if (occupants.length > 0) {
        findings.push(rule('KKM-10TH-CAREER', 'JM (Jaimini Sutras) Ch.2-3',
          `Career significator(s) ${occupants.join(', ')} occupy the 10th sign from Karakamsha (${SIGN_PROPS[tenthFromKK]?.name}) — Jaimini's deepest-level career indicator. Classical Jaimini tradition reads this as the soul's career path confirmed at the karmic level.`,
          'HIGH', 3));
        careerScore += 3;
      }
    }
  ...
  // ── B7: 11TH LORD STRONG FOR CAREER INCOME (net-new for career context) ─
  // BPHS Ch 34 — 11th house = professional gains. Engine has l11 in wealth; pulling into career.
  {
    const l11c = lordOf(11, chart);
    const l11cSign = signOfPlanet(l11c, chart);
    const l11cDig = getDignity(l11c, l11cSign);
    if (l11cDig === 'EXALTED' || l11cDig === 'OWN' || l11cDig === 'MOOLATRIKONA' || l11cDig === 'FRIEND') {
      findings.push(rule('L11-CAREER-STRONG', 'BPHS Ch 34 — 11th house = gains',
        `11th lord ${l11c} well-placed (${l11cDig.toLowerCase()}) — income channel from career is structurally supported. The 11th house represents the fulfillment and gains from professional activity. Strong 11th lord indicates career translates effectively into income.`,
        'MEDIUM', 2));
      careerScore += 2;
    } else if (l11cDig === 'DEBILITATED' || l11cDig === 'ENEMY') {
      findings.push(rule('L11-CAREER-WEAK', 'BPHS Ch 34',
        `11th lord ${l11c} weakly placed (${l11cDig.toLowerCase()}) — career-income channel under stress. Hard work may not translate proportionally to gains without deliberate financial management.`,
        'MEDIUM', -1));
      careerScore -= 1;
    }
  }

  // ── B8: 2ND LORD FAVORABLE FOR CAREER INCOME (net-new for career context) ─
  // Phaladeepika Ch 13 — 2nd house = accumulated wealth retention from career
  {
    const l2c = lordOf(2, chart);
    const l2cSign = signOfPlanet(l2c, chart);
    const l2cDig = getDignity(l2c, l2cSign);
    if (l2cDig === 'EXALTED' || l2cDig === 'OWN' || l2cDig === 'MOOLATRIKONA' || l2cDig === 'FRIEND') {
      findings.push(rule('L2-CAREER-STRONG', 'PD Ch.13 — 2nd house income retention',
        `2nd lord ${l2c} well-placed (${l2cDig.toLowerCase()}) — career-income retention supported. The 2nd house represents accumulated wealth from professional activity. Strong 2nd lord indicates career earnings are retained and compounded.`,
        'MEDIUM', 1));
      careerScore += 1;
    }
  }

  // ── B9: 10TH HEAVILY AFFLICTED AGGREGATE CHECK (net-new) ─────────────
  // BPHS Ch 34 v.18 — multiple malefics in 10th without benefic protection
  // NOTE: waning Moon as malefic deferred to Sprint 1.5 (Shadbala work).
  {
    const h10 = planetsInHouse(10, chart);
    const maleficsIn10 = h10.filter(p => isMalefic(p));
    const beneficsIn10 = h10.filter(p => isBenefic(p));
    if (maleficsIn10.length >= 2 && beneficsIn10.length === 0) {
      // Check if any benefic aspects 10th
      const beneficAspects10 = ['Jupiter','Venus','Mercury','Moon'].some(p => planetAspectsHouse(p, chart, 10));
      const score = beneficAspects10 ? -2 : -3;
      findings.push(rule('10TH-MULTI-MAL', 'BPHS Ch 34 v.18',
        `Multiple malefics (${maleficsIn10.join(', ')}) in 10th house with no benefic in 10th${beneficAspects10 ? ' (benefic aspect provides partial relief)' : ' and no benefic aspect'}. Classical severe career affliction pattern. Career in this configuration requires strategic navigation around authority, competition, and professional visibility.`,
        'HIGH', score));
      careerScore += score;
    } else if (maleficsIn10.length >= 1 && beneficsIn10.length === 0) {
      findings.push(rule('10TH-MAL-NO-BEN', 'BPHS Ch 34 v.18',
        `${maleficsIn10[0]} in 10th house without benefic presence. Moderate career friction. Professional visibility and authority require more deliberate effort.`,
        'MEDIUM', -1));
      careerScore -= 1;
    }
  }

  // ── B10: 10TH LORD IN DUSTHANA — graduated severity (extension of existing rule) ──
  // Engine currently fires L10-H${l10house} with narrative. Adding explicit graduated score.
  if ([6,8,12].includes(l10house)) {
    const severityScore = l10house === 8 ? -3 : l10house === 12 ? -2 : -1; // 6th = upachaya, softer
    findings.push(rule('L10-DUSTHANA-CAREER', 'PD Ch.10',
      `10th lord ${l10} in ${l10house}th house (dusthana) — career channel compromised. House ${l10house} themes${l10house===6?' (service, competitors, litigation — with upachaya compensating effect)':l10house===8?' (transformation, obstacles, hidden work)':' (foreign, loss, dissolution)'}. Career expression through non-mainstream channels is the classical reading.`,
      'MEDIUM', severityScore));
    careerScore += severityScore;
  }
...
function evaluateWealth(chart) {
  ...
  // PHASE 2: Age gate
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 16) {
    findings.push(rule('AGE-TOO-EARLY-WEALTH', 'Classical age threshold', `AGE-GATED: Chart subject age ${chart.subjectAge} — wealth themes activate through working years.`, 'HIGH', 0));
    return { findings, wealthScore: 0, sources: [], ageGated: true,
      verdict: 'AGE-GATED: Wealth analysis becomes meaningful from age 16+.' };
  }
  const l2 = lordOf(2, chart);
...
function evaluateForeignTravel(chart) {
  ...
  // PHASE 2: Age gate
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 16) {
    findings.push(rule('AGE-TOO-EARLY-FOREIGN', 'Classical age threshold', `AGE-GATED: Chart subject age ${chart.subjectAge} — foreign travel themes activate in teenage years through relevant dashas.`, 'HIGH', 0));
    return { findings, foreignScore: 0, settlementLikelihood: 'unknown', ageGated: true,
      verdict: 'AGE-GATED: Foreign travel analysis becomes meaningful from teenage years onward.' };
  }
...
function evaluateEducation(chart) {
  const findings = [];
  // PHASE 2: Age gate
  if (chart.subjectAge !== null && chart.subjectAge !== undefined && chart.subjectAge < 6) {
    findings.push(rule('AGE-TOO-EARLY-EDU', 'Classical age threshold', `AGE-GATED: Chart subject age ${chart.subjectAge} — detailed education analysis becomes meaningful from age 6+.`, 'HIGH', 0));
    return { findings, eduScore: 0, fields: [], ageGated: true,
      verdict: 'AGE-GATED: Education analysis becomes meaningful from age 6+.' };
  }
  const l4 = lordOf(4, chart);
  const l5 = lordOf(5, chart);
  const l4house = houseOfPlanet(l4, chart);
...
class VedicRulesEngine {
  ...
  evaluate() {
    ...
    // PHASE 2: Normalize subjectAge. chart.js computes age from dob and passes as
    // chart.subjectAge. If absent (old callers), set null — age-gating degrades gracefully.
    // Engine NEVER calls new Date() — determinism preserved.
    if (c.subjectAge === undefined) c.subjectAge = null;
    ...
    return {
      ...
      // PHASE 2: age-gated domains return { verdict: 'AGE-GATED: ...' } directly (no COMPOUND rule).
      // cv() searches findings for COMPOUND rule ID — falls back to marriage.verdict if cv returns ''.
      marriage:       { verdict: cv(marriage.findings,'MARRIAGE-COMPOUND') || marriage.verdict || '', score:marriage.marriageScore, timing:marriage.timing, spouse:marriage.spouseDescription, rules:marriage.findings },
      children:       { verdict: cv(children.findings,'CHILDREN-COMPOUND') || children.verdict || '', score:children.childScore, rules:children.findings },
      ...
      career:         { verdict: cv(career.findings,'CAREER-COMPOUND') || career.verdict || '', score:career.careerScore, fields:career.fields, rules:career.findings },
      wealth:         { verdict: cv(wealth.findings,'WEALTH-COMPOUND') || wealth.verdict || '', score:wealth.wealthScore, sources:wealth.sources, rules:wealth.findings },
      ...
      foreignTravel:  { verdict: cv(foreignTravel.findings,'FOREIGN-COMPOUND') || foreignTravel.verdict || '', score:foreignTravel.foreignScore, settlement:foreignTravel.settlementLikelihood, rules:foreignTravel.findings },
      property:       { verdict:cv(property.findings,'PROPERTY-COMPOUND'), score:property.propertyScore, rules:property.findings },
      education:      { verdict: cv(education.findings,'EDU-COMPOUND') || education.verdict || '', score:education.eduScore, fields:education.fields, rules:education.findings },
