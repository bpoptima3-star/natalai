...
Do NOT write generic astrology. Make every line specific to this house placement.`;
      ...
      const processSection = async (sec) => {
        try {
          ...
          return { title: sec.title, html: htmlOut };
        } catch(e) {
          // Retry once after 2 seconds
          try {
            await new Promise(res=>setTimeout(res,2000));
            const text2 = await claude(apiKey, sec.prompt, sec.tok);
            const htmlOut2 = mdToHTML(text2);
            if(sec.title.includes('Year at a Glance')){
              const lines=text2.trim().split('\n').filter(l=>l.trim());
              const styledLines=lines.map((l,i)=>i===lines.length-1?`<p><em>${l}</em></p>`:`<p>${l}</p>`).join('');
              return {title:sec.title, html:`<div class="year-glance">${styledLines}</div>`};
            }
            return { title: sec.title, html: htmlOut2 };
          } catch(e2) {
            return { title: sec.title, html: `<p style="color:#999;font-style:italic">Section temporarily unavailable — please regenerate.</p>` };
          }
        }
      };

      const mainSections = await Promise.all(
        SECTION_PROMPTS.filter(sec=>!sec.isSubPeriod).map(processSection)
      );
