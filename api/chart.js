    ...
    if(d?.[0]){
      ...
      try{
        ...
        if(d2.results?.[0]?.timezone){
          const tz=dstAwareTz(d2.results[0].timezone, birthDate) ?? Math.round(lon/15);
          return{lat,lon,tz};
        }
      }catch(e2){}
      return{lat,lon,tz:Math.round(lon/15)};
    }
  }catch(e){}
...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  // ── MODE: ASK ASTROLOGER ─────────────────────────────────────────────────
  if(body._ask){
    const{question,history,firstName,topic,natalCtx,dashaCtx,transitCtx,lat,lon,tz}=body;
      ...
      if(lat && lon){
        try{
          ...
          prashnaData = {
            ...
            keyRules: prashna.rules.slice(0,2).map(r=>({favorable:r.favorable,rule:r.rule.slice(0,100),source:r.source})),
          };
        }catch(pe){ /* prashna optional */ }
      }

      // Topic-specific guidance
      const topicNote = {
        career:   'This is a career/work question. Be practical and specific — what can they actually DO.',
        love:     'This is a love/relationship question. Be emotionally warm and honest.',
        money:    'This is a money question. Be direct about timing and risks, no vague optimism.',
        health:   'This is a health question. Be gentle and practical. Always suggest consulting a doctor for medical decisions.',
        travel:   'This is a travel/relocation question. Focus on timing and direction.',
        children: 'This is a question about children/fertility. Be compassionate and specific.',
        general:  '',
      }[topic||'general'] || '';

      // Build conversation history for context
      const historyText = (history||[]).slice(-4).map(m =>
        (m.role==='user'?firstName:' Astrologer') + ': ' + m.content.slice(0,200)
      ).join('\n');

      const systemPrompt = `You are ${firstName}'s personal Vedic astrologer. You know their chart completely — you have studied it in depth. You are brilliant, warm, direct, and occasionally irreverent. You speak like a trusted friend who happens to have deep astrological knowledge.

CRITICAL STYLE RULES:
- NEVER start your response with a markdown header (# or ##). Start with their name or a direct statement.
- NEVER use bullet points or numbered lists. Flowing prose only.
- Use *italics* for emphasis at most 2-3 times. Never **bold**.
- Never say "according to your chart" or "your chart shows" — just say what you see.
- Never use Sanskrit terms without instant plain-English explanation.
- Don't summarize their chart back to them — get straight to the insight.
- Use their name (${firstName}) naturally once or twice, not at the start of every sentence.
- End with one question or observation that opens the conversation, not a formal close.
- 4-6 sentences total. Flowing, personal, specific.

WHAT YOU KNOW ABOUT ${firstName}:
${natalCtx||''}

THEIR CURRENT PLANETARY ERA:
${dashaCtx||''}

WHAT THE SKY LOOKS LIKE TODAY:
${transitCtx||''}
${prashnaCtx ? '\nPRASHNA KUNDALI (cast the moment they asked this):\n' + prashnaCtx : ''}
${topicNote ? '\nTONE NOTE: ' + topicNote : ''}`;
      ...
      const answer = await claude(apiKey, userMessage, 520, systemPrompt);
      return res.status(200).json({ answer, prashna: prashnaData });
