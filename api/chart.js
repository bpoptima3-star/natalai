...
function scoreSignature(signature, ctx) {
  let total = 0, maxPossible = 0;
  const firedRules = [];
  for (const factor of signature.factors) {
    // BUILD-2b: genderFilter respect — skip rules that don't apply to this gender.
    if (factor.genderFilter && factor.genderFilter !== 'both' && factor.genderFilter !== ctx.gender) continue;
    const score = factor.check(ctx);
    if (score > 0) {
      // Negative weights subtract from total naturally (score * negativeWeight = negative).
      total += score * factor.weight;
      firedRules.push({
        id: factor.id,
        score,
        weight: factor.weight,
        contribution: +(score * factor.weight).toFixed(3),
        citation: factor.citation,
      });
    }
    // BUILD-2b: maxPossible accumulates |weight| so normalization handles bidirectional scoring.
    // normalizedScore lands in [-1, 1] meaningfully. windowThreshold compares net positive.
    maxPossible += Math.abs(factor.weight);
  }
  return {
    rawScore: total,
    maxPossible,
    normalizedScore: maxPossible > 0 ? total / maxPossible : 0,
    firedRules,
  };
}
...
function scanLifetimeForEvent(signature, natalChartFull, natalMoonLon, dob, gender) {
  const [birthY, birthM, birthD] = dob.split('-').map(Number);
  const boundaries = getAllDashaBoundaries(natalMoonLon, birthY, birthM, birthD);
  console.error('[SCANNER] enter, domain=' + signature.name + ', boundaries=' + boundaries.length);
  ...
  for (const boundary of boundaries) {
    ...
    // Filter: skip if outside 120-year window (null maha).
    if (!dashaAtTarget.maha) continue;
    const age = boundary.ageAtBoundary;
    // BUILD-2b: age bucket resolved via _bucketForAge helper (handles fractional ages via Math.floor).
    const ageBucket = signature.ageWindows ? _bucketForAge(age, signature.ageWindows) : 'unknown';
    const ctx = {
      natalChart: natalChartFull,
      enrichedChart: natalChartFull, // BUILD-2b: sibling alias — rules read enrichedChart by convention
      targetDate: boundary,
      dashaAtTarget: {
        maha:       dashaAtTarget.maha.name      || dashaAtTarget.maha.lord,
      ...
      transitsAtTarget: transits,
      ageBucket,
      ageAtBoundary: age,
      gender,
    };
    const scoreResult = scoreSignature(signature, ctx);
    scoredPoints.push({
      boundary,
      score: scoreResult,
      transits,
      dasha: ctx.dashaAtTarget,
      age,
    });
  }
...
// ═════════════════════════════════════════════════════════════════════════════
// SPRINT 1 PART 2b — MARRIAGE SIGNATURE
// 23 classical rules with bidirectional scoring (positives open, negatives subtract).
// Gender-variant via Saravali Stree Jataka. All rules read enrichment fields from Part 2a.
// Scanner walks lifetime dasha boundaries scoring against this signature; aggregator clusters
// high-scoring boundaries and applies age-bucket multiplier biased toward realistic marriage ages.
// ═════════════════════════════════════════════════════════════════════════════

// Dignity → score on classical scale
function _dignityScore(dignity) {
  switch (dignity) {
    ...
    case 'enemy':        return 0.2;
    case 'debilitated':  return 0;
    default:             return 0.4;
  }
}

// Dasha-level match → contribution scale (maha full / antar half / pratyantar quarter)
function _dashaMatch(dashaAtTarget, planetName) {
  if (!dashaAtTarget) return 0;
  if (dashaAtTarget.maha === planetName) return 1.0;
  if (dashaAtTarget.antar === planetName) return 0.5;
  if (dashaAtTarget.pratyantar === planetName) return 0.25;
  return 0;
}

// Standard rashi lords for Upapada sign-lord lookup (Rule 15)
const _SIGN_LORDS_FOR_SIG = ['Mars','Venus','Mercury','Moon','Sun','Mercury','Venus','Mars','Jupiter','Saturn','Saturn','Jupiter'];

EVENT_SIGNATURES.marriage = {
  name: 'marriage',
  description: 'When does/did marriage occur, what type, what quality',
  genderVariant: true,
  ageWindows: {
    young:     [20, 25],
    classical: [26, 32],
  ...
  // Age-bucket multipliers — bias SAME astrological scores toward humanly-realistic ages.
  // Real astrologers apply this implicitly; we make it explicit per domain.
  ageBucketMultipliers: {
    young:     1.3,
    classical: 1.5,  // modal marriage window
    late:      0.9,
    beyond:    0.5,
    very_late: 0.2,  // sharp decay; base rate near zero past 55
  },
  windowThreshold: 0.25,  // lowered from 0.35 framework default — afflicted charts still surface ONE weak window via fallback
  factors: [
    // ── POSITIVE INDICATORS (openings) ──

    // RULE 1: 7th house has benefic
    { id: '7th_house_has_benefic', weight: 4, citation: 'BPHS Ch 80 v.6', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec?.planets) return 0;
        const benefics = ['Jupiter','Venus','Mercury','Moon'];
        for (const b of benefics) { const p = ec.planets[b]; if (p && p.house === 7) return 1.0; }
        return 0;
      } },

    // RULE 2: 7th lord dignity scale
    { id: '7th_lord_strong', weight: 5, citation: 'BPHS Ch 80 v.8', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec) return 0;
        const seventhLord = ec.houseLords?.[7]; if (!seventhLord) return 0;
        return _dignityScore(ec.dignityFlags?.[seventhLord] || 'neutral');
      } },

    // RULE 3: 7th lord favorable house position
    { id: '7th_lord_favorable_house', weight: 3, citation: 'Phaladeepika Ch 9 v.4', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec?.planets) return 0;
        const seventhLord = ec.houseLords?.[7]; if (!seventhLord) return 0;
        const h = ec.planets[seventhLord]?.house; if (!h) return 0;
        if ([1,4,5,7,9,10].includes(h)) return 1.0;
        if ([2,11].includes(h)) return 0.5;
        return 0;
      } },

    // RULE 4: Venus strong (males +4, females +2). TODO Sprint 1.5: Venus retrograde reduction (-0.3) deferred — needs retrograde flag in enrichment.
    { id: 'venus_strong_male', weight: 4, citation: 'BPHS Ch 80 v.10', genderFilter: 'M',
      check: (ctx) => _dignityScore(ctx.enrichedChart?.dignityFlags?.Venus || 'neutral') },
    { id: 'venus_strong_female', weight: 2, citation: 'BPHS Ch 80 v.10 (Saravali Stree Jataka adjusted weight)', genderFilter: 'F',
      check: (ctx) => _dignityScore(ctx.enrichedChart?.dignityFlags?.Venus || 'neutral') },

    // RULE 5: Jupiter strong (males +2, females +5). +0.2 bonus if Jupiter in 1/5/7/9/10/11.
    { id: 'jupiter_strong_male', weight: 2, citation: 'BPHS Ch 80 v.12', genderFilter: 'M',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        let score = _dignityScore(ec?.dignityFlags?.Jupiter || 'neutral');
        const jh = ec?.planets?.Jupiter?.house;
        if (jh && [1,5,7,9,10,11].includes(jh)) score = Math.min(1.0, score + 0.2);
        return score;
      } },
    { id: 'jupiter_strong_female', weight: 5, citation: 'BPHS Ch 80 v.12 (Saravali — Jupiter dominant for female)', genderFilter: 'F',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        let score = _dignityScore(ec?.dignityFlags?.Jupiter || 'neutral');
        const jh = ec?.planets?.Jupiter?.house;
    ...
    // RULE 6: Venus dasha active
    { id: 'venus_dasha_active', weight: 5, citation: 'BPHS Ch 47 + Ch 80', genderFilter: 'both',
      check: (ctx) => _dashaMatch(ctx.dashaAtTarget, 'Venus') },

    // RULE 7: Jupiter dasha active (males +3, females +6)
    { id: 'jupiter_dasha_male', weight: 3, citation: 'BPHS Ch 47', genderFilter: 'M',
      check: (ctx) => _dashaMatch(ctx.dashaAtTarget, 'Jupiter') },
    { id: 'jupiter_dasha_female', weight: 6, citation: 'BPHS Ch 47 (Saravali Stree Jataka)', genderFilter: 'F',
      check: (ctx) => _dashaMatch(ctx.dashaAtTarget, 'Jupiter') },

    // RULE 8: 7th lord dasha active
    { id: '7th_lord_dasha_active', weight: 6, citation: 'BPHS Ch 47 — dasha of karaka activates matter', genderFilter: 'both',
      check: (ctx) => {
        const seventhLord = ctx.enrichedChart?.houseLords?.[7]; if (!seventhLord) return 0;
        return _dashaMatch(ctx.dashaAtTarget, seventhLord);
      } },

    // RULE 9: Navamsa 7th lord strong
    { id: 'navamsa_7th_lord_strong', weight: 4, citation: 'Jaimini Sutras + BPHS Navamsa', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const seventhLord = ec?.houseLords?.[7]; if (!seventhLord) return 0;
        const navH = ec?.navamsha?.[seventhLord]?.house; if (!navH) return 0;
        if ([1,4,5,7,9,10].includes(navH)) return 1.0;
        if ([6,8,12].includes(navH)) return 0;
        return 0.5;
      } },

    // RULE 10: Darakaraka favorable Navamsa
    { id: 'darakaraka_favorable_navamsa', weight: 4, citation: 'Jaimini Sutras Ch 1', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const dk = ec?.darakaraka; if (!dk) return 0;
        const navH = ec?.navamsha?.[dk]?.house; if (!navH) return 0;
        if ([1,4,5,7,9,10].includes(navH)) return 1.0;
        if ([6,8,12].includes(navH)) return 0;
        return 0.5;
      } },

    // RULE 11: Multiple benefics aspect 7th
    { id: 'multiple_benefics_aspect_7th', weight: 5, citation: 'Phaladeepika Ch 9 v.7', genderFilter: 'both',
      check: (ctx) => ctx.enrichedChart?.cancellations?.multipleBeneficsAspect7th ? 1.0 : 0 },

    // RULE 12: Jupiter aspects 7th
    { id: 'jupiter_aspects_7th', weight: 6, citation: 'Phaladeepika Ch 9 — strongest marriage opening', genderFilter: 'both',
      check: (ctx) => ctx.enrichedChart?.cancellations?.jupiterAspects7th ? 1.0 : 0 },

    // RULE 13: Venus exalted or own
    { id: 'venus_exalted_or_own', weight: 5, citation: 'Phaladeepika Ch 9', genderFilter: 'both',
      check: (ctx) => ctx.enrichedChart?.cancellations?.venusExaltedOrOwn ? 1.0 : 0 },

    // RULE 14: 7th lord exalted or own
    { id: '7th_lord_exalted_or_own', weight: 5, citation: 'Phaladeepika Ch 9', genderFilter: 'both',
      check: (ctx) => ctx.enrichedChart?.cancellations?.seventhLordExaltedOrOwn ? 1.0 : 0 },

    // RULE 15: Upapada lord favorable
    { id: 'upapada_lord_favorable', weight: 4, citation: 'Jaimini Sutras — Upapada Lagna for marriage precision', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const ul = ec?.upapadaLagna; if (!ul) return 0;
        const lagnaSignIdx = ec.lagnaIdx;
        const ulSign = (lagnaSignIdx + ul - 1) % 12;
        const ulLord = _SIGN_LORDS_FOR_SIG[ulSign]; if (!ulLord) return 0;
        const dig = ec.dignityFlags?.[ulLord] || 'neutral';
        if (['exalted','moolatrikona','own','friend'].includes(dig)) return 1.0;
        if (dig === 'neutral') return 0.5;
        return 0;
      } },

    // RULE 16: Classical age + dasha karaka active
    { id: 'classical_age_with_dasha_trigger', weight: 3, citation: 'BPHS Ch 80 — dasha of karaka in classical age', genderFilter: 'both',
      check: (ctx) => {
        if (ctx.ageBucket !== 'classical') return 0;
        const d = ctx.dashaAtTarget;
        const seventhLord = ctx.enrichedChart?.houseLords?.[7];
        const hits = ['Venus','Jupiter',seventhLord].some(name =>
          name && (d.maha === name || d.antar === name || d.pratyantar === name)
        );
        return hits ? 1.0 : 0;
      } },

    // ── NEGATIVE INDICATORS (frictions) ──

    // RULE 17: Saturn in 7th uncancelled. Saturn exalted (Libra) in 7th → 0 friction (late-but-stable per Flag 13).
    { id: 'saturn_in_7th_uncancelled', weight: -5, citation: 'BPHS Ch 80 v.14 + Phaladeepika Ch 9 cancellation rules', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const sat = ec?.planets?.Saturn; if (!sat) return 0;
        if (sat.house !== 7) return 0;
        if (ec?.dignityFlags?.Saturn === 'exalted') return 0; // exalted Saturn in 7th: late but stable, no friction
        const c = ec?.cancellations || {};
        const anyCancel = c.jupiterAspects7th || c.venusExaltedOrOwn || c.seventhLordExaltedOrOwn || c.multipleBeneficsAspect7th;
        return anyCancel ? 0.5 : 1.0;
      } },

    // RULE 18: Mangal Dosha severe uncancelled
    { id: 'mangal_dosha_severe', weight: -4, citation: 'BPHS + regional; Saravali cancellation logic', genderFilter: 'both',
      check: (ctx) => {
        const m = ctx.enrichedChart?.mangalDosha; if (!m) return 0;
        if (m.severity === 'severe') return 1.0;
        if (m.severity === 'mild') return 0.5;
        return 0;
      } },

    // RULE 19: 7th has malefic with no benefic. Waning Moon computed inline (Moon-Sun longitude diff ≥ 180°).
    { id: '7th_has_malefic_no_benefic', weight: -3, citation: 'BPHS Ch 80 v.16', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const planets = ec?.planets; if (!planets) return 0;
        const moonLon_ = planets.Moon?.longitude;
        const sunLon_  = planets.Sun?.longitude;
        const isWaningMoon = (moonLon_ != null && sunLon_ != null) && (((moonLon_ - sunLon_ + 360) % 360) >= 180);
        const maleficSet = new Set(['Sun','Mars','Saturn','Rahu','Ketu']);
        if (isWaningMoon) maleficSet.add('Moon');
        const beneficSet = new Set(['Jupiter','Venus','Mercury']);
        if (!isWaningMoon) beneficSet.add('Moon');
        const maleficsIn7 = Object.entries(planets).filter(([p, pd]) => pd.house === 7 && maleficSet.has(p));
        if (maleficsIn7.length === 0) return 0;
        const beneficIn7 = Object.entries(planets).some(([p, pd]) => pd.house === 7 && beneficSet.has(p));
        if (beneficIn7) return 0.3;
        if (ec?.cancellations?.jupiterAspects7th) return 0.3;
        return 1.0;
      } },

    // RULE 20: 7th lord in dusthana
    { id: '7th_lord_in_dusthana', weight: -4, citation: 'Phaladeepika Ch 9 v.11', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const seventhLord = ec?.houseLords?.[7]; if (!seventhLord) return 0;
        const h = ec?.planets?.[seventhLord]?.house; if (!h) return 0;
        return [6,8,12].includes(h) ? 1.0 : 0;
      } },

    // RULE 21: Venus debilitated (Neecha Bhanga detection deferred to Sprint 2)
    { id: 'venus_debilitated', weight: -3, citation: 'BPHS Ch 80 v.18', genderFilter: 'both',
      check: (ctx) => {
        const dig = ctx.enrichedChart?.dignityFlags?.Venus;
        if (dig === 'debilitated') return 1.0;
        if (dig === 'enemy') return 0.5;
        return 0;
      } },

    // RULE 22: Jupiter debilitated (female only)
    { id: 'jupiter_debilitated_female', weight: -3, citation: 'Saravali Stree Jataka', genderFilter: 'F',
      check: (ctx) => ctx.enrichedChart?.dignityFlags?.Jupiter === 'debilitated' ? 1.0 : 0 },

    // RULE 23: 8th lord afflicted (female longevity)
    { id: '8th_lord_afflicted_female', weight: -3, citation: 'Saravali Stree Jataka — 8th = marriage longevity', genderFilter: 'F',
      check: (ctx) => {
        const e8 = ctx.enrichedChart?.eighthHouse; if (!e8) return 0;
        let score = 0;
        const dig = e8.eighthLordDignity;
        if (dig === 'debilitated') score = 1.0;
        else if (dig === 'enemy') score = 0.5;
        if (e8.maleficsIn8th > 0 && e8.beneficsIn8th === 0) score += 0.3;
        return Math.min(1.0, score);
      } },
  ],
};

// Spouse profile helper — read by buildScannerContextForClaude when domain is marriage.
// Translates Darakaraka planet → spouse personality archetype, D9 element → background hint,
// Upapada lagna sign → additional spouse indicator. All from enrichment fields, no recomputation.
const _SIGN_NAMES_FOR_PROFILE = ['Aries','Taurus','Gemini','Cancer','Leo','Virgo','Libra','Scorpio','Sagittarius','Capricorn','Aquarius','Pisces'];

function getSpouseProfile(enrichedChart) {
  if (!enrichedChart) return null;
  const dk = enrichedChart.darakaraka;
  const dkNavamsa = enrichedChart.navamsha?.[dk];
  const ulHouse = enrichedChart.upapadaLagna;
  const lagnaSign = enrichedChart.lagnaIdx;
  const ulSign = (ulHouse != null) ? (lagnaSign + ulHouse - 1) % 12 : null;
  const dkPersonalityMap = {
    Sun:     'authoritative, confident, leader-like',
    Moon:    'emotional, caring, family-oriented',
    Mars:    'energetic, sometimes aggressive, action-driven',
    Mercury: 'analytical, communicative, intellectually-engaged',
    Jupiter: 'wise, religious or philosophical, generous',
    Venus:   'beautiful, refined, pleasure-loving, artistic',
    Saturn:  'serious, hardworking, steady, possibly older or more mature',
    Rahu:    'unconventional, foreign-connected, intense',
    Ketu:    'spiritual, detached, mysterious',
  };
  const personalityHint = dkPersonalityMap[dk] || 'unique character';
  const backgroundHintsByElement = ['fiery/independent','earthy/practical','airy/intellectual','watery/emotional'];
  const dkNavamsaSign = dkNavamsa?.sign ?? 0;
  const elementIdx = dkNavamsaSign % 4;
  return {
    darakaraka: dk,
    personalityHint,
    backgroundHint: backgroundHintsByElement[elementIdx],
    darakakaSignD9: dkNavamsaSign,
    upapadaSignIdx: ulSign,
    upapadaSignName: ulSign != null ? _SIGN_NAMES_FOR_PROFILE[ulSign] : null,
    notes: [],
  };
}

// ═════════════════════════════════════════════════════════════════════════════
// SPRINT 1 PART 3a — CAREER SIGNATURE
// 20 classical rules using 7-state dignity (consistent with marriage signature).
// Industry-neutral. Reads enrichment fields from Part 2a. Same architecture: bidirectional
// scoring, age-bucket multipliers, weak-signal fallback. genderVariant: false (no gender splits).
// Shadbala NOT read at rule layer — rishi voice integrates Shadbala via existing shadbalaCtx flow.
// ═════════════════════════════════════════════════════════════════════════════

// Career-specific archetypes — planet → kind of work that planet rules
const _CAREER_ARCHETYPES = {
  Sun:     'authority, leadership, government, executive roles',
  Moon:    'public-facing work, hospitality, nurturing professions, food, women-focused industries',
  Mars:    'engineering, military, sports, surgery, real estate, action-oriented work',
  Mercury: 'communication, writing, analysis, trade, technology, teaching',
  Jupiter: 'teaching, law, advisory roles, finance, religion, philosophy',
  Venus:   'arts, beauty, luxury, entertainment, hospitality, design, relationships work',
  Saturn:  'long-term professions, structured work, mining, manufacturing, justice, elderly care',
  Rahu:    'foreign business, technology, unconventional paths, media, innovation',
  Ketu:    'spiritual work, research, isolation-based work, mysticism, healing',
};

// Industry → which planets support it. Used by industryCompatibility check.
const _CAREER_INDUSTRY_MAP = {
  'hospitality':      ['Venus','Moon'],
  'tech':             ['Mercury','Rahu'],
  'software':         ['Mercury','Rahu'],
  'real estate':      ['Mars','Saturn','Venus'],
  'teaching':         ['Jupiter','Mercury'],
  'medicine':         ['Sun','Mars','Jupiter'],
  ...
  'law':              ['Jupiter','Saturn'],
  'sports':           ['Mars','Sun'],
  'food':             ['Moon','Venus','Jupiter'],
  'fashion':          ['Venus','Moon'],
  'spiritual':        ['Ketu','Jupiter'],
  'foreign business': ['Rahu','Mercury'],
  'foreign':          ['Rahu','Mercury'],
};

EVENT_SIGNATURES.career = {
  name: 'career',
  description: 'When does career take off, what direction, what timing',
  genderVariant: false,
  ageWindows: {
    young: [18, 25],
    early: [26, 32],
  ...
  // Career age multipliers — modal career peak (33-50) gets strongest bias.
  // Career signatures define their own multipliers per domain (marriage uses different shape).
  ageBucketMultipliers: {
    young: 1.2,
    early: 1.3,
    peak:  1.4,
  ...
  factors: [
    // ── POSITIVE INDICATORS (career openings) ──

    // RULE 1: 10th house has benefic or career karaka (Sun for authority)
    { id: '10th_house_has_benefic_or_karaka', weight: 4, citation: 'BPHS Ch 34 v.3', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec?.planets) return 0;
        const careerKarakas = ['Jupiter','Venus','Mercury','Sun'];
        for (const p of careerKarakas) {
          const pl = ec.planets[p]; if (pl && pl.house === 10) return 1.0;
        }
        return 0;
      } },

    // RULE 2: 10th lord dignity scale
    { id: '10th_lord_strong', weight: 5, citation: 'BPHS Ch 34 v.5', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec) return 0;
        const tenthLord = ec.houseLords?.[10]; if (!tenthLord) return 0;
        return _dignityScore(ec.dignityFlags?.[tenthLord] || 'neutral');
      } },

    // RULE 3: 10th lord favorable house position
    { id: '10th_lord_favorable_house', weight: 4, citation: 'Phaladeepika Ch 10 v.4', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart; if (!ec?.planets) return 0;
        const tenthLord = ec.houseLords?.[10]; if (!tenthLord) return 0;
        const h = ec.planets[tenthLord]?.house; if (!h) return 0;
        if ([1,10].includes(h)) return 1.0;
        if ([4,7].includes(h)) return 0.6;
        if ([5,9].includes(h)) return 0.9;
        if (h === 11) return 0.85;
        if (h === 2) return 0.7;
        if ([3,6].includes(h)) return 0.4;
        if ([8,12].includes(h)) return 0;
        return 0;
      } },

    // RULE 4: Amatyakaraka dignity strong
    { id: 'amatyakaraka_strong', weight: 5, citation: 'Jaimini Sutras — Amatyakaraka is career significator', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const amk = ec?.amatyakaraka; if (!amk) return 0;
        return _dignityScore(ec.dignityFlags?.[amk] || 'neutral');
      } },

    // RULE 5: Amatyakaraka favorable house
    { id: 'amatyakaraka_favorable_house', weight: 3, citation: 'Jaimini Sutras', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const amk = ec?.amatyakaraka; if (!amk) return 0;
        const h = ec.planets?.[amk]?.house; if (!h) return 0;
        if ([1,4,5,7,9,10].includes(h)) return 1.0;
        if (h === 11) return 0.85;
        if ([6,8,12].includes(h)) return 0;
        return 0.5;
      } },

    // RULE 6: Sun dignity (authority)
    { id: 'sun_strong_for_authority', weight: 3, citation: 'BPHS Ch 34 — Sun rules authority', genderFilter: 'both',
      check: (ctx) => _dignityScore(ctx.enrichedChart?.dignityFlags?.Sun || 'neutral') },

    // RULE 7: Saturn dignity (work capacity)
    { id: 'saturn_favorable_for_work', weight: 3, citation: 'BPHS Ch 34 — Saturn rules sustained work', genderFilter: 'both',
      check: (ctx) => _dignityScore(ctx.enrichedChart?.dignityFlags?.Saturn || 'neutral') },

    // RULE 8: 10th lord dasha active
    { id: '10th_lord_dasha_active', weight: 6, citation: 'BPHS Ch 47 + Ch 34', genderFilter: 'both',
      check: (ctx) => {
        const tenthLord = ctx.enrichedChart?.houseLords?.[10]; if (!tenthLord) return 0;
        return _dashaMatch(ctx.dashaAtTarget, tenthLord);
      } },

    // RULE 9: Amatyakaraka dasha active
    { id: 'amatyakaraka_dasha_active', weight: 5, citation: 'Jaimini + BPHS Ch 47', genderFilter: 'both',
      check: (ctx) => {
        const amk = ctx.enrichedChart?.amatyakaraka; if (!amk) return 0;
        return _dashaMatch(ctx.dashaAtTarget, amk);
      } },

    // RULE 10: Sun dasha active during career ages
    { id: 'sun_dasha_active_career_ages', weight: 3, citation: 'BPHS Ch 47', genderFilter: 'both',
      check: (ctx) => {
        if (!['young','early','peak'].includes(ctx.ageBucket)) return 0;
        return _dashaMatch(ctx.dashaAtTarget, 'Sun');
      } },

    // RULE 11: Saturn dasha active during career ages
    { id: 'saturn_dasha_active_career_ages', weight: 4, citation: 'BPHS Ch 47 — Saturn activates sustained work', genderFilter: 'both',
      check: (ctx) => {
        if (!['young','early','peak'].includes(ctx.ageBucket)) return 0;
        return _dashaMatch(ctx.dashaAtTarget, 'Saturn');
      } },

    // RULE 12: D10 10th lord dignity (using natal dignity as proxy).
    // Half-weight when d10.d10LagnaApproximate is true (backwards-compat path Part 2a).
    { id: 'd10_10th_lord_strong', weight: 4, citation: 'BPHS Dashamsha + Phaladeepika', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const d10 = ec?.d10; if (!d10 || d10.lagna == null) return 0;
        const tenthSignOfD10 = (d10.lagna + 9) % 12;
        const tenthLordOfD10 = _SIGN_LORDS_FOR_SIG[tenthSignOfD10];
        if (!tenthLordOfD10) return 0;
        let score = _dignityScore(ec.dignityFlags?.[tenthLordOfD10] || 'neutral');
        if (d10.d10LagnaApproximate) score *= 0.5;
        return score;
      } },

    // RULE 13: D10 lagna lord dignity (half-weight if d10LagnaApproximate)
    { id: 'd10_lagna_lord_strong', weight: 3, citation: 'Dashamsha chapter', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const d10 = ec?.d10; if (!d10 || d10.lagna == null) return 0;
        const d10LagnaLord = _SIGN_LORDS_FOR_SIG[d10.lagna];
        if (!d10LagnaLord) return 0;
        let score = _dignityScore(ec.dignityFlags?.[d10LagnaLord] || 'neutral');
        if (d10.d10LagnaApproximate) score *= 0.5;
        return score;
      } },

    // RULE 14: 10th-from-Karakamsha occupied by benefic/Sun/Saturn (career significators)
    { id: '10th_from_karakamsha_occupied', weight: 3, citation: 'Jaimini — Karakamsha career path', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        if (ec?.karakamshaLagna == null) return 0;
        const tenthFromKamsa = (ec.karakamshaLagna + 9) % 12;
        const careerSignificators = new Set(['Jupiter','Venus','Mercury','Moon','Sun','Saturn']);
        for (const [planet, pd] of Object.entries(ec.planets || {})) {
          if (pd.sign === tenthFromKamsa && careerSignificators.has(planet)) return 1.0;
        }
        return 0;
      } },

    // RULE 15: Kendra-Trikona Raja Yoga active in dasha.
    // Per Flag 2 resolution: yoga must be ACTIVE in current dasha to score; yoga existing but
    // not in dasha returns 0 (no background noise floor across all 81 boundaries).
    { id: 'raja_yoga_active_in_dasha', weight: 4, citation: 'BPHS Raja Yogas Chapter', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const hl = ec?.houseLords; if (!hl) return 0;
        const kendraLords = [hl[1], hl[4], hl[7], hl[10]].filter(Boolean);
        const trikonaLords = [hl[1], hl[5], hl[9]].filter(Boolean);
        // Find any (kendra lord, trikona lord) pair of DIFFERENT planets that share a house
        const yogaPairs = [];
        for (const kL of kendraLords) {
          for (const tL of trikonaLords) {
            if (kL === tL) continue;
            const kH = ec.planets?.[kL]?.house;
            const tH = ec.planets?.[tL]?.house;
            if (kH && tH && kH === tH) yogaPairs.push([kL, tL]);
          }
        }
        if (yogaPairs.length === 0) return 0;
        // Yoga exists. Score only if any participating planet is active in dasha at this target.
        const d = ctx.dashaAtTarget;
        for (const [a, b] of yogaPairs) {
          const aActive = d.maha === a || d.antar === a || d.pratyantar === a;
          const bActive = d.maha === b || d.antar === b || d.pratyantar === b;
          if (aActive || bActive) return 1.0;
        }
        return 0;
      } },

    // RULE 16: 11th lord dignity (gains, professional income)
    { id: '11th_lord_strong', weight: 3, citation: 'BPHS Ch 34', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const eleventhLord = ec?.houseLords?.[11]; if (!eleventhLord) return 0;
        return _dignityScore(ec.dignityFlags?.[eleventhLord] || 'neutral');
      } },

    // RULE 17: 2nd lord favorable (income/accumulated wealth)
    { id: '2nd_lord_favorable', weight: 2, citation: 'Phaladeepika — 2nd is income/wealth', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const secondLord = ec?.houseLords?.[2]; if (!secondLord) return 0;
        return _dignityScore(ec.dignityFlags?.[secondLord] || 'neutral');
      } },

    // ── NEGATIVE INDICATORS (career frictions) ──

    // RULE 18: 10th lord in dusthana (6/8/12 with graduated severity)
    { id: '10th_lord_in_dusthana', weight: -4, citation: 'Phaladeepika Ch 10', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const tenthLord = ec?.houseLords?.[10]; if (!tenthLord) return 0;
        const h = ec?.planets?.[tenthLord]?.house;
        if (h === 6) return 0.7;   // 6th is upachaya — friction but some career value
        if (h === 8) return 1.0;   // 8th = severe
        if (h === 12) return 0.8;  // 12th = strong friction
        return 0;
      } },

    // RULE 19: Saturn debilitated (sustained-effort capacity broken)
    { id: 'saturn_debilitated_career', weight: -3, citation: 'BPHS — debilitated Saturn', genderFilter: 'both',
      check: (ctx) => {
        const dig = ctx.enrichedChart?.dignityFlags?.Saturn;
        if (dig === 'debilitated') return 1.0;
        if (dig === 'enemy') return 0.4;
        return 0;
      } },

    // RULE 20: 10th heavily afflicted by malefics with no benefic. Waning Moon computed inline
    // (Moon-Sun longitude diff ≥ 180°) — same approach as marriage Rule 19.
    { id: '10th_heavily_afflicted', weight: -3, citation: 'BPHS Ch 34 v.18', genderFilter: 'both',
      check: (ctx) => {
        const ec = ctx.enrichedChart;
        const planets = ec?.planets; if (!planets) return 0;
        const moonLon_ = planets.Moon?.longitude;
        ...
        const beneficSet = new Set(['Jupiter','Venus','Mercury']);
        if (!isWaningMoon) beneficSet.add('Moon');
        const maleficsIn10 = Object.entries(planets).filter(([p, pd]) => pd.house === 10 && maleficSet.has(p));
        const beneficsIn10 = Object.entries(planets).filter(([p, pd]) => pd.house === 10 && beneficSet.has(p));
        if (maleficsIn10.length >= 2 && beneficsIn10.length === 0) {
          // Check for benefic aspect on 10th from aspect matrix
          const aspectMatrix = ec.aspectMatrix || {};
          for (const b of beneficSet) {
            if (aspectMatrix[b]?.aspectedHouses?.includes(10)) return 0.4;
          }
          return 1.0;
        }
        if (maleficsIn10.length >= 1 && beneficsIn10.length === 0) {
          return 0.3;
        }
        return 0;
      } },
    ...
    try{
      ...
      try {
        ...
        if (path === 'C' && natalMoonLon && dob && clientEngineInput) {
          ...
          const signature = EVENT_SIGNATURES[domain];
          if (signature) {
            const scoredPoints = scanLifetimeForEvent(signature, enrichedChart, natalMoonLon, dob, enrichedChart.gender || 'M');
            const windows = aggregateScoredPointsIntoWindows(scoredPoints, signature);
            if (windows.length > 0) {
              scannerCtx = buildScannerContextForClaude(windows, signature, question, enrichedChart);
            } else {
            ...
            console.error('[SCANNER] no signature registered for domain=' + domain + ' (path=C question went unscored)');
          }
        }
      } catch (se) {
...
GROUNDING:
1. The verdicts above ("the deeper pattern the texts taught you to see") are the source of truth for every claim about marriage, career, wealth, health, family, children, foreign travel, property, education, mind, yogas, dasha, soul purpose, spouse, and remedies. Every astrological claim in your response must trace to a specific verdict line. If a topic isn't in the verdicts, you don't speculate about it. The chart positions and transits are SUPPORTING context for explaining the verdicts in plain language — they are NEVER an independent source for new astrological claims.

2. If the verdicts don't address what the user asked, say so plainly: "the chart shows X strongly but doesn't speak directly to your specific question — here's the closest read." Do not invent astrology to fill gaps.
...
6. Acknowledge the specific factual error in one sentence. Recompute the affected reading. Continue. Do not apologize repeatedly. Do not discuss your architecture. Do not ask the user to help fix the system. Do not explain how you read the chart. Move forward.
...
9. American English. No Sanskrit terms in user-facing prose (translations: rising sign, birth star, period, sub-phase, pattern, soul planet, spouse planet, public image point).
