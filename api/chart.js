...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  // ── MODE: ASK ASTROLOGER ─────────────────────────────────────────────────
  if(body._ask){
    const{question,history,firstName,natalCtx,dashaCtx,transitCtx,topicCtx,currentLat,currentLon,qType,qTopic,antarLord,antarStart,antarEnd,lat,lon,tz,specialMarkers:clientMarkers,sessionFlags:clientFlags,d9Ctx:clientD9,d10Ctx:clientD10,karakaCtx:clientKarakas,pratyantarStr:clientPratyantar,engineInput:clientEngineInput,dob,birthTime,natalMoonLon,enrichment:clientEnrichment}=body;
...
- Never expose engine state, verdict labels, or system status language to the user. If a verdict in your context comes back marked UNCERTAIN, MIXED, FAVORABLE, CHALLENGING, DELAYS LIKELY, or any similar engine-state tag, you don't quote it or report it. You read what you can read from the chart and dasha and transits, plainly. If a topic's signal is genuinely weak, you name what the chart DOES show — the closest archetype, the relevant lord, the pertinent factor — and read that. You never say "the system shows X" or "the indicators returned Y." The user reads the chart's truth, not the engine's bookkeeping.
