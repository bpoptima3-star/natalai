...
function wrapHTML(sections,name,type,cd){
  ...
  <div id="${sid}" style="border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:0${isKey?';border-left:3px solid rgba(191,154,48,.4)':''}">
    <div style="padding:32px 0 10px${isKey?';padding-left:3px':''}"><h2 class="rep-sh" style="${isKey?'font-size:26px;':''}${''}">${s.title}</h2></div>
    <div style="padding:18px 0 32px;font-family:Cormorant Garamond,serif;font-size:17px;line-height:1.8;color:#2a2218 !important;background:#ffffff !important">${s.html}</div>
  </div>`;}).join('');
...
.rep-sh::after{content:'';flex:1;height:1px;background:linear-gradient(to right,rgba(191,154,48,.5),transparent);margin-left:20px}
...
Write EXACTLY 5 short punchy lines. No paragraphs, no headers, no bullet points. Just 5 lines of truth:\nLine 1: What kind of year this is — annual Rising Sign ${RS[srLagnaSign1]} + dominant planet energy in one sentence\nLine 2: The single biggest opportunity this year — specific, cite a period and dates\nLine 3: The one thing to navigate carefully — specific period and dates\nLine 4: The energy shift${birthdayInWindow?` on birthday ${fmtDt(nextSRCal)} — what changes`:' — when the biggest period transition happens'}\nLine 5: One-line verdict — what this year is fundamentally FOR\nBe concrete. Be direct. Sound like someone who has studied this chart and is speaking only to ${name}.`},
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        const cardBorderColor = ['Jupiter','Venus','Moon','Mercury'].includes(p.planet)
          ? '#2d7a4f' : ['Saturn','Mars','Rahu','Ketu'].includes(p.planet)
          ? '#a03030' : (p.color||'#bf9a30');
        const cardBg = ['Jupiter','Venus','Moon','Mercury'].includes(p.planet)
          ? 'rgba(45,122,79,.03)' : ['Saturn','Mars','Rahu','Ketu'].includes(p.planet)
          ? 'rgba(160,48,48,.03)' : '#ffffff';
      ...
      const processSection = async (sec) => {
        ...
        if(sec.isMonthGuide){
          ...
          for(let i=0;i<12;i++){
            ...
            const mBorderColor=['Jupiter','Venus','Moon','Mercury'].includes(p.planet)?'#2d7a4f':['Saturn','Mars','Rahu','Ketu'].includes(p.planet)?'#a03030':(p.color||'#bf9a30');
            const mBgColor=['Jupiter','Venus','Moon','Mercury'].includes(p.planet)?'rgba(45,122,79,.025)':['Saturn','Mars','Rahu','Ketu'].includes(p.planet)?'rgba(160,48,48,.025)':'#ffffff';
...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
        ...
        const tocHTML = `<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:14px;padding-top:12px;border-top:1px solid rgba(0,0,0,.06)">
          ${tocSections.map((id,i)=>`<a href="#${id}" style="font-size:11px;color:#bf9a30;text-decoration:none;padding:3px 8px;border:1px solid rgba(191,154,48,.3);border-radius:20px;font-family:Outfit,sans-serif;letter-spacing:.03em">${tocLabels[i]}</a>`).join('')}
        </div>`;
        const summaryHTML = `<div style="background:#fafaf8;border:1px solid rgba(191,154,48,.2);border-radius:12px;padding:24px 28px;margin-bottom:4px">
          ...
          <div style="display:flex;flex-direction:column;gap:11px">
            ${lines.slice(1).map((line,i)=>`<div style="display:flex;align-items:flex-start;gap:10px;font-size:14px;color:#3d3d3d;line-height:1.6"><span style="width:6px;height:6px;border-radius:50%;background:${dots[i]||'#bf9a30'};flex-shrink:0;margin-top:7px"></span><span>${boldFirstWords(line)}</span></div>`).join('')}
          </div>
          ${tocHTML}
        </div>`;
