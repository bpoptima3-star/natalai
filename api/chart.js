...
Jupiter: house ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        const lines = cleanInterp.split('\n').filter(l=>l.trim()).map(l=>
          l.startsWith('~transit~')
            ? `<p style="font-family:Cormorant Garamond,serif;font-size:15px;line-height:1.7;color:#7c5a2a !important;font-style:italic;padding-top:4px;border-top:1px solid rgba(191,154,48,.15);margin-top:8px;margin-bottom:0">${l.replace('~transit~','').trim()}</p>`
            : `<p style="font-family:Cormorant Garamond,serif;font-size:16px;line-height:1.72;color:#2a2218 !important;margin:0 0 6px">${l.trim()}</p>`
        ).join('');
        return `<div style="border-radius:10px;overflow:hidden;border:1px solid rgba(0,0,0,.08);background:#ffffff !important;box-shadow:0 1px 4px rgba(0,0,0,.04);border-left:3px solid ${p.color||'#bf9a30'};margin-bottom:14px">
  <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px 12px;border-bottom:1px solid rgba(0,0,0,.06)">
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <div>
        <div style="font-family:Cormorant Garamond,serif;font-size:19px;font-weight:500;color:#1a1a1a !important">${p.planet}</div>
        <div style="font-family:Inter,sans-serif;font-size:11px;color:#888888 !important;font-weight:400">${p.start} → ${p.end} &nbsp;·&nbsp; ${p.days} days</div>
      </div>
    </div>
    ${badge}
  </div>
