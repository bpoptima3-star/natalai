...
function computeSahams(srSid,srLagna,isDayBirth){
  ...
  const raw={
    Punya:isDayBirth?n360(L+(S.Moon||0)-(S.Sun||0)):n360(L+(S.Sun||0)-(S.Moon||0)),
    Karma:isDayBirth?n360(L+(S.Saturn||0)-(S.Sun||0)):n360(L+(S.Sun||0)-(S.Saturn||0)),
    Vivah:isDayBirth?n360(L+(S.Venus||0)-(S.Saturn||0)):n360(L+(S.Saturn||0)-(S.Venus||0)),
    Mrityu:isDayBirth?n360(L+n360(srLagna+210)-(S.Moon||0)):n360(L+(S.Moon||0)-n360(srLagna+210))
  };
  const result={};
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      const itthasala1 = tajikAspects1.filter(a=>a.type==='itthasala'&&a.nature==='benefic').map(a=>`${a.planet1}↔${a.planet2} ${a.aspect}°`).join(', ');
      const ishrafa1 = tajikAspects1.filter(a=>a.type==='ishrafa'&&a.nature==='malefic').map(a=>`${a.planet1}↔${a.planet2} ${a.aspect}°`).join(', ');
      // ── SAHAMS (Arabic Parts) ────────────────────────────────────────────
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      const NEWFEATURES = [
        ...
        shadbalaCtx || '',
        tajikAspects1.length ? `Annual Tajik aspects — Itthasala (favorable): ${itthasala1||'none'} | Ishrafa (separating): ${ishrafa1||'none'}` : '',
        sahamsStr ? `Annual Sahams: ${sahamsStr}` : '',
      ].filter(Boolean).join('\n');
...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  // ── MODE: ASK ASTROLOGER ─────────────────────────────────────────────────
  if(body._ask){
    const{question,history,firstName,natalCtx,dashaCtx,transitCtx,topicCtx,lat,lon,tz}=body;
    ...
    try{
      ...
      const systemBlock = `You are Shri Jyotish, a warm and wise Vedic astrologer speaking directly to ${name}. Answer their question about this chart:
...
- Plain conversational language — no jargon without instant explanation
...
- If FOCUS AREA is provided, answer from those specific houses and planets — never generic
- Use the planetary strength context (if provided) to calibrate confidence: 'strong positive' = results flow; 'strongly challenging' = this takes more effort than it should; 'contested' = mixed experience
- Never mention technical strength scores — only the human lived experience`;
