...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  if(body._prashna){
    const{question,lat,lon,tz,natalContext,natalFull,transitContext,dashaContext,firstName}=body;
    ...
    try{
      ...
      const prompt = `You are a Vedic astrologer having a real conversation with ${name}. Be warm, direct, personal — like a brilliant friend who happens to know astrology deeply. No stiff formal language. No disclaimers. No Sanskrit jargon. Speak in American English.

═══ NATAL CHART (D1) — ${name}'s birth blueprint ═══
${natalFull||natalContext||''}

═══ CURRENT PLANETARY PERIODS ═══
${dashaContext||''}

═══ TODAY'S TRANSITS (what the sky looks like right now) ═══
${transitContext||''}

═══ PRASHNA — chart cast for THIS exact question at ${prashna.castTime} ═══
${prashna.context}
...
═══ HOW TO ANSWER ═══
1. Open with a direct, personal answer — yes/no/be careful/looks good — based on ALL three layers: natal + dasha + prashna verdict (${prashna.verdict.label})
2. Name the 1-2 most relevant planets from the natal chart and what they're doing RIGHT NOW (in transit or dasha)
3. Reference the prashna verdict and 1 key rule as supporting evidence
4. Give them something specific and useful — a timing, a caution, or an action
5. End warmly, like a friend who cares about them

Keep it to 4-5 sentences max. No bullet points. No section headers. Just flowing, personal, insightful conversation. Talk TO ${name}, not about them.`;

      const answer = await claude(apiKey, prompt, 450);
      return res.status(200).json({
        answer,
        prashna:{
