...
function getTodayData(lagnaSign,moonSign){
  ...
  else if(satFromMoon===3)sadeSati={phase:'Dhaiya (4th house)',note:"Saturn in 4th from Moon — domestic pressures, career friction."};
  else if(satFromMoon===7)sadeSati={phase:'Ashtama Shani (8th house)',note:"Saturn in 8th from Moon — most challenging Dhaiya period."};
...
const LANG=`RULES: American English only. No Sanskrit terms. Use: "Rising Sign" not Lagna, "Moon Sign" not Rashi, "Birth Star" not Nakshatra, "Major Life Period" not Mahadasha, "Active Phase" not Antardasha, "Planetary Pattern" not Yoga, "Saturn Pressure Cycle" not Sade Sati. All predictions use "indicators suggest" or "strong potential for" — never absolute statements. Reference specific planets and periods. Warm, direct tone like a knowledgeable friend.`;
...
Format: Use ## for section headers. Write in flowing paragraphs with bullet points where appropriate. 2000-2500 words total. Personal, direct, honest.`;
        ...
        const compatHTML=compatText.replace(/^## (.+)$/gm,'<h2 class="rep-sh">$1</h2>').replace(/^### (.+)$/gm,'<h3 style="font-family:Georgia,serif;font-size:17px;color:#bf9a30;margin:16px 0 8px">$1</h3>').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>').replace(/\*(.+?)\*/g,'<em>$1</em>').replace(/^[•\-] (.+)$/gm,'<li>$1</li>').replace(/(<li>.*\n?)+/g,'<ul>$&</ul>').replace(/\n\n/g,'</p><p>').replace(/^(?!<)/gm,'<p>').replace(/<p><\/p>/g,'');
        const html=wrapHTML([{title:'Your Compatibility Reading',html:compatHTML}],`${name} + ${name2||'Partner'}`,reportType,{lagna,rashi,nakshatra,dob});
        logToSheets({event:'report_generated',name,reportType,dob,lagna,rashi,period:mahaName});
        return res.status(200).json({html,chartData:{name,dob,lagna,rashi,nakshatra}});
      }
