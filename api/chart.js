...
H7 lord: ${_eHL.find(hl=>hl.house===7)?.desc||''} | H7 aspects: ${_eM7A.join(', ')||'none'}`;
        ...
        if(reportType==='cosmic'){
          sectionDefs=[
            // ── COMPAT: 3 Haiku calls, max 370 tok each (was 7 calls, tok:1200 max) ──
            // Engine verdicts from twoCore handle all facts. Claude writes voice.

            {title:'💑 Who You Both Are', tok:350, prompt:`${LANG}
COMPATIBILITY: ${name} + ${name2||'Partner'} | Score: ${totalScore}/36
${name}: ${lagna} Rising · ${rashi} Moon | ${name2||'Partner'}: ${RS[ls2]} Rising · ${RS[ms2]} Moon
${y1} × ${y2} animal energy | Gana: ${g1}+${g2} (${ganaScore}/6) | Nadi: ${nadiDosha?'SAME — health flag':'compatible'}
${twoCore}

Write 3 lines:
— ${name}'s nature, love style, and what they bring to partnership (${lagna} Rising + ${rashi} Moon + ${y1} energy — be specific)
— ${name2||'Partner'}'s nature, love style, and what they bring (${RS[ls2]} Rising + ${RS[ms2]} Moon + ${y2} energy — be specific)  
— What happens when these two meet: the attraction dynamic, the friction point, the growth opportunity
Vivid, specific, personal. Talk about real people.`},

            {title:'🔮 Your Connection & Chemistry', tok:370, prompt:`${LANG}
COMPATIBILITY: ${name} + ${name2||'Partner'} | Score: ${totalScore}/36
Physical chemistry: ${yoniScore}/4 ${yoniEnemy?'(OPPOSITE — tension + intense attraction)':''}
Mental bond: ${grahaScore}/5 | Emotional: ${bhakootBad?'Challenging (0/7)':'Harmonious (7/7)'} | Health: ${nadiDosha?'Nadi Dosha — same energy':'compatible'}
${twoCore}

Write 4 focused lines:
— Physical and emotional chemistry: what ${yoniScore}/4 means for this couple specifically — be honest about attraction AND friction
— Mental connection (${grahaScore}/5): how their minds work together — communication, understanding, where they'll struggle
— Emotional compatibility: Moon energy — what ${bhakootBad?'the tension':'the harmony'} means in day-to-day life
— The raw truth: 2 genuine strengths AND 2 genuine challenges for THIS specific pairing
Direct, honest, specific. No generic astrology language.`},

            {title:'⏰ Long-Term & The Verdict', tok:300, prompt:`${LANG}
COMPATIBILITY: ${name} + ${name2||'Partner'} | Score: ${totalScore}/36
${name} current period: ${mahaName}-${antarName} | ${name2||'Partner'} chart: ${RS[ls2]} Rising
${twoCore}

Write 3 lines:
— Best timing window for commitment based on ${name}'s current ${mahaName} period (cite actual dates)
— 3 specific pieces of advice for THIS couple — tailored to their actual scores and patterns
— The Verdict: honest, compassionate, direct. What is the real potential here? End with one empowering truth.
Honest, warm, actionable.`},

          ]; // end cosmic sectionDefs
        } else {
          sectionDefs=[
...
Write 3 lines:
— ${name}'s nature and what they bring to partnership (${lagna} Rising + ${rashi} Moon + ${y1} energy)
— ${name2||'Partner'}'s nature and what they bring (${RS[ls2]} Rising + ${RS[ms2]} Moon + ${y2} energy)
— What happens when these two meet: attraction dynamic, friction point, growth opportunity
Vivid, specific, personal.`},
...
Write 4 focused lines:
— Physical and emotional chemistry: what ${yoniScore}/4 means for this couple — honest about attraction AND friction
— Mental connection (${grahaScore}/5): how their minds work together, where they'll struggle
— Emotional compatibility: what ${bhakootBad?'the Moon tension':'the Moon harmony'} means day-to-day
— Raw truth: 2 genuine strengths AND 2 genuine challenges for THIS pairing
Direct, honest, specific.`},

            {title:'⏰ Long-Term & The Verdict', tok:300, prompt:`${LANG}
COMPATIBILITY: ${name} + ${name2||'Partner'} | Score: ${totalScore}/36
${name} current period: ${mahaName}-${antarName}
${twoCore}

Write 3 lines:
— Best timing window for commitment based on ${name}'s ${mahaName} period (actual dates)
— 3 specific pieces of advice tailored to their actual scores and patterns
— The Verdict: honest, compassionate, direct. Real potential + one empowering truth.
Warm, actionable.`},

          ]; // end compat sectionDefs
        }
...
${NEWFEATURES}`;
      ...
      const SECTION_PROMPTS = reportType === 'timing' ? [
        // ── TIMING REPORT: 3 Haiku calls (was 4-5) ───────────────────────────
        // Templates (no Haiku call) handle blueprint, month guide, career grid

        {title:'📅 Your Timing Blueprint', tok:0, isTemplate:true, buildFn:buildTimingBlueprintHTML},

        {title:'⚡ Power Windows & Key Dates', tok:320, prompt:`${LANG}
...
${buildVerdictCtx('year')}

Write TWO parts:

PART 1 — TOP 3 POWER WINDOWS: For each give: month range → planet energy → 1 sentence what to do.
PART 2 — YOUR 10 KEY DATES: List exactly 10 dates/windows this year. Format: **Month YYYY**: what to do.

Be specific to ${name}'s actual period dates. Keep each line short.`},

        {title:'📆 Month-by-Month Guide', tok:0, isMonthGuide:true},

        {title:'🎯 Timing for ${eventType||"Your Goals"}', tok:260, prompt:`${LANG}
${name} | ${lagna} Rising · ${rashi} Moon | Goal: ${eventType||'major life decisions'}
Periods: ${allPeriods.slice(0,4).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}
${buildVerdictCtx('year')}

Write 4 lines:
— Best 2 specific windows for this goal (period name + dates)
— One thing to watch for or avoid
— 3-year outlook for this area given ${mahaName} period
— One actionable step to take now`},
        ...
        // ── NATAL REPORT: 4 Haiku calls (was 6) ──────────────────────────────
        // Lean prompts — engine verdicts do the heavy lifting
        // Claude articulates, not interprets

        {title:'🌟 You — Chart & Character', tok:350, prompt:`${LANG}
${name} | ${lagna} Rising · ${rashi} Moon · ${nakshatra} Birth Star
Rising: ${RD[0]||''} ${RD[1]||''}
Moon: ${MD[0]||''} ${MD[1]||''}
Birth Star: ${(STAR_DESC[nakshatra]||[])[0]||''}
Soul planet (Atmakaraka): ${atmStr||atmakaraka}
Top pattern: ${topYoga}
${chartRulerStr}
${vargottamaStr||''}
${buildVerdictCtx('soul')}

Write 4 lines for ${name} — talk TO them:
— What ${lagna} Rising + ${rashi} Moon + ${nakshatra} Birth Star creates together as a combination (specific, not generic)
— Their biggest natural gift from this chart — name the actual talent
— Their soul planet (${atmakaraka}) and what it means they're here to master this lifetime
— One thing their chart says they are actively building toward right now
Warm, direct. No intro. Reference actual planet names.`},

        {title:'⚡ Life, Career & Relationships', tok:380, prompt:`${LANG}
${name} | ${lagna} Rising · ${rashi} Moon
Career: H10 lord ${houseLords.find(hl=>hl.house===10)?.lord||''} | H7 lord ${houseLords.find(hl=>hl.house===7)?.lord||''}
D9 Venus: ${venusD9} | D9 Moon: ${moonD9}
Best career periods: ${allPeriods.filter(p=>['Jupiter','Venus','Mercury','Sun'].includes(p.planet)).slice(0,2).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(', ')||'see periods'}
${buildVerdictCtx('career')}
${buildVerdictCtx('love')}

Write 5 focused lines:
— CAREER: What their chart says about their professional direction and natural authority (cite H10 lord position)
— CAREER: Best 2 specific windows this year for work/money moves (period names + dates)
— WEALTH: Their wealth pattern and primary income style
— RELATIONSHIPS: What ${lagna} Rising + D9 Venus in ${venusD9} says about the partner they actually need
— RELATIONSHIPS: Best specific window this year for love or deepening connection (period + dates)
Direct, specific. Use 'indicators suggest' for career/money. Use warm personal tone for relationships.`},

        {title:'⏰ Your Current Chapter', tok:350, prompt:`${LANG}
${name} is in ${mahaName} Major Period (ends ${mahaEnds}, ${mahaYrsLeft} yrs left) | Active Phase: ${antarName} (ends ${antarEnds}, ${antarMosLeft} mo left)
${satNote||''}
${dashaSandhi||''}
${buildNarrativeCtx(planetNarratives,'Saturn')}
${buildTimingCtx(timingWindows3,'general')}
${buildVerdictCtx('dasha')}
${buildVerdictCtx('health')}

Write 4 lines:
— What ${mahaName}-${antarName} period is asking ${name} to do, face, or build RIGHT NOW
— What shifts when this phase ends (${antarEnds}) — what's coming next
— Health and family themes this year — use 'patterns associated with'
— One empowering truth: what this period, despite its challenges, is building in ${name}
${todayData.sadeSati?'Include one honest line about Saturn Pressure Cycle.':''}
Personal, direct, specific dates. No jargon.`},

        {title:'✦ Remedies & Guidance', tok:280, prompt:`${LANG}
${name} | ${lagna} Rising · ${rashi} Moon
${rulesResults?.remedies?.needed?.length > 0 ?
  'Planets needing support: ' + rulesResults.remedies.needed.map(r=>r.planet+'('+r.reason+')').join(', ')
  : 'No major planetary afflictions'}
${rulesResults?.remedies?.verdicts?.filter(v=>v?.startsWith('REMEDY FOR'))?.slice(0,3)?.join('\n')||''}
${rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('ANCESTRAL'))||''}
${rulesResults?.ashtakavarga?.verdicts?.find(v=>v?.includes('WEAK HOUSES'))||''}

Write 4 specific, actionable lines for ${name}:
— The 1-2 planets in this chart that most need strengthening and one concrete action for each (specific mantra, day, or practice)
— One ancestral or karmic pattern this chart shows that needs conscious attention
— The single most important behavioral shift this chart asks of ${name} right now
— One closing empowering truth: what they're building that is real and lasting
Warm, specific, actionable. These are gifts, not warnings.`},
