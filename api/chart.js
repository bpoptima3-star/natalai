...
async function geocode(place, birthDate){
  ...
  try{
    ...
    if(d.results?.[0]){
      ...
      // Indian subcontinent: always 5.5 (India doesn't observe DST)
      const isIndia = loc.latitude>=8 && loc.latitude<=37 && loc.longitude>=68 && loc.longitude<=97;
      if(isIndia) return{lat:loc.latitude,lon:loc.longitude,tz:5.5};
      const tzName2 = loc.timezone||'';
      if(tzName2.includes('Asia/Kolkata')||tzName2.includes('Asia/Calcutta')) return{lat:loc.latitude,lon:loc.longitude,tz:5.5};
      if(tzName2.includes('Asia/Kathmandu')||tzName2.includes('Asia/Katmandu')) return{lat:loc.latitude,lon:loc.longitude,tz:5.75};
      if(tzName2.includes('Asia/Kabul')) return{lat:loc.latitude,lon:loc.longitude,tz:4.5};
      if(tzName2.includes('Asia/Colombo')) return{lat:loc.latitude,lon:loc.longitude,tz:5.5};
      if(tzName2.includes('Australia/Adelaide')||tzName2.includes('Australia/Darwin')) return{lat:loc.latitude,lon:loc.longitude,tz:9.5};
      // Use IANA timezone name for DST-accurate historical offset
      const tz = dstAwareTz(loc.timezone, birthDate)
                 ?? (loc.utc_offset_seconds != null ? loc.utc_offset_seconds/3600 : Math.round(loc.longitude/15));
  ...
  try{
    ...
    if(d?.[0]){
      ...
      // Try to resolve IANA tz from Open-Meteo using the resolved coordinates
      try{
        ...
        if(d2.results?.[0]?.timezone){
          const tz=dstAwareTz(d2.results[0].timezone, birthDate) ?? Math.round(lon/15*2)/2;
          return{lat,lon,tz};
        }
      }catch(e2){}
      return{lat,lon,tz:Math.round(lon/15*2)/2}; // nearest 0.5hr increment
    }
  }catch(e){}
  ...
  for(const[country,capital]of Object.entries(COUNTRY_CAPITALS)){
    if(lower.includes(country)){
      ...
      if(tzName2.includes('Asia/Colombo')) return{lat:loc.latitude,lon:loc.longitude,tz:5.5};
      if(tzName2.includes('Australia/Adelaide')||tzName2.includes('Australia/Darwin')) return{lat:loc.latitude,lon:loc.longitude,tz:9.5};
          const tz=dstAwareTz(loc.timezone, birthDate) ?? (loc.utc_offset_seconds!=null ? loc.utc_offset_seconds/3600 : Math.round(loc.longitude/15));
          return{lat:loc.latitude,lon:loc.longitude,tz};
        }
      }catch(e){}
    }
...
function wrapHTML(sections,name,type,cd){
  ...
  const body=sections.filter(s=>s&&s.title!==undefined).map(s=>{
    ...
    return `
  <div id="${sid}" style="border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:0${isKey?';border-left:3px solid rgba(191,154,48,.4)':''}">
    <div style="padding:36px 0 14px${isKey?';padding-left:14px':''}"><h2 class="rep-sh" style="${isKey?'font-size:26px;':''}${''}">${s.title}</h2></div>
    <div style="padding:20px 48px 40px;font-family:Cormorant Garamond,serif;font-size:17px;line-height:1.85;color:#2a2218 !important;background:#ffffff !important;max-width:760px">${s.html}</div>
  </div>`;}).join('');
...
.rep-sh{font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:400;color:#1a1a1a;padding-bottom:14px;border-bottom:none;margin-bottom:0;display:flex;align-items:center;gap:10px}
...
.p-card-body p{font-family:'Cormorant Garamond',serif;font-size:16px;line-height:1.72;color:#2a2218;margin:0 0 6px}
...
<div style="padding:40px 48px;background:#fafaf8 !important">${body}</div>
...
function computeTimingWindows(shadbala,mahaName,antarName,allPeriods,todayData){
  ...
  const activeAP=allPeriods?.find(p=>p.current);
  const windows={};
  for(const[area,relPlanets]of Object.entries(TIMING_AREA_PLANETS)){
    ...
    const nextFav=allPeriods?.find(p=>BEN.includes(p.planet)&&relPlanets.includes(p.planet)&&!p.current);
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      const _saturnsDoomFull  = buildSaturnsDoom(todayData.sadeSati, false);
      const _saturnsDoomCmpct = buildSaturnsDoom(todayData.sadeSati, true);
      const _critDegreeFull   = buildCriticalDegreeBirth(natal, false);
      const _critDegreeCmpct  = buildCriticalDegreeBirth(natal, true);
      const _marsDisruptFull  = buildMarsDisruption(natal, shadbala, false);
      const _marsDisruptCmpct = buildMarsDisruption(natal, shadbala, true);
      // Package for use in ask handler and reports
      ...
      // ── SHADBALA INSIGHTS — practical life statements per planet ─────────
      const sbInsights = buildShadbalaInsights(
        natal.sid, lagnaSign, shadbala, natal.planets,
        mahaName, antarName, todayData
      );
      const chartSummary = sbInsights.summary || '';
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      // Formula: navamsaSign = (signIndex * 9 + Math.floor(degInSign / (30/9))) % 12
      // Adjusts for sign type: movable=Aries start, fixed=Leo start, dual=Sag start
      const NAVAMSA_START = [0, 9, 6, 3, 0, 9, 6, 3, 0, 9, 6, 3]; // Aries/Leo/Sag start by sign
      const calcNavamsa = (lon) => {
        const signIdx = so(lon);
        const degInSign = n360(lon) % 30;
        const pada = Math.floor(degInSign / (30/9));
        return (NAVAMSA_START[signIdx] + pada) % 12;
      };
      // Compute navamsa for all planets
      const navamsa = {};
      for(const [planet, lon] of Object.entries(natal.sid)) {
        navamsa[planet] = calcNavamsa(lon);
      }
      const navamsaLagna = calcNavamsa(natal.lagna);

      // Vargottama: planet in same sign in D1 and D9 — very strong
      const vargottama = Object.entries(natal.sid)
        .filter(([p, lon]) => so(lon) === navamsa[p])
        .map(([p]) => p);
      ...
      const moonD9  = RS[navamsa['Moon']];
      const jupD9   = RS[navamsa['Jupiter']];
      ...
      const RD = RISING_DESC[lagna] || [];
      const MD = MOON_DESC[rashi] || [];

      // ── ENHANCED CHART CONTEXT — new features injected into all prompts ──
      const NEWFEATURES = [
        navamsaStr,
        vargottamaStr,
        atmStr ? `Soul planet: ${atmStr}` : '',
        combustStr,
        gandantaStr,
        dashaSandhi,
        pratyantarStr ? `Active short window: ${pratyantarStr}` : '',
        savStr,
        bhavaStr ? `House position shifts: ${bhavaStr}` : '',
        retroStr,
        yogaKarakaStr,
        ykNote,
        parivartanaStr,
        ...
        aspect10note,
        aspect7note,
        kpStr ? 'KP Sub-Lords: '+kpStr : '',
        dispositorStr ? 'Dispositor chains: '+dispositorStr : '',
        shadbalaCtx || '',
        tajikAspects1.length ? `Annual applying aspects (favorable): ${itthasala1||'none'} | Separating aspects: ${ishrafa1||'none'}` : '',
        karakaCareer ? `Soul and career planets: ${karakaCareer.replace(/\n/g, ' | ')}` : '',
        chartSummary ? `Chart overview: ${chartSummary}` : '',
        // Top 3 planet insights for relevant topic
        buildInsightsCtx(sbInsights.insights, ['Saturn','Jupiter','Mercury']),
        karakaLove ? `Partnership planets: ${karakaLove.replace(/\n/g, ' | ')}` : '',
        d9Ctx ? `D9 Soul chart: ${d9Ctx}` : '',
        sahamsStr ? `Annual sensitive points: ${sahamsStr}` : '',
        // Special chart markers — full version for reports
        _saturnsDoomFull   ? `SATURN DOOM ACTIVE: ${_saturnsDoomFull}` : '',
        _critDegreeFull    ? `CRITICAL DEGREE BIRTH: ${_critDegreeFull}` : '',
        _marsDisruptFull   ? `MARS DISRUPTION: ${_marsDisruptFull}` : '',
      ].filter(Boolean).join('\n');
      const SD = STAR_DESC[nakshatra] || [];
...
${NEWFEATURES}`;
      ...
      const SECTION_PROMPTS = reportType === 'timing' ? [
        ...
        {title:'📅 Your Timing Blueprint',tok:280,prompt:`${LANG}\nPre-analyzed chart:\n${chartBase}\n${buildNarrativeCtx(planetNarratives,'Saturn')}\n${buildNarrativeCtx(planetNarratives,'Jupiter')}\n${buildTimingCtx(timingWindows3,'career')}\n${buildTimingCtx(timingWindows3,'money')}\nAnnual periods: ${allPeriods.map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}\n\nWrite 3 sharp lines:\n— What ${name}'s ${lagna} Rising + ${rashi} Moon says about how they naturally time decisions\n— What their ${mahaName} Major Life Period (ending ${mahaEnds}) means for major moves right now\n— The single most important thing to know about reading their annual periods for best results\nPersonal, specific, no generic advice.`},
        {title:'⚡ Your Power Windows This Year',tok:300,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon\
${shadbalaCtx ? 'Key strengths: '+shadbalaCtx.split('\n').slice(1,4).join(' | ') : ''}\n${buildNarrativeCtx(planetNarratives,'Saturn')}\n${buildNarrativeCtx(planetNarratives,'Jupiter')}\n${buildTimingCtx(timingWindows3,'career')}\n${buildTimingCtx(timingWindows3,'money')}\n${buildTimingCtx(timingWindows3,'love')}\nAnnual periods: ${allPeriods.map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}\nKey pattern: ${topYoga}\nJupiter: ${todayData.jupTransit.quality} (house ${todayData.jupTransit.house} from Moon)\n\nIdentify TOP 3 POWER WINDOWS. For each: dates → why it works (which planet, what it supports) → what to do.\nThen name 2 periods requiring more care and why.\nBe specific to ${name}'s actual period dates. Reference planet names directly.`},
        {title:'📆 Month-by-Month Guide',tok:0,isMonthGuide:true},
        {title:`🎯 Timing for ${eventType||'Your Goals'}`,tok:280,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon | Event: ${eventType||'major life decisions'}\nPeriods: ${allPeriods.map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}\nKey pattern: ${topYoga}\n\nWrite 3-4 lines:\n— Best 2-3 specific months/windows for this event (cite period names and exact dates)\n— What to watch for in those windows specifically for ${name}\n— What could complicate timing and when\n— 3-year outlook: what this event type looks like given ${name}'s current ${mahaName} period\nSpecific, practical, personal.`},
        {title:'💼 Career & Financial Timing',tok:280,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon | ${mahaName} Major Life Period (ends ${mahaEnds})\nCareer periods: ${allPeriods.filter(p=>['Jupiter','Venus','Mercury','Sun','Mars'].includes(p.planet)).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(', ')}\nConservative: ${allPeriods.filter(p=>['Saturn','Rahu','Ketu'].includes(p.planet)).map(p=>p.planet+' '+p.start+'→'+p.end).join(', ')||'none'}\nPattern: ${topYoga}\n\nWrite 3 sharp lines:\n— Best specific months for career moves/launches/negotiations (cite periods and dates)\n— Best specific months for financial decisions and investments\n— What ${name}'s ${mahaName} Major Life Period means for their career trajectory through ${mahaEnds}\nReference actual period names and dates.`},
        {title:'🎯 Your 10 Key Dates',tok:700,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon\nAnnual periods: ${allPeriods.map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}\n\nGive ${name} exactly 10 specific dates or windows over the next 12 months. Format: **[Month Year]**: [What to do — be specific to their actual period planet and house energy]. Make each one actionable. Tie each to a real period. End with one empowering closing sentence.`},
      ] : [
        // Natal report sections — 8 sections
        {title:'🌟 Your Cosmic Blueprint',tok:320,prompt:`${LANG}\nPre-analyzed chart:\n${chartBase}\nAnnual periods today→${yearEndFmt}: ${allPeriods.slice(0,3).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' (NOW)':'')).join(', ')}\n${atmStr?'Soul planet: '+atmStr:''}\n${chartRulerStr}\n\nAdd EXACTLY 3-4 personal lines for ${name}:\n— How their ${lagna} Rising + ${rashi} Moon + ${nakshatra} Birth Star create a specific unusual combination\n— Their soul planet (${atmakaraka}) reveals what this person is here to master — name it directly\n${finalDispositors.length?'— Their chart ruler ('+chartRuler[0]+') governs most planetary energy — one specific consequence of this':'— What their strongest pattern says about their life direction'}\n— One insight about what this chart says they are building toward right now\nDirect, warm, no intro. Reference actual planet names.`},
        {title:'⚡ Your Personality & Natural Strengths',tok:340,prompt:`${LANG}\nPre-analyzed chart:\n${chartBase}\n${shadbalaCtx ? shadbalaCtx.split('\n').slice(0,4).join('\n') : ''}\nPlanets: ${pSummary}\nPatterns: ${yogas.map(y=>y.name+': '+y.meaning).join(' | ')||'standard chart'}\n${buildNarrativeCtx(planetNarratives,'Sun')}\n${buildNarrativeCtx(planetNarratives,'Moon')}\n${buildNarrativeCtx(planetNarratives,'Mercury')}\n\nWrite 3-4 warm but sharp lines about ${name} — talk TO them, not about them:\n— Their single biggest natural gift: what ${lagna} Rising + ${rashi} Moon creates together that most people don't have. Be specific.\n— How they actually process hard moments — not clinical, but human. What their ${moonSubLord?moonSubLord+' Moon sub-lord':'Moon in '+nakshatra} means for how they feel things and recover.\n— Their instinctive money/work style — one honest sentence about how they earn and what they need to feel secure professionally.\n— (If 4th line) One thing their chart says they're still learning to trust about themselves.\nWarm, direct, personal. No astrology jargon. Talk like a trusted friend who happens to read charts.`},
        {title:'⏰ Your Current Life Chapter',tok:420,prompt:`${LANG}\n${name} is in their ${mahaName} Major Life Period (ends ${mahaEnds}, ${mahaYrsLeft} yrs left). Active Phase: ${antarName} (ends ${antarEnds}, ${antarMosLeft} months left).\nRising: ${lagna} — ${RD[0]||''}\nMoon: ${rashi} — ${MD[0]||''}${satNote?' | '+satNote:''}\n${shadbalaCtx ? shadbalaCtx.split('\n').slice(0,3).join('\n') : ''}\n${buildNarrativeCtx(planetNarratives,'Moon')}\n${buildNarrativeCtx(planetNarratives,'Saturn')}\n${buildTimingCtx(timingWindows3,'general')}\n\nWrite 3-4 personal lines:\n— What this ${mahaName} period is asking ${name} to develop or confront — focus is ${ptCurrent?.focus||mahaName+' themes'}. Energy is ${ptCurrent?.energy||'active'}.\n— What the ${antarName} phase (ending ${antarEnds}) means RIGHT NOW in practical terms\n— H1 lord (self): ${houseLords.find(hl=>hl.house===1)?.desc||''} — How this period interacts with their ${lagna} Rising and ${rashi} Moon — what it amplifies or challenges\n${dashaSandhi?'— DASHA TRANSITION: '+dashaSandhi+' — acknowledge this turning point':''}\n${combustStr?'— Combust planets: '+combustStr+' — weakened, interpret carefully':''}\n${vargottamaStr?'— '+vargottamaStr+' — exceptional natural strength':''}\n${todayData.sadeSati?'— One honest line about the Saturn Pressure Cycle and how to work with it':'— What shifts when this chapter ends (${mahaEnds}) and what to prepare'}\n${lagnaChainStr?'— Lagna lord chain: '+lagnaChainStr+' — this reveals why some areas keep recurring':''}\n${lagnaSubLord?'— Life direction sub-lord is '+lagnaSubLord+' — this planet is the final controller of life direction':''}\nDirect, specific, personal. Reference actual dates.`},
        {title:'✦ Your Year at a Glance',tok:300,prompt:`${LANG}\nData:\n${core}\nAnnual chart: ${munthaStr} | Year lords: ${panchaStr}\n${buildTimingCtx(timingWindows3,'career')}\n${buildTimingCtx(timingWindows3,'love')}
Write EXACTLY 5 short punchy lines. No paragraphs, no headers, no bullet points. Just 5 lines of truth:\nLine 1: What kind of year this is — annual Rising Sign ${RS[srLagnaSign1]} + dominant planet energy in one sentence\nLine 2: The single biggest opportunity this year — specific, cite a period and dates\nLine 3: The one thing to navigate carefully — specific period and dates\nLine 4: The energy shift${birthdayInWindow?` on birthday ${fmtDt(nextSRCal)} — what changes`:' — when the biggest period transition happens'}\nLine 5: One-line verdict — what this year is fundamentally FOR\nBe concrete. Be direct. Sound like someone who has studied this chart and is speaking only to ${name}.`},
        {title:'🗓 Your Solar Year — Sub-Period Guide',tok:0,isSubPeriod:true},
        {title:'💼 Career, Money & Opportunities',tok:340,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon\
${shadbalaCtx ? shadbalaCtx.split('\n').slice(0,4).join('\n') : ''}\nPlanets: ${pSummary}\nKey pattern: ${topYoga}\n${buildNarrativeCtx(planetNarratives, houseLords.find(hl=>hl.house===10)?.lord||'Saturn')}\n${buildTimingCtx(timingWindows3,'career')}\nBest career periods this year: ${allPeriods.filter(p=>['Jupiter','Venus','Mercury','Sun'].includes(p.planet)).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(', ')||'see periods'}\nCaution periods: ${allPeriods.filter(p=>['Saturn','Rahu','Ketu'].includes(p.planet)).map(p=>p.planet+' '+p.start+'→'+p.end).join(', ')||'none noted'}\nH10 lord: ${houseLords.find(hl=>hl.house===10)?.desc||''}\nH10 aspects: ${career10aspects.join(', ')||'none'}\nH2 lord: ${houseLords.find(hl=>hl.house===2)?.desc||''} | H11 lord: ${houseLords.find(hl=>hl.house===11)?.desc||''}\n${yogaKarakaStr}\n${parivartanaStr?parivartanaStr:''}\n${tenthLordSubLord?'Career lord ('+tenthLord+') sub-lord is '+tenthLordSubLord+' — this controls career outcome':''}\n${chartRulerStr}\n\nWrite 3-4 sharp lines:\n— Career house lord (${tenthLord}) sits in H${houseLords.find(hl=>hl.house===10)?.sitsInHouse} — what this means for direction and authority (be specific)\n— H10 has ${career10aspects.filter(p=>['Jupiter','Venus','Moon','Mercury'].includes(p)).length} benefic and ${career10aspects.filter(p=>['Saturn','Mars','Sun','Rahu','Ketu'].includes(p)).length} malefic aspects — factor this in\n— ${name}'s long-term career direction from their chart patterns (cite planets/houses)\n— Best 2 specific windows this year for career moves (cite period names and dates)\n— Financial pattern from the chart — wealth style and what to watch\n— One professional caution or timing insight from the data\nUse 'indicators suggest' language. Reference actual periods and dates.`},
        {title:'❤️ Love, Relationships & Connection',tok:400,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon\
${shadbalaCtx ? shadbalaCtx.split('\n').slice(0,4).join('\n') : ''}\n${MD[0]||''} ${MD[1]||''}\n${buildNarrativeCtx(planetNarratives,'Venus')}\n${buildTimingCtx(timingWindows3,'love')}\nRelationship patterns: ${patterns.filter(p=>p.type.includes('Mars')||p.type.includes('emotion')).map(p=>p.type+': '+p.meaning).join(' | ')||'none detected'}\nBest relationship windows: ${allPeriods.filter(p=>['Venus','Jupiter','Moon'].includes(p.planet)).map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(', ')||'check periods'}\nH7 lord: ${houseLords.find(hl=>hl.house===7)?.desc||''} | H7 aspects: ${marriage7aspects.join(', ')||'none'} (${aspect7note})\nNavamsa (D9) marriage chart: Venus in ${venusD9} | Moon in ${moonD9} | D9 Rising: ${navamsaLagnaSign}\n${parivartanaStr?parivartanaStr:''}\n${venusSubLord?'Venus sub-lord is '+venusSubLord+' — this is the final controller of relationship fulfillment':''}\n${moonChainStr}\n\nWrite 3-4 personal lines:\n— ${name}'s relationship style from ${lagna} Rising + ${rashi} Moon + D9 Venus in ${venusD9} (what partner quality the soul needs)\n— Venus sub-lord ${venusSubLord} controlling fulfillment — what this means for WHEN and HOW love manifests\n— ${name}'s relationship style from their ${lagna} Rising + ${rashi} Moon — what they actually need in a partner\n— Their attachment pattern — how they bond, withdraw, and what triggers them (be honest)\n— Best specific window this year for love or deepening a connection (cite period and dates)\n— One honest observation about a relationship pattern to watch or transform\nDirect, specific, personal. Legal-safe language.`},
        {title:'🏃 Health, Family & Growth Edges',tok:320,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon · ${nakshatra} Birth Star\n${RD[2]||''} ${MD[2]||''}\n${shadbalaCtx ? shadbalaCtx.split('\n').slice(0,3).join('\n') : ''}\n${buildNarrativeCtx(planetNarratives,'Sun')}\n${buildNarrativeCtx(planetNarratives,'Mars')}\n${buildTimingCtx(timingWindows3,'health')}\nChallenges: ${topPattern||'none flagged'}\nSaturn: ${satNote||'not active'}\n${combustStr?'Combust (quiet): '+combustStr:''}\n${kpSubLords['Moon']?'Moon in '+kpSubLords['Moon'].nakshatra+' nakshatra, sub-lord '+kpSubLords['Moon'].subLord:''}\n\nWrite 3 personal lines:\n— Health patterns: body areas and wellbeing indicated — use 'patterns associated with'. Note Moon sub-lord ${moonSubLord} influence on stress/recovery.\n— Family and home themes this year + what ${mahaName} period means domestically\n— The biggest growth edge this chart asks ${name} to develop — end with something genuinely empowering\nHonest, warm, specific.`},
      ];
...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  // ── MODE: ASK ASTROLOGER ─────────────────────────────────────────────────
  if(body._ask){
    const{question,history,firstName,natalCtx,dashaCtx,transitCtx,topicCtx,currentLat,currentLon,qType,qTopic,antarLord,antarStart,antarEnd,lat,lon,tz,specialMarkers:clientMarkers,sessionFlags:clientFlags,d9Ctx:clientD9,d10Ctx:clientD10}=body;
    ...
    try{
      ...
      const satWarn = natalSatHouse
        ? `Natal Saturn is permanently in H${natalSatHouse[1]}. Transit Saturn is elsewhere today. Never confuse them.`
        : '';

      const systemBlock = `You are a master Vedic astrologer with decades of practice. You have been handed a fully computed chart for ${name}. Your job is to give a reading that feels like it was written specifically for this person — because it was.
...
${satWarn}
${markersCtx}
${qTopic === 'love' && clientD9 ? 'SOUL CHART (D9): ' + clientD9 : ''}
${qTopic === 'career' && clientD10 ? 'CAREER CHART (D10): ' + clientD10 : ''}

HOW A MASTER ASTROLOGER REASONS — FOLLOW THIS EXACTLY:

Step 1 — IDENTIFY the relevant house(s) for this question.
  Career/money → H10, H2, H11. Love/marriage → H7, H5, H1. Health → H1, H6, H8.
  Father → H9 + Sun. Mother → H4 + Moon. Siblings → H3 + Mars.
  Children → H5 + Jupiter. Spouse → H7 + Venus/Jupiter. In-laws → H6.
  NEVER say "I don't have their chart." Every question about family/others is answered from THIS chart.

Step 2 — READ the relevant house lord's condition.
  Where is the lord placed? Is it in a friend's sign, enemy's sign, exalted, debilitated?
  What is its Shadbala strength? Strong = results flow naturally. Weak = capable but friction.
  Is it in a good house (1,2,4,5,7,9,10,11) or difficult house (6,8,12)?

Step 3 — LAYER the Dasha.
  What major and minor period is running? Is the Dasha lord connected to the relevant house?
  Strong Dasha lord + strong natal planet = period of real delivery.
  Strong Dasha lord + weak natal planet = effort brings results but slower.
  Weak Dasha lord = even good charts struggle to deliver in this period.

Step 4 — CHECK current transits.
  Is a benefic or malefic currently transiting the relevant house?
  Jupiter transiting H1/4/7/10 from lagna or Moon = expansion in that area.
  Saturn transiting = discipline required, results delayed but real.
  Note: transit Saturn ≠ natal Saturn. Never confuse the two.

Step 5 — USE Prashna if present.
  Prashna is cast for THIS moment and THIS question. Its verdict is specific to the next 30-45 days.
  Favorable Prashna + strong natal indicators = high confidence answer.
  Unfavorable Prashna + weak natal indicators = counsel caution clearly.
  Never ignore the Prashna verdict when it is present.

Step 6 — SYNTHESISE into one truth.
  Do not list the ingredients. Arrive at the conclusion.
  Bad: "Your 10th lord is in H9 and Jupiter aspects it and the Dasha is favorable."
  Good: "The chart is strongly positioned for career recognition this year — the timing and the natal setup are both pointing the same direction. This is a window to press hard."
  The person wants to know what to think and feel, not what the chart contains.

Step 7 — CALIBRATE your confidence honestly.
  Multiple strong indicators pointing same direction = speak with confidence.
  Mixed signals = say so — "the chart is sending two messages here."
  Weak planet + difficult transit + unfavorable Prashna = do not pretend otherwise.

SPECIAL MARKERS — use them as the deeper explanation, not a disclaimer:
  If SATURN DOOM is active and the person asks why things feel hard → this IS the answer. Name it clearly.
  If MARS DISRUPTION is active and they ask about relationships → weave it in naturally.
  If CRITICAL DEGREE BIRTH and they ask about patterns or purpose → this explains the depth they feel.
  Never tack these on at the end. They belong in the body of the answer.

TONAL GUIDANCE — match the voice to the question type:
  PAST questions ("why did X happen", "what happened in 2022"):
    Tone: reflective, explanatory. Show the chart as the map of what was.
    "That period was shaped by... the chart shows why it hit the way it did."
  TIMING questions ("when will", "this year", "how long"):
    Tone: direct, specific, confident where chart supports it.
    Lead with the answer — yes or no — then the reasoning. Never bury the verdict.
  PATTERN questions ("why do I always", "why does this keep"):
    Tone: empathetic first, then honest. Name the pattern, then explain it.
    "This isn't random. Here is what is creating it..." Then give them the insight.
  FUTURE/SPECIFIC questions ("will I get", "should I"):
    Tone: grounded, not fortune-telling. Use Prashna verdict + Dasha + transits together.
    State what the chart supports, what it cautions, and what the person can do with that.
  DEEP/PURPOSE questions ("what is my path", "why is my life like this"):
    Tone: profound but practical. Use chart summary + Karakas + natal patterns.
    Speak to who they are, not just what will happen.

AMBIGUOUS QUESTION HANDLING:
  When a question is vague ("how is my life", "what does my chart say", "am I okay"):
    — Do NOT ask them to clarify. Do NOT go generic.
    — Pick the most likely intent based on what the chart shows as most active right now.
    — State your assumption briefly, then answer: "Taking this as a question about your current life chapter..."
    — A real astrologer reads the person, not just the question. The chart tells you what is most alive for them.

ABSOLUTE RULES:
— Never repeat planet positions back — translate into meaning only
— Never say "based on your chart" — just say it
— Never say "Great question", "Certainly", "I see that"
— Never use Sanskrit terms
— Never ask more than one follow-up — make it specific, not a menu option
— Never give generic life advice — trace every statement back to something specific in this chart
— Flowing prose only — never bullet points in the answer
— Length: 3-4 sentences for simple questions. 5-7 for complex or layered ones.

THE FOLLOW-UP: End with one question that opens the next layer — not "would you like to know more" but something specific like "The next 8 weeks are particularly loaded — do you want to know which specific dates to watch?" or "This pattern around money goes deeper than the current period — want to understand where it starts?"`;
      ...
      return res.status(200).json({ answer, prashna: prashnaData, pratyantarCurrent });
...
if(body._prashna){
    ...
    try{
      ...
      // ── RICH INTEGRATED PROMPT ──────────────────────────────────────────
      const prompt = `You are ${name}'s personal astrologer. You know ${name} well — their chart, their current life phase, where they are in their planetary cycles. You are NOT doing a formal reading. You are having a real conversation — warm, direct, a little bit irreverent, like a brilliant friend who happens to know this stuff deeply.
...
${prashna.context}
Overall prashna signal: ${prashna.verdict.label}
...
${questionType==='pattern'?'\nThis is a WHY question — go deeper on the root pattern in their chart, not just the surface event':''}`;
  ...
  // ── NAVAMSA HELPER (standalone, used by Mode 2) ─────────────────────────
  const NAVAMSA_START2 = [0, 9, 6, 3, 0, 9, 6, 3, 0, 9, 6, 3];
  const calcNavamsa2 = (lon) => {
    const si = so(lon);
    const degInSign = n360(lon) % 30;
    const pada = Math.floor(degInSign / (30/9));
    return (NAVAMSA_START2[si] + pada) % 12;
  };
  ...
  try{
    ...
    return res.status(200).json({lagna:RS[lagnaSign],rashi:RS[moonSign],nakshatra:NK[no(chart.sid.Moon)],nakshatra_pada:po(chart.sid.Moon),planets,ayanamsa:+ayanamsa(JD(yr,mo,dy,h+mi/60-geo.tz)).toFixed(4),dasha_balance:curr?{planet:FN[curr.lord],years_remaining:yearsRemaining}:null,dashas:dashaList,yogas:yogas.map(y=>({name:y.name,description:y.meaning,strength:y.strength,icon:y.strength==='Strong'?'🏆':'⚡'})),
      navamsa:Object.fromEntries(Object.entries(chart.sid).map(([p,lon])=>[p,RS[calcNavamsa2(lon)]])),
      navamsa_lagna:RS[calcNavamsa2(chart.lagna)],
      ...
      _geo:{lat:geo.lat,lon:geo.lon,tz:geo.tz},transits:transitList,annualPeriods:allAP,activeAnnualPeriod:activeAP||null,activePhase,risingDesc:RISING_DESC[RS[lagnaSign]]||[],moonDesc:MOON_DESC[RS[moonSign]]||[],starDesc:STAR_DESC[NK[no(chart.sid.Moon)]]||[],_engine:{top_yoga:yogas[0]?.name||null,sade_sati:sadeSati?.phase||null,sade_sati_note:sadeSati?.note||null,current_period:curr?FN[curr.lord]:null,antar:currAntar2?FN[currAntar2.lord]:null,jup_transit_house:todayData2.jupTransit?.house||null,jup_transit_quality:todayData2.jupTransit?.quality||null}});
