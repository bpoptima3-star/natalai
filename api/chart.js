module.exports = async function handler(req, res) {
  ...
  if (body._direct && body._prompt) {
    try {
      ...
      if (!r.ok) return res.status(500).json({ error: 'Claude error', details: d });
...
Lahiri Ayanamsa, Whole Sign houses, Vimshottari Dasha. English names only (Aries not Mesha, Venus not Shukra etc). Keep Rahu and Ketu.
