...
module.exports = async function handler(req, res) {
  ...
  if (body._direct && body._prompt) {
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-sonnet-4-6', max_tokens: 8000, messages: [{ role: 'user', content: body._prompt }] }),
      });
      const d = await r.json();
      if (!r.ok) {
        console.error('Sonnet API error:', JSON.stringify(d));
        return res.status(500).json({ error: 'Claude API error', details: d });
      }
      const raw = d.content?.[0]?.text || '';
      console.log('Report raw response (first 200):', raw.slice(0, 200));
      const cleaned = raw.replace(/```json\s*/g, '').replace(/```\s*/g, '').trim();
      try { return res.status(200).json(JSON.parse(cleaned)); }
      catch(e) {
        console.error('Report JSON parse failed. Raw:', cleaned.slice(0, 500));
        return res.status(500).json({ error: 'JSON parse failed', raw: cleaned.slice(0, 400) });
      }
    } catch(err) {
  ...
  // ── MODE 2: Free chart — simplified template to fit in tokens ──
...
Adjust ALL values for the actual birth details. Calculate accurate planetary positions.`;
  ...
  try {
    ...
    const d = await r.json();
    if (!r.ok) {
      console.error('Haiku API error:', JSON.stringify(d));
      return res.status(500).json({ error: 'Claude API error', details: d });
    }
    const raw = d.content?.[0]?.text || '';
    console.log('Free chart raw (first 200):', raw.slice(0, 200));
    const cleaned = raw.replace(/```json\s*/g, '').replace(/```\s*/g, '').trim();
    try { return res.status(200).json(JSON.parse(cleaned)); }
    catch(e) {
      console.error('Free chart JSON parse failed:', cleaned.slice(0, 300));
      return res.status(500).json({ error: 'JSON parse failed', raw: cleaned.slice(0, 300) });
    }
  } catch(err) {
    console.error('Free chart handler error:', err.message);
    return res.status(500).json({ error: err.message });
  }
};
