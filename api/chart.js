...
function getPrashnaTiming(prashnaChart, targetHouseOrPlanet) {
  ...
  if(!timingNote) {
    ...
    const approxDays = Math.ceil(remaining * 2.3); // ~2.3 days per degree
    timingNote = `Moon moves through ${RS[moonSign]} for approximately ${Math.min(approxDays, 25)} more days — this is the active window for this question`;
  }
...
function computePrashna(questionText, lat, lon, tz, natalChartData) {
  const now = new Date();
  const yr=now.getUTCFullYear(), mo=now.getUTCMonth()+1, dy=now.getUTCDate();
  const h=now.getUTCHours(), mi=now.getUTCMinutes();

  // Cast Prashna chart for THIS moment at questioner's location
  const prashnaChart = computeChart(yr, mo, dy, h+(tz||5.5), mi, tz||5.5, lat, lon);
  const lagnaSign = so(prashnaChart.lagna);
  ...
  return {
    ...
    context: `PRASHNA CHART (cast: ${dateStr} ${timeStr})
Question category: ${category.label}
Prashna Ascendant: ${lagnaSignName} | Ruling planet: ${ascLord}
Moon: ${RS[so(prashnaChart.sid.Moon)]} ${moonPhase.phase} (${moonStr?.strength||'?'}) — ${moonPhase.meaning.slice(0,60)}
Ascendant lord ${ascLord}: ${ascLordStr?.sign} ${ascLordStr?.houseLabel} (${ascLordStr?.strength||'?'}${ascLordStr?.dignity?' '+ascLordStr.dignity:''})
${category.label} house lord ${primaryHouseLord}: ${primaryLordStr?.sign} ${primaryLordStr?.houseLabel} (${primaryLordStr?.strength||'?'}${primaryLordStr?.dignity?' '+primaryLordStr.dignity:''})
Key indicators: ${keyRules.slice(0,2).map(r=>(r.favorable?'✓':'✗')+' '+r.rule.slice(0,60)).join(' | ')}
...
async function geocode(place){
  ...
  // Layer 1: Open-Meteo (fast, good coverage)
  try{
  ...
  // Layer 2: Nominatim/OpenStreetMap (much broader, knows every town globally)
  try{
    ...
    if(d?.[0]){
      ...
      // Estimate timezone from longitude (rough but always works)
      const tzOffset=Math.round(lon/15);
      return{lat,lon,tz:tzOffset};
    }
  }catch(e){}

  // Layer 3: Country capital fallback — detect country from input
  ...
  // Layer 4: UTC default — never crash, always generate
  // Use London as neutral default (UTC+0)
...
function wrapHTML(sections,name,type,cd){
  const LABELS={natal:'Birth Chart + Year Reading',timing:'Life Timing Guide',cosmic:'Cosmic Chemistry Reading'};
  const label=LABELS[type]||'Astrological Reading';
  const today=new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'});
  const body=sections.map(s=>`
  <div style="border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:0">
    <div style="padding:28px 0 0"><h2 class="rep-sh">${s.title}</h2></div>
    <div style="padding:4px 0 28px;font-family:Cormorant Garamond,serif;font-size:17px;line-height:1.8;color:#2a2218 !important;background:#ffffff !important">${s.html}</div>
  </div>`).join('');
...
.rep-sh{font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:400;color:#1a1a1a;padding-bottom:10px;border-bottom:none;margin-bottom:16px;display:flex;align-items:center;gap:10px}
.rep-sh::after{content:'';flex:1;height:1px;background:linear-gradient(to right,#bf9a30,transparent);margin-left:12px}
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      let geo;
      geo=await geocode(pob||'New Delhi, India');
...
Jupiter Transit: House ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      if(reportType==='compat'||reportType==='cosmic'){
        let geo2;
        geo2=await geocode(pob2||pob||'New Delhi, India');
...
${name} Navamsa (D9 marriage chart): D9 Rising ${navamsaLagnaSign} | Venus D9 in ${venusD9} | Moon D9 in ${moonD9}
${vargottamaStr?`${name} ${vargottamaStr}`:''}${combustStr?`\n${name} ${combustStr}`:''}
${name} patterns: ${yogas.map(y=>y.name).join(', ')||'standard'}
${yogaKarakaStr}
${parivartanaStr}
H7 (partnership) lord: ${houseLords.find(hl=>hl.house===7)?.desc||''} | Aspects: ${marriage7aspects.join(', ')||'none'}`;
...
American English only. Personal, warm, honest.`;
      ...
      const munthaStr = muntha ? `Annual Lagna Progressor (Muntha): ${muntha.signName}, lord ${muntha.lordName}` : '';

      // Panchadhikari (5 Tajik year rulers)
      const srLagnaLon = n360(calcLagna(lastSRJD,geo.lat,geo.lon)-srAy1x);
      const pancha = getPanchadhikari(srLagnaLon, n360(natal.sid.Sun-ayanamsa(lastSRJD)), natal.sid.Moon, muntha?.sign||0);
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      const NAVAMSA_START = [0, 4, 8, 0, 4, 8, 0, 4, 8, 0, 4, 8]; // Aries/Leo/Sag start by sign
      ...
      // Atmakaraka: planet with highest degree in sign (soul indicator)
      ...
      const combustStr = combustPlanets.length
        ? `Combust (weakened): ${combustPlanets.join(', ')}`
        : '';
      ...
      const gandantaStr = gandantaPlanets.length
        ? `Gandanta (transition stress): ${gandantaPlanets.join(', ')}`
        : '';
      ...
      const RETRO_CHECK = ['Mercury','Venus','Mars','Jupiter','Saturn'];
      ...
      const parivartanaStr = parivartanaYogas.length
        ? `Parivartana Yogas: ${parivartanaYogas.map(y=>`${y.planets} (${y.houses}, ${y.type})`).join('; ')}`
        : '';
      ...
      const YOGA_KARAKA_BY_LAGNA = {
        Aries:       {planet:'Saturn', note:'Rules H10 (kendra) + H11 — exceptional for Aries, karma & gains'},
        Taurus:      {planet:'Saturn', note:'Rules H9 (trikona) + H10 (kendra) — most powerful yoga karaka'},
        Gemini:      {planet:'Venus',  note:'Rules H5 (trikona) + H12 — creative and spiritual gift'},
        Cancer:      {planet:'Mars',   note:'Rules H5 (trikona) + H10 (kendra) — most powerful for Cancer'},
        Leo:         {planet:'Mars',   note:'Rules H4 (kendra) + H9 (trikona) — fortune and home'},
        Virgo:       {planet:'Venus',  note:'Rules H2 + H9 (trikona) — wealth and fortune planet'},
        Libra:       {planet:'Saturn', note:'Rules H4 (kendra) + H5 (trikona) — most powerful for Libra'},
        Scorpio:     {planet:'Jupiter',note:'Rules H2 + H5 (trikona) — wisdom and wealth'},
        Sagittarius: {planet:'Mars',   note:'Rules H5 (trikona) + H12 — creative force'},
        Capricorn:   {planet:'Venus',  note:'Rules H5 (trikona) + H10 (kendra) — most powerful for Capricorn'},
        Aquarius:    {planet:'Venus',  note:'Rules H4 (kendra) + H9 (trikona) — fortune and home'},
        Pisces:      {planet:'Mars',   note:'Rules H2 + H9 (trikona) — wealth and fortune'},
      };
      ...
      // Vimshottari sequence + years (DS order: Ke,Ve,Su,Mo,Ma,Ra,Ju,Sa,Me)
      ...
      const navamsaLagnaSign = RS[navamsaLagna];
      const vargottamaStr = vargottama.length
        ? `Vargottama (exceptionally strong): ${vargottama.join(', ')}`
        : '';
      // Key planets in navamsa for marriage/soul analysis
      const venusD9 = RS[navamsa['Venus']];
      ...
      const NEWFEATURES = [
        navamsaStr,
        vargottamaStr,
        atmStr ? `Atmakaraka (soul planet): ${atmStr}` : '',
        combustStr,
        gandantaStr,
        dashaSandhi,
        savStr,
        bhavaStr ? `Bhava Chalit shifts: ${bhavaStr}` : '',
        retroStr,
        yogaKarakaStr,
        ykNote,
        parivartanaStr,
      ...
      const satNote = todayData.sadeSati ? `Saturn Pressure active — ${todayData.sadeSati.phase}: ${todayData.sadeSati.note}` : '';
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        const lines = cleanInterp.split('\n').filter(l=>l.trim()).map(l=>
          l.startsWith('~transit~')
            ? `<p style="font-family:Georgia,'Times New Roman',serif;font-size:14px;line-height:1.7;color:#7c5a2a;font-style:italic;padding-top:6px;border-top:1px solid #e8d5a0;margin-top:8px;margin-bottom:0">${l.replace('~transit~','').trim()}</p>`
            : `<p style="font-family:Georgia,'Times New Roman',serif;font-size:15px;line-height:1.7;color:#222222;margin:0 0 8px">${l.trim()}</p>`
        ).join('');
        return `<div style="border-radius:10px;overflow:hidden;border:1px solid #e8e8e8;background:#ffffff;border-left:4px solid ${p.color||'#bf9a30'};margin-bottom:14px">
      ...
      const processSection = async (sec) => {
        ...
        if(sec.isMonthGuide){
          ...
          for(let i=0;i<12;i++){
            ...
            rows.push(
              '<div style="border-radius:10px;overflow:hidden;border:1px solid #e8e8e8;border-left:4px solid '+(p.color||'#bf9a30')+';margin-bottom:12px;background:#ffffff">'
              ...
              +'<div style="font-family:\'Cormorant Garamond\',Georgia,serif;font-size:17px;font-weight:500;color:#1a1a1a">'+mLabel+nowBadge+'</div>'
              +'<div style="display:flex;align-items:center;gap:6px">'
              +'<span style="font-size:15px">'+(p.emoji||'✦')+'</span>'
              +'<span style="font-family:Arial,sans-serif;font-size:11px;color:'+(p.color||'#bf9a30')+';font-weight:600">'+p.planet+(houseOrd?' · '+houseOrd:'')+statusStr+'</span>'
              +'</div></div>'
              +'<div style="padding:12px 16px 14px;background:#ffffff">'
              +'<p style="font-family:Georgia,\'Times New Roman\',serif;font-size:15px;line-height:1.7;color:#222222;margin:0 0 6px">'+theme+'</p>'
              +'<p style="font-family:Georgia,\'Times New Roman\',serif;font-size:14px;line-height:1.65;color:#555555;font-style:italic;margin:0">'+action+'</p>'
              +'</div></div>'
            );
          }
        ...
        try {
          ...
          for(let ci=0; ci<enhancedPeriods.length; ci+=CARD_CHUNK){
            ...
            const batchResults = await Promise.all(batch.map(async (p) => {
              ...
              if(ptE) {
                interpretation += '\n' + rnd(LEGAL_HIGH) + ': ' + ptE.positive + '.';
              } else if(phdE&&phdE[1]) {
                interpretation += '\n' + phdE[1];
              }
              // Line 3: Watch line with legal-safe language
              if(ptE&&ptE.watch) {
                interpretation += '\n~transit~' + rnd(LEGAL_CAUTION) + ': ' + ptE.watch + '.';
              } else {
...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
        ...
        const labels = ['Who you are','Where you are','Biggest opening this year','Watch out for','This report covers'];
        const dots = ['#bf9a30','#bf9a30','#29956a','#c04040','#6e6e73'];
        const summaryHTML = `<div style="background:#fafaf8;border:1px solid rgba(191,154,48,.2);border-radius:12px;padding:24px 28px;margin-bottom:4px">
          <div style="font-family:'Cormorant Garamond',serif;font-size:22px;font-weight:400;color:#1a1a1a;line-height:1.35;margin-bottom:20px;font-style:italic">${lines[0]||''}</div>
          <div style="display:flex;flex-direction:column;gap:12px">
            ${lines.slice(1).map((line,i)=>`<div style="display:flex;align-items:flex-start;gap:12px;font-size:14px;color:#3d3d3d;line-height:1.6"><span style="width:6px;height:6px;border-radius:50%;background:${dots[i+1]||'#bf9a30'};flex-shrink:0;margin-top:7px"></span><span><strong style="color:#1a1a1a">${labels[i+1]||''}:</strong> ${line}</span></div>`).join('')}
          </div>
          <div style="margin-top:16px;padding-top:12px;border-top:1px solid rgba(0,0,0,.06);font-size:11px;color:#b0b0b0;font-family:'Outfit',sans-serif;letter-spacing:.04em">Full report follows below ↓</div>
        </div>`;
  ...
  if(body._prashna){
    const{question,lat,lon,tz,natalContext,promptOverride}=body;
    ...
    try{
      ...
      // Use client-built prompt if provided (it's already shaped per question type)
      // Otherwise build a default Prashna prompt
      const prompt = promptOverride
        ? `${LANG}\n\n${promptOverride}\n\nPRASHNA DATA:\n${prashna.context}`
        : `${LANG}
You are answering a specific question for this person. American English only.
${natalContext?'NATAL: '+natalContext+'\n':''}${prashna.context}
QUESTION: "${question}"
Answer like a trusted astrologer who knows this person. Lead with what you actually think. Reference real planets and houses. 3-4 lines max. Talk to them directly.`;
      const answer = await claude(apiKey, prompt, 350);
      return res.status(200).json({
        answer,
        prashna:{
  ...
  const NAVAMSA_START2 = [0, 4, 8, 0, 4, 8, 0, 4, 8, 0, 4, 8];
  ...
  try{
    ...
    // Retrograde check for free chart mode
      ...
      const RETRO_CHECK_M2 = ['Mercury','Venus','Mars','Jupiter','Saturn'];
    ...
    return res.status(200).json({lagna:RS[lagnaSign],rashi:RS[moonSign],nakshatra:NK[no(chart.sid.Moon)],nakshatra_pada:po(chart.sid.Moon),planets,dasha_balance:curr?{planet:FN[curr.lord],years_remaining:yearsRemaining}:null,dashas:dashaList,yogas:yogas.map(y=>({name:y.name,description:y.meaning,strength:y.strength,icon:y.strength==='Strong'?'🏆':'⚡'})),
      ...
      kp_sublords:Object.fromEntries(Object.entries(chart.sid).map(([p,lon])=>[p,{nak:NK[no(lon)],nakLord:FN[NL[no(lon)]],subLord:(()=>{const ni=no(lon),ns=360/27,di=n360(lon)%ns,nl=FN[NL[ni]],si=VIM_SEQ.indexOf(nl);let cur=0;for(let i=0;i<9;i++){const pl=VIM_SEQ[(si+i)%9],sp=(VIM_YRS[pl]/120)*ns;if(di>=cur&&di<cur+sp)return pl;cur+=sp;}return nl;})()}])),
      summary,_geo:{lat:geo.lat,lon:geo.lon,tz:geo.tz},transits:transitList,annualPeriods:allAP,activeAnnualPeriod:activeAP||null,activePhase,risingDesc:RISING_DESC[RS[lagnaSign]]||[],moonDesc:MOON_DESC[RS[moonSign]]||[],starDesc:STAR_DESC[NK[no(chart.sid.Moon)]]||[],_engine:{top_yoga:yogas[0]?.name||null,sade_sati:sadeSati?.phase||null,sade_sati_note:sadeSati?.note||null,current_period:curr?FN[curr.lord]:null,antar:currAntar2?FN[currAntar2.lord]:null,jup_transit_house:todayData2.jupTransit?.house||null,jup_transit_quality:todayData2.jupTransit?.quality||null}});
