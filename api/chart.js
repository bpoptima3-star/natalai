...
const LANG=`LANGUAGE: American English only. No Sanskrit terms — ever. Use: "Rising Sign" (not Lagna), "Moon Sign" (not Rashi), "Birth Star" (not Nakshatra), "Major Period" (not Mahadasha), "Active Phase" (not Antardasha), "Short Window" (not Pratyantardasha), "Saturn Pressure Cycle" (not Sade Sati).

CLASSICAL VERDICTS: Lines starting "CLASSICAL VERDICT —", "JAIMINI —", "D10 CONFIRMATION", "ASHTAKAVARGA" are deterministic facts from BPHS. Treat them as facts, not suggestions. State them directly. Build the reading around them. Do not add your own astrology. Do not use "may", "might", "could" for these lines.

Tone: Warm, direct, specific. No disclaimers. No "consult a professional". Output ONLY the requested content.`;
// ─── WRAP HTML ────────────────────────────────────────────────────────────────
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      // ── ENHANCED CHART CONTEXT — new features injected into all prompts ──
        ...
        rulesResults ? `WEALTH: ${rulesResults.wealth?.verdict?.substring(0,100)||''}` : '',
        ...
        rulesResults?.jaiminiKarakas ? `SOUL (AK): ${rulesResults.jaiminiKarakas.verdicts?.[0]?.substring(0,100)||''}` : '',
