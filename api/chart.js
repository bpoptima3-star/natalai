 ...
 */
function buildEngineChart(natal, lagnaSign, gender, mahaName, antarName) {
  ...
  return {
    ...
    planets,
    houseLords,
    dasha: { maha: mahaName, antar: antarName, pratyantar: null }
  };
}
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      // ── VEDIC RULES ENGINE — deterministic verdicts (BPHS, Phaladeepika, Saravali) ──
      // All chart data already computed above — engine applies 300+ classical rules
      let rulesResults = null;
      try {
        if (VedicRulesEngine) {
          const engChart = buildEngineChart(natal, lagnaSign, gender, mahaName, antarName);
          const engine = new VedicRulesEngine(engChart);
          rulesResults = engine.evaluate();
        }
      } catch(e) {
...
Jupiter Transit: House ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      if(reportType==='compat'||reportType==='cosmic'){
        ...
        try {
          if (VedicRulesEngine) {
            const ec2 = buildEngineChart(natal2, ls2, gender2||'F', '', '');
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      const NEWFEATURES = [
        ...
        rulesResults ? `CLASSICAL VERDICT — MENTAL NATURE: ${rulesResults.mentalNature?.verdicts?.slice(0,2).join(' | ')||''}` : '',
        rulesResults ? `CLASSICAL YOGAS DETECTED: ${rulesResults.yogas?.summary||''}` : '',
        rulesResults ? `CLASSICAL DASHA READING: ${rulesResults.dashaResults?.verdict||''}` : '',
      ].filter(Boolean).join('\n');
