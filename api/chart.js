...
.rep-meta-label{font-size:8px;font-weight:600;letter-spacing:.2em;text-transform:uppercase;color:rgba(255,255,255,.35)}
...
${name} patterns: ${yogas.map(y=>y.name).join(', ')||'standard'}`;
      ...
      const SECTION_PROMPTS = reportType === 'timing' ? [
        ...
        {title:'💼 Career, Money & Opportunities',tok:950,prompt:`${LANG}\nData:\n${core}\nWrite 3-4 rich paragraphs (280-350 words total): (1) ${name}'s long-term career direction based on natal patterns. (2) Financial patterns and wealth potential. (3) Best months THIS YEAR for career moves, negotiations, launches (cite specific periods). (4) What to avoid financially and professionally. Be specific — reference actual planetary patterns and period dates. Legal-safe: "indicators suggest" language.`},
        {title:'❤️ Love, Relationships & Connection',tok:1000,prompt:`${LANG}\nData:\n${core}\n${patterns.filter(p=>p.type.includes('Mars')||p.type.includes('emotion')).map(p=>p.type+': '+p.meaning).join('\n')}\nWrite 3-4 rich paragraphs (300-380 words total): (1) ${name}'s relationship style and what they need in a partner. (2) Attachment patterns and emotional tendencies (reference any patterns above). (3) Relationship timing THIS YEAR — best periods for new connections or deepening existing ones. (4) Red flags to watch in their own relationship behavior. Honest, specific, personal. Legal-safe language.`},
        {title:'🏃 Health, Family & Growth Edges',tok:950,prompt:`${LANG}\nData:\n${core}\nWrite 3 rich paragraphs (270-340 words total): (1) Health patterns from the chart — body areas to monitor, this year's health-focused periods, mental/emotional wellbeing guidance. Note: "your chart shows patterns associated with" — never diagnose. (2) Family and home themes this year. Parent/sibling dynamics if indicated. (3) Growth edges — what life is asking ${name} to develop, what challenges are actually opportunities, and an empowering closing paragraph about their chart's overall story.`},
      ];
  ...
  <div class="p-card-body">${lines}</div>
      ...
      const buildPeriodPrompt = (p) => {
        const houseMeaning = p.srHouse ? HM[p.srHouse]||'' : '';
        const natalCtx = p.natalHouse ? `${p.planet} sits in your ${p.natalHouse}th house natally` : '';
        return `${LANG}
Write a reading for ONE sub-period in ${name}'s annual solar chart. 4-5 short punchy lines. No paragraphs. No headers.
Period: ${p.planet} · ${p.start} → ${p.end} · ${p.days} days${p.current?' · ACTIVE ('+p.daysLeft+' days left)':''}
In annual chart: ${p.planet} is in the ${p.srHouse||'?'}th House — ${houseMeaning}
Natal context: ${natalCtx}${p.srPlanetStatus?' · '+p.srPlanetStatus+' in the solar chart':''}
Transit context: ${transitCtx}

Line 1: What the energy feels like — personal, second person, sensory and specific to THIS house
Line 2: The specific action or window — direct, almost instructional
Line 3: The nuance, caution, or natal layer — what to watch or use carefully
Line 4 (only if transit confirms or complicates): Start with ~transit~ then 1-2 lines, conversational, no label
Tone: A brilliant friend who has studied ${name}'s chart speaking directly and personally.
Do NOT write generic astrology. Make every line specific to this house placement.`;
      };
      ...
      // Run all sections + all sub-period cards in parallel
      const sectionResults = await Promise.all([
        // Regular sections (skip isSubPeriod placeholder)
        ...SECTION_PROMPTS.filter(sec=>!sec.isSubPeriod).map(async (sec) => {
          try {
            const text = await claude(apiKey, sec.prompt, sec.tok);
            const html = text
              .replace(/^## (.+)$/gm,'<h2 class="rep-sh">$1</h2>')
              .replace(/^### (.+)$/gm,'<h3 style="font-family:Cormorant Garamond,serif;font-size:19px;color:#9a7020;margin:14px 0 8px">$1</h3>')
              ...
              .replace(/^(?!<)/gm,'<p>')
              .replace(/<p><\/p>/g,'');
            // Special styling for Year at a Glance
            ...
            return { title: sec.title, html: `<p style="color:#999;font-style:italic">Section temporarily unavailable.</p>` };
          }
        }),
        // Sub-period cards section (one Claude call per period, all parallel)
        (async () => {
          if(!SECTION_PROMPTS.find(s=>s.isSubPeriod)) return null;
          try {
            const cardResults = await Promise.all(
              enhancedPeriods.map(async (p) => {
                try {
                  const interpretation = await claude(apiKey, buildPeriodPrompt(p), 260);
                  // Inject phase divider before first phase-2 period
                  const divider = (birthdayInWindow && p.phaseTwo && !enhancedPeriods.find((ep,i)=>ep.phaseTwo&&enhancedPeriods.indexOf(ep)<enhancedPeriods.indexOf(p))) ? phaseDividerHTML : '';
            ...
            return {title:'🗓 Your Solar Year — Sub-Period Guide', html:'<p>Sub-period guide unavailable.</p>'};
          }
        })(),
      ].filter(Boolean));

      const finalSections = sectionResults.filter(Boolean);
