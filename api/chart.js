    ...
    try{
      ...
      try {
        if (VedicRulesEngine) {
          ...
          try {
            const { computeCharaDasha } = require('./vedic-rules-engine-v3');
...
if(body._prashna){
    ...
    try{
      ...
      let ithasalaVerdict = '';
      try {
        const { evaluatePrashnaAspects } = require('./vedic-rules-engine-v3');
  ...
  try{
    ...
    return res.status(200).json({lagna:RS[lagnaSign],rashi:RS[moonSign],nakshatra:NK[no(chart.sid.Moon)],nakshatra_pada:po(chart.sid.Moon),planets,ayanamsa:+ayanamsa(JD(yr,mo,dy,h+mi/60-geo.tz)).toFixed(4),dasha_balance:curr?{planet:FN[curr.lord],years_remaining:yearsRemaining}:null,dashas:dashaList,yogas:yogas.map(y=>({name:y.name,description:y.meaning,strength:y.strength,icon:y.strength==='Strong'?'🏆':'⚡'})),
      ...
      _engineVerdicts: freeChartEngine ? {
        marriage:    freeChartEngine.marriage?.verdict,
        career:      freeChartEngine.career?.verdict,
        wealth:      freeChartEngine.wealth?.verdict,
        health:      freeChartEngine.health?.verdict,
        children:    freeChartEngine.children?.verdict,
        father:      freeChartEngine.father?.verdict,
        mother:      freeChartEngine.mother?.verdict,
        siblings:    freeChartEngine.siblings?.verdict,
        foreignTravel: freeChartEngine.foreignTravel?.verdict,
        spirituality:freeChartEngine.spirituality?.verdict,
        yogas:       freeChartEngine.yogas?.summary,
        dasha:       freeChartEngine.dasha?.verdict,
        upapada:     freeChartEngine.upapada?.verdicts?.[0],
        arudha:      freeChartEngine.arudhaLagna?.verdicts?.[0],
        remedies:    freeChartEngine.remedies?.summary,
        transits:    freeChartEngine.transits?.verdicts?.slice(0,2),
