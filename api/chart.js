...
American English only. Personal, direct, warm. Talk to ${name}, not about them.`;
  ...
  // ── MODE: ASK ASTROLOGER ─────────────────────────────────────────────────
  if(body._ask){
    const{question,history,firstName,natalCtx,dashaCtx,transitCtx,topicCtx,currentLat,currentLon,qType,qTopic,antarLord,antarStart,antarEnd,lat,lon,tz,specialMarkers:clientMarkers,sessionFlags:clientFlags,d9Ctx:clientD9,d10Ctx:clientD10,karakaCtx:clientKarakas,pratyantarStr:clientPratyantar,engineInput:clientEngineInput,dob,birthTime,natalMoonLon,enrichment:clientEnrichment,shadbala:clientShadbala,dashas:clientDashas}=body;
    ...
    try{
      ...
      try {
        if (VedicRulesEngine && clientEngineInput) {
          ...
          // PHASE 5: askRulesCtx ships engine FINDINGS to rishi, not compressed bucket strings.
          // Routed domain gets top-8 findings by |weight| with classical citations.
          // Non-routed domains stay compressed one-liners (token budget discipline).
          // Phase 1 audit root cause: the old builder discarded all findings and shipped only the
          // 5-bucket compressed verdict — the rishi pattern-matched around it instead of narrating
          // from actual classical evidence. This fixes that at the information-flow level.

          // ── Sensitivity detection (inline — SENSITIVE_RULE_PATTERNS from Phase 2 engine) ──────
          ...
          // ── Domain verdict helper (handles .verdict vs .verdicts[] shapes) ────────────────────
          const _domainSummary = (obj) => {
            if (!obj) return '';
            if (obj.verdict) return obj.verdict;
            if (obj.verdicts && obj.verdicts.length) return obj.verdicts[0].substring(0, 150);
            if (obj.summary) return obj.summary;
            return '';
          };

          // ── Top-N findings builder for routed domain ───────────────────────────────────────────
          // Sorts by |weight| descending, excludes COMPOUND rule (it becomes the Overall line),
          // caps at 8. Each finding ships with verdict text + classical source citation.
          ...
          // ── Full-findings block for the routed (primary) domain ───────────────────────────────
          const _buildFullFindingsBlock = (label, domainObj, domainScore, extraMeta) => {
            if (!domainObj) return '';
            const findings = _topFindings(domainObj.rules, 8);
            const overall = _domainSummary(domainObj);
            const isSensitive = _hasSensitiveFinding(domainObj.rules);
            const lines = [];
            lines.push('──────────');
            lines.push(label + ' (engine findings — narrate FROM these, do not introduce chart facts not present here):');
            if (overall) lines.push('  Overall: ' + overall);
            if (findings.length) {
              lines.push('  Key findings (top ' + findings.length + ' by classical weight):');
              for (const f of findings) {
                const text = (f.verdict||'').slice(0, 220);
                const src  = f.source ? ' [' + f.source + ']' : '';
                lines.push('    - ' + text + src);
              }
            }
            if (domainScore !== undefined) {
              lines.push('  Score context: ' + domainScore + ' (internal — NEVER state this number or any derived percentage to the user)');
            }
            if (isSensitive) {
              lines.push('  Sensitivity: HIGH — At least one fired finding carries a difficult classical condition.');
              lines.push('    Frame with care: lead with what is possible, frame difficulty as path-requiring-navigation,');
              lines.push('    never deliver as impossibility or fated verdict. Phase 6 persona rules extend this.');
            }
          ...
          // ── Primary domain resolution (Flag 4) ───────────────────────────────────────────────
          // 1. qTopic if it maps to a domain with rules
          // 2. else detectEventDomain(question) result
          // 3. else no primary (all domains stay compressed — Path A / general question)
          ...
          const primaryDomain = (_qtDomain && er[_qtDomain]?.rules?.length) ? _qtDomain
            ...
            : null;
          // Secondary: if qTopic and detectEventDomain differ, surface secondary as a visible one-liner
          ...
          // ── Build askRulesCtx ─────────────────────────────────────────────────────────────────
          const ctxLines = ['\n\nCLASSICAL ANALYSIS (pre-computed from chart — narrate from findings below; do not introduce facts not present here):'];

          if (primaryDomain === 'marriage') {
            ctxLines.push(_buildFullFindingsBlock('MARRIAGE ANALYSIS', er.marriage, er.marriage?.score,
              `Timing: ${er.marriage?.timing || 'depends on dasha activation'} | Spouse indicators: ${er.marriage?.spouse?.slice(0,3).join(', ')||'—'}`));
          } else if (primaryDomain === 'career') {
            ctxLines.push(_buildFullFindingsBlock('CAREER ANALYSIS', er.career, er.career?.score,
              `Fields: ${er.career?.fields?.slice(0,4).join(', ')||'—'}`));
          } else if (primaryDomain === 'health') {
            ctxLines.push(_buildFullFindingsBlock('HEALTH ANALYSIS', er.health, er.health?.score, null));
          } else if (primaryDomain === 'wealth') {
            ctxLines.push(_buildFullFindingsBlock('WEALTH ANALYSIS', er.wealth, er.wealth?.score,
              `Sources: ${er.wealth?.sources?.slice(0,3).join(', ')||'—'}`));
          } else if (primaryDomain === 'children') {
            ctxLines.push(_buildFullFindingsBlock('CHILDREN ANALYSIS', er.children, er.children?.score, null));
          } else if (primaryDomain === 'foreignTravel') {
            ctxLines.push(_buildFullFindingsBlock('FOREIGN TRAVEL ANALYSIS', er.foreignTravel, er.foreignTravel?.score, null));
          } else if (primaryDomain === 'education') {
            ctxLines.push(_buildFullFindingsBlock('EDUCATION ANALYSIS', er.education, er.education?.score, null));
          } else if (primaryDomain === 'dasha') {
            ctxLines.push(_buildFullFindingsBlock('DASHA ANALYSIS', er.dasha, null, null));
          } else if (primaryDomain === 'jaiminiKarakas') {
            ctxLines.push(_buildFullFindingsBlock('SOUL PURPOSE / JAIMINI KARAKAS', er.jaiminiKarakas, null, null));
          } else if (primaryDomain) {
            // Catch-all for any other mapped domain
            ctxLines.push(_buildFullFindingsBlock(primaryDomain.toUpperCase() + ' ANALYSIS', er[primaryDomain], er[primaryDomain]?.score, null));
          }

          // Secondary domain — visible one-liner, placed prominently (not buried)
          if (secondaryDomain) {
            ctxLines.push(secondaryDomain.toUpperCase() + ': ' + _domainSummary(er[secondaryDomain]));
          }

          // ── Compressed one-liners for non-primary domains ────────────────────────────────────
