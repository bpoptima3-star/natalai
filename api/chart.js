...
function wrapHTML(sections,name,type,cd){
  ...
  <div style="border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:0">
    <div style="padding:28px 0 0"><h2 style="font-family:Cormorant Garamond,serif;font-size:24px;font-weight:400;color:#1a1a1a;padding-bottom:10px;margin-bottom:16px;display:flex;align-items:center;gap:10px">${s.title}<span style="flex:1;height:1px;background:linear-gradient(to right,#bf9a30,transparent);display:inline-block;margin-left:12px"></span></h2></div>
    <div style="padding:4px 0 28px;font-family:Cormorant Garamond,serif;font-size:17px;line-height:1.8;color:#2a2218">${s.html}</div>
  </div>`).join('');
...
<div style="padding:40px 48px;background:#fafaf8">${body}</div>
...
Jupiter: house ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      const SECTION_PROMPTS = reportType === 'timing' ? [
        ...
        {title:'📆 Month-by-Month Guide',tok:1200,prompt:`${LANG}\n${name} | ${lagna} Rising · ${rashi} Moon\nPeriods: ${allPeriods.map(p=>p.planet+' '+p.start+'→'+p.end+(p.current?' NOW':'')).join(' | ')}\n\nFor each of the next 12 months (use actual month names and year, e.g. 'May 2026:'): 2-3 sentences — which period is active, key theme for ${name} specifically, one concrete action. Personal calendar tone. Reference actual planet names and dates.`},
        ...
        {title:'🗓 Your Solar Year — Sub-Period Guide',tok:1600,isSubPeriod:true},
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        const badge = isActive
          ? `<span style="font-family:Inter,sans-serif;font-size:10px;font-weight:600;letter-spacing:.06em;padding:4px 10px;border-radius:20px;text-transform:uppercase;background:#fef3cd;color:#92650a">Active · ${p.daysLeft} days left</span>`
          : p.phaseTwo ? `<span style="font-family:Inter,sans-serif;font-size:10px;font-weight:600;letter-spacing:.06em;padding:4px 10px;border-radius:20px;text-transform:uppercase;background:#f0ece0;color:#7c5a2a">New Solar Year</span>`
          : `<span style="font-family:Inter,sans-serif;font-size:10px;font-weight:600;letter-spacing:.06em;padding:4px 10px;border-radius:20px;text-transform:uppercase;background:#f0f0f0;color:#666">Upcoming</span>`;
        ...
        const lines = cleanInterp.split('\n').filter(l=>l.trim()).map(l=>
          l.startsWith('~transit~')
            ? `<p style="font-family:Cormorant Garamond,serif;font-size:15px;line-height:1.7;color:#7c5a2a;font-style:italic;padding-top:4px;border-top:1px solid rgba(191,154,48,.15);margin-top:8px;margin-bottom:0">${l.replace('~transit~','').trim()}</p>`
            : `<p style="font-family:Cormorant Garamond,serif;font-size:16px;line-height:1.72;color:#2a2218;margin:0 0 6px">${l.trim()}</p>`
        ).join('');
        return `<div style="border-radius:10px;overflow:hidden;border:1px solid rgba(0,0,0,.08);background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.04);border-left:3px solid ${p.color||'#bf9a30'};margin-bottom:14px">
  <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 18px 12px;border-bottom:1px solid rgba(0,0,0,.06)">
    <div style="display:flex;align-items:center;gap:10px">
      <span style="font-size:20px;line-height:1">${p.emoji||'✦'}</span>
      <div>
        <div style="font-family:Cormorant Garamond,serif;font-size:19px;font-weight:500;color:#1a1a1a">${p.planet}</div>
        <div style="font-family:Inter,sans-serif;font-size:11px;color:#888;font-weight:400">${p.start} → ${p.end} &nbsp;·&nbsp; ${p.days} days</div>
      </div>
    </div>
    ${badge}
  </div>
  <div style="padding:8px 18px;font-size:11px;font-weight:500;letter-spacing:.04em;border-bottom:1px solid rgba(0,0,0,.05);background:${p.color}12;color:${p.color}">${houseLabel}${statusTag}</div>
  <div style="padding:14px 18px 16px">${lines}</div>
</div>`;
      };

      // Build sub-period prompt for each period (all run in parallel with other sections)
      const buildPeriodPrompt = (p) => {
        // Get pre-written house interpretation from library (zero tokens)
        const phdBase = (PHD[p.planet]&&p.srHouse&&PHD[p.planet][p.srHouse]) ? PHD[p.planet][p.srHouse] : null;
        const phdText = phdBase ? phdBase.join('\n') : (HM[p.srHouse]||'');
        const natalCtx = p.natalHouse ? `${p.planet} sits in house ${p.natalHouse} natally` : '';
        return `${LANG}
CONTEXT (do not include in output — use only as background):
${name} | ${lagna} Rising · ${rashi} Moon | ${mahaName} Major Life Period
${p.planet} in Annual House ${p.srHouse||'?'}${p.srPlanetStatus?' ('+p.srPlanetStatus+')':''}${natalCtx?' | Natal: '+natalCtx:''}
House meaning: ${phdText}
Transit: ${transitCtx}

OUTPUT RULES — STRICT:
- Write EXACTLY 1-2 plain sentences. No headers. No bold. No dashes. No markdown. No labels.
- Sentence 1: What this means personally for ${name} — reference their natal position or current ${mahaName} period. Start with "you" or their name.
- Sentence 2 (optional, only if transit strongly confirms or challenges): Start with ~transit~ then one personal sentence.
- If transit is neutral — write only 1 sentence. Do not pad.
Output nothing except the 1-2 sentences.`;
      };

      // Phase divider HTML (if birthday falls in window)
      const phaseDividerHTML = birthdayInWindow
        ? `<div style="background:linear-gradient(135deg,#f0e8d0,#faf5ec);border:1px solid rgba(191,154,48,.3);border-radius:8px;padding:14px 18px;margin:16px 0;text-align:center"><p style="font-family:Cormorant Garamond,serif;font-size:15px;color:#7c5a2a;margin:0;font-style:italic">✦ &nbsp; From your birthday ${fmtDt(nextSRCal)}, a new solar cycle begins &nbsp; ✦</p></div>`
        : '';
        ...
        try {
          ...
          for(let ci=0; ci<enhancedPeriods.length; ci+=CARD_CHUNK){
            ...
            const batchResults = await Promise.all(batch.map(async (p) => {
              try {
                const interpretation = await claude(apiKey, buildPeriodPrompt(p), 260);
                ...
                return buildSubPeriodCard(p, p.planet + ' period — please regenerate.', '');
              }
            }));
      ...
      const finalSections = [...mainSections, subPeriodSection].filter(Boolean);
  ...
  if(body._prashna){
    const{question,lat,lon,tz,natalContext}=body;
    ...
    try{
      ...
      // Build combined prompt: natal context + prashna context + LANG
      const natalPart = natalContext ?
        `NATAL CHART CONTEXT:\n${natalContext}\n\n` : '';
      const prompt=`${LANG}
You are a Jyotisha Prashna (horary astrology) expert for an American audience.
${natalPart}${prashna.context}

The person asked: "${question}"

Using BOTH the Prashna chart above AND the natal chart context, write 3-4 personal sentences:
1. What the Prashna chart says about this specific question right now (cite 1 actual planet/house)
2. What the verdict means practically for them — honest, not generic
3. The timing note — when to expect movement or clarity
4. One specific action they should or should not take based on this chart

Tone: direct, personal, like a brilliant friend who has studied both their chart and this moment.
American English only. Start immediately — no greeting.`;
      const answer = await claude(apiKey, prompt, 300);
      return res.status(200).json({
        answer,
        prashna:{
