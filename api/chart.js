...
${name} patterns: ${yogas.map(y=>y.name).join(', ')||'standard'}`;
        ...
        const compatPrompt=`${LANG}
You are writing a Vedic compatibility reading for an American audience. Be honest, specific, and personal.
${compatCore}
Write a comprehensive compatibility report covering:
1. **Overall Compatibility** (what ${totalScore}/36 means in real terms)
2. **Physical & Emotional Chemistry** (the ${y1}-${y2} animal archetype pairing — describe in plain English what this means for their energy dynamic)
3. **Mental Connection** (${grahaScore}/5 — how their minds work together)
4. **Temperament Match** (${g1}+${g2} — ${ganaScore}/6 — do their natures complement?)
5. **Long-Term Potential** (${bhakootBad?'moon sign tension present':'moon signs harmonious'})
6. **Red Flags & Green Flags** (specific to THEIR combination)
7. **Relationship Psychology** (attachment patterns, communication style, what each needs)
8. **Best Timing for Commitment** (based on their current periods)
9. **Making It Work** (5 specific pieces of advice for THIS couple)
10. **The Verdict** (honest, compassionate summary)
Format: Use ## for section headers. Write in flowing paragraphs with bullet points where appropriate. 2000-2500 words total. Personal, direct, honest.`;

        // ── Cosmic Chemistry — dedicated animal archetype prompt ──────────────
        let finalHtml,finalTitle;
        if(reportType==='cosmic'){
          const cosmicPrompt=`${LANG}
You are a sharp, perceptive astrologer for an American audience. Write a premium Animal Energy Compatibility report. American English only, direct and intelligent.
${name}: Rising ${lagna}, Moon ${rashi}, Birth Star ${NK[ni1]}, Animal archetype: ${y1}, Nature: ${g1}
${name2||'Partner'}: Rising ${RS[ls2]}, Moon ${RS[ms2]}, Birth Star ${NK[ni2]}, Animal archetype: ${y2}, Nature: ${g2}
Physical Energy Score: ${yoniScore}/4${yoniEnemy?' — OPPOSITE ENERGY TYPES (challenging physical chemistry)':''} | Mental Bond: ${grahaScore}/5 | Temperament: ${ganaScore}/6 | Moon Compatibility: ${bhakootBad?'Challenging':'Harmonious'}
Write a detailed Cosmic Chemistry report covering:
1. **Your Animal Energy Types** — explain ${name}'s ${y1} energy and ${name2||'partner'}'s ${y2} energy in vivid plain English. What does each archetype mean for personality, desire, and relating?
2. **Physical & Energetic Chemistry (${yoniScore}/4)** — what this score means for attraction, touch, and day-to-day energy between them. Be honest${yoniEnemy?' — this is an opposite pairing which creates tension':''}
3. **Mental Connection (${grahaScore}/5)** — how their minds interact, communication styles, what lights each other up intellectually
4. **Temperament & Nature (${ganaScore}/6)** — ${g1} + ${g2} — do they flow together or create friction?
5. **Emotional & Moon Compatibility** — ${bhakootBad?'Moon sign tension exists — explain what this means practically':'Moon signs support each other — describe the emotional ease'}
6. **The Raw Truth** — Red flags and green flags specific to THIS pairing
7. **How to Make This Work** — 5 specific, practical pieces of advice for this exact combination
8. **The Verdict** — honest, compassionate, direct summary. What is the true potential of this pairing?
Format: ## headers, flowing paragraphs, 1800-2200 words. Personal, honest, direct.`;
          const cosmicText=await claude(apiKey,cosmicPrompt,3000);
                const cosmicHTML=mdToHTML(cosmicText);
          finalHtml=wrapHTML([{title:'Your Cosmic Chemistry Reading',html:cosmicHTML}],`${name} + ${name2||'Partner'}`,reportType,{lagna,rashi,nakshatra,dob});
          finalTitle=`${name} + ${name2||'Partner'}`;
        } else {
          const compatText=await claude(apiKey,compatPrompt,3000);
          const compatHTML=mdToHTML(compatText);
          finalHtml=wrapHTML([{title:'Your Compatibility Reading',html:compatHTML}],`${name} + ${name2||'Partner'}`,reportType,{lagna,rashi,nakshatra,dob});
          finalTitle=`${name} + ${name2||'Partner'}`;
        }
