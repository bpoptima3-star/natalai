...
function wrapHTML(sections,name,type,cd){
  ...
  <div class="rep-section">
    <div class="rep-sh-wrap"><h2 class="rep-sh">${s.title}</h2></div>
    <div class="rep-content">${s.html}</div>
  </div>`).join('');
...
.rep-content{padding:4px 0 28px;font-family:'Cormorant Garamond',serif;font-size:17px;line-height:1.8;color:#2a2218}
.rep-content p{margin:0 0 14px;font-family:'Cormorant Garamond',serif}
.rep-content strong{font-weight:600;color:#1a1a1a}
.rep-content em{color:#9a7020;font-style:italic}
.rep-content ul{margin:8px 0 14px;padding-left:0;list-style:none}
.rep-content li{padding:5px 0 5px 18px;position:relative;font-family:'Cormorant Garamond',serif}
.rep-content li::before{content:'✦';position:absolute;left:0;color:#bf9a30;font-size:10px;top:8px}
.year-glance{background:linear-gradient(135deg,#0d0d0d,#1a1008);border-radius:12px;padding:24px 28px;margin:0 0 4px;color:#fff}
.year-glance p{font-family:'Cormorant Garamond',serif;font-size:16px;line-height:1.75;color:rgba(255,255,255,.88);margin:0 0 8px}
.year-glance p:last-child{margin:0;color:#bf9a30;font-style:italic;font-weight:500}
.year-glance strong{color:#f0d080}
...
// ─── MARKDOWN → HTML ─────────────────────────────────────────────────────────
const mdToHTML=t=>t.replace(/^# (.+)$/gm,'<h3 style="font-family:Cormorant Garamond,serif;font-size:20px;font-weight:500;color:#1a1a1a;margin:16px 0 8px">$1</h3>').replace(/^## (.+)$/gm,'<h2 class="rep-sh">$1</h2>').replace(/^### (.+)$/gm,'<h3 style="font-family:Georgia,serif;font-size:17px;color:#bf9a30;margin:16px 0 8px">$1</h3>').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>').replace(/\*(.+?)\*/g,'<em>$1</em>').replace(/^[•-] (.+)$/gm,'<li>$1</li>').replace(/(<li>[^\n]*\n?)+/g,s=>'<ul>'+s+'</ul>').replace(/\n\n+/g,'</p><p>').replace(/^(?!<)/gm,'<p>').replace(/<p><\/p>/g,'');
  ...
  if(body._report){
    ...
    try{
      ...
      const enhancedPeriods=allPeriods.map(p=>{
        ...
        const pk=revFN[p.planet];
        const srH=pk&&srSidX[pk]!==undefined?((so(srSidX[pk])-srLagnaSignX+12)%12)+1:null;
        const nH=pk&&natal.sid[pk]!==undefined?((so(natal.sid[pk])-lagnaSign+12)%12)+1:null;
        const srHouseSign=pk&&srSidX[pk]!==undefined?RS[so(srSidX[pk])]:null;
        const srPlanetStatus=pk?pStatus(pk,srH?so(srSidX[pk]):-1):'';
...
Jupiter: house ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        const badge = isActive
          ? `<span class="p-card-badge active">Active · ${p.daysLeft} days left</span>`
          : `<span class="p-card-badge upcoming">Upcoming</span>`;
        const lines = interpretation.split('\n').filter(l=>l.trim()).map(l=>
          l.startsWith('~transit~')
            ? `<p class="transit">${l.replace('~transit~','').trim()}</p>`
            : `<p>${l.trim()}</p>`
        ).join('');
        return `<div class="p-card" style="border-left:3px solid ${p.color||'#bf9a30'}">
  ...
  <div class="p-card-body">${lines}</div>
      ...
      const buildPeriodPrompt = (p) => {
        ...
        return `${LANG}
${name} | ${lagna} Rising · ${rashi} Moon
${p.emoji||'✦'} ${p.planet} · ${p.start} → ${p.end} · ${p.days}d${p.current?' · ACTIVE NOW ('+p.daysLeft+'d left)':''}
Annual house ${p.srHouse||'?'}${p.srPlanetStatus?' ('+p.srPlanetStatus+')':''}:
${phdText}
Natal: ${natalCtx} | Transit: ${transitCtx}

Add EXACTLY 2 personal lines ONLY — do NOT restate the house meaning above:
1. What this specifically means for ${name} given their natal context and current ${mahaName} period — personal, direct
2. Only if a transit above strongly confirms or challenges: start with ~transit~ and say it personally in 1 line
Skip line 2 entirely if the transit is neutral. Be specific. No generic astrology.`;
      };

      // Phase divider HTML (if birthday falls in window)
      const phaseDividerHTML = birthdayInWindow
        ? `<div class="phase-divider"><p>✦ &nbsp; From your birthday ${fmtDt(nextSRCal)}, a new solar cycle begins &nbsp; ✦</p></div>`
        : '';
      ...
      const processSection = async (sec) => {
        try {
          ...
          if(sec.title.includes('Year at a Glance')){
            ...
            const styledLines=lines.map((l,i)=>i===lines.length-1?`<p><em>${l}</em></p>`:`<p>${l}</p>`).join('');
            return {title:sec.title, html:`<div class="year-glance">${styledLines}</div>`};
          }
          ...
          const cardResults = await Promise.all(
            enhancedPeriods.map(async (p) => {
              try {
                ...
                return buildSubPeriodCard(p, 'Reading temporarily unavailable.', '');
              }
            })
          );
          subPeriodSection = {
