...
Jupiter: house ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      const buildSubPeriodCard = (p, interpretation, transitNote) => {
        ...
        return `<div class="p-card" style="border-left-color:${p.color||'#bf9a30'}">
  <div class="p-card-header">
    <div class="p-card-planet">
      <span class="p-card-emoji">${p.emoji||'✦'}</span>
      <div>
        <div class="p-card-name">${p.planet}</div>
        <div class="p-card-dates">${p.start} → ${p.end} &nbsp;·&nbsp; ${p.days} days</div>
      </div>
    </div>
    ${badge}
  </div>
  <div class="p-card-house" style="background:${p.color}12;color:${p.color}">${houseLabel}${statusTag}</div>
  <div class="p-card-body">${lines}</div>
</div>`;
      };
