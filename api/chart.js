        ...
        try {
          ...
          for(let ci=0; ci<enhancedPeriods.length; ci+=CARD_CHUNK){
            ...
            const batchResults = await Promise.all(batch.map(async (p) => {
              ...
              let interpretation = '';
              if(phdEntry) {
                // Line 1: theme, Line 2: action — already personal from PHD
                interpretation = phdEntry[0];
                if(phdEntry[1]) interpretation += '\n' + phdEntry[1];
                // Add natal + mahadasha context as a grounding line
                const natalNote = p.natalHouse
                  ? p.planet+' sits natally in your '+p.natalHouse+['st','nd','rd'][p.natalHouse-1||3]+' house — this annual activation builds on that foundation.'
                  : '';
                const periodNote = mahaName
                  ? 'This period unfolds within your '+mahaName+' Major Life Period, colouring everything with its themes.'
                  : '';
                const contextLine = natalNote || periodNote;
                if(contextLine) interpretation += '\n~transit~' + contextLine;
              } else {
