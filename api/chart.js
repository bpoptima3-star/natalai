 ...
 */
function buildEngineChart(natal, lagnaSign, gender, mahaName, antarName, pratyantarName, navamshaData) {
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      // ── NAVAMSHA (D9) — computed here so engine and prompts both use it ──
      ...
      for (const [planet, lon] of Object.entries(natal.sid)) {
        ...
        navamshaForEngine[planet] = {
          ...
          degree: +((lon%30)%(30/9)*9).toFixed(2)
        };
      }
      ...
      try {
        if (VedicRulesEngine) {
          const engChart = buildEngineChart(natal, lagnaSign, gender, mahaName, antarName, pratyantarStr, navamshaForEngine);
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      // Compute navamsa for all planets — full D1-style objects {sign, signName, house, degree}
      const navamsa = {};
      for(const [planet, lon] of Object.entries(natal.sid)) {
        const d9Sign  = calcNavamsa(lon);
        const d9House = ((d9Sign - navamsaLagna + 12) % 12) + 1;
