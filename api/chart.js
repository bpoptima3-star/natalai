...
function classifyQuestionPath(question, hasTemporalReference) {
  ...
  const eventSearchPatterns = [
    /\bwhen (did|will|do) i\b/,
    /\bwhen (did|will) (my|our)\b/,
    /\bwhat year (did|will)\b/,
    /\bin which year\b/,
    /\bat what age (did|will) i\b/,
    /\bwhen (was|were) my\b/,
  ];
...
function detectEventDomain(question) {
  const q = question.toLowerCase();
  if (/\bmarri|\b(wedding|spouse|husband|wife|partner)\b/.test(q)) return 'marriage';
