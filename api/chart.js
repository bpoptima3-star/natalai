...
const LANG=`American English only. No Sanskrit (use Rising Sign, Moon Sign, Birth Star, Major Period, Active Phase, Short Window, Saturn Pressure Cycle). Lines starting CLASSICAL/JAIMINI/D10/ASHTAKAVARGA are deterministic facts — state directly, no may/might/could. Warm, no disclaimers, output only requested content.`;
// ─── WRAP HTML ────────────────────────────────────────────────────────────────
...
End with one empowering closing sentence.`},
          ...
          sectionDefs=[
            ...
            {title:'⏰ Timing, Advice & The Verdict',tok:1100,prompt:`${LANG}\nCOMPATIBILITY: ${name} + ${name2||'Partner'}\n${twoCore}\nWrite: (1) Best timing for commitment based on their current periods (1 paragraph). (2) 3 specific pieces of advice for THIS couple — practical, direct, tailored to their actual scores (each 1-2 sentences, do NOT cut short). (3) The Verdict — honest, compassionate, direct summary. What is the true potential of this relationship? End with an empowering but truthful closing paragraph. IMPORTANT: Complete all 3 advice items and the full verdict — do not truncate.`},
          ];
        }
...
Jupiter: house ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      const SECTION_PROMPTS = reportType === 'timing' ? [
        ...
        {title:'⚡ Your Power Windows This Year',tok:340,prompt:`${LANG}${buildTajikCtx()}\n${name} | ${lagna} Rising · ${rashi} Moon\
