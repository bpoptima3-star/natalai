...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      const NEWFEATURES = [
        ...
        // Secondary areas missing from NEWFEATURES
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('DEBT')) ? `CLASSICAL VERDICT — DEBT: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('DEBT'))}` : '',
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('PILGRIMAGE')) ? `CLASSICAL VERDICT — PILGRIMAGE: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('PILGRIMAGE'))}` : '',
        // Chara Dasha (Jaimini sign-based current period)
        rulesResults?.charaDasha?.verdicts?.[0] ? `JAIMINI CHARA DASHA: ${rulesResults.charaDasha.verdicts[0]}` : '',
      ].filter(Boolean).join('\n');
