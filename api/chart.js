...
const LANG=`LANGUAGE: American English only. Zero Sanskrit — not even in parentheses. NEVER use: Mahadasha, Antardasha, Lagna, Rashi, Nakshatra, Sade Sati, Ashtama Shani, Yoga, Dosha, Kendra, Trikona, Pratyantardasha. Use ONLY: "Rising Sign" not Lagna, "Moon Sign" not Rashi, "Birth Star" not Nakshatra, "Major Life Period" not Mahadasha, "Active Phase" not Antardasha, "Short Window" not Pratyantardasha, "Planetary Pattern" not Yoga, "Challenge Pattern" not Dosha, "Saturn Pressure Cycle" not Sade Sati.

CRITICAL — HOW TO USE CLASSICAL VERDICTS:
Lines starting with "CLASSICAL VERDICT —" or "JAIMINI —" or "D10 CONFIRMATION" or "D7 CONFIRMATION" or "ASHTAKAVARGA" are pre-computed deterministic facts from BPHS, Phaladeepika, Saravali, and Jaimini Sutram. These are NOT suggestions. They are the reading.

1. START FROM THESE VERDICTS. Build the reading around them. Do not add your own astrology.
2. State them as facts — no "may", "might", "could", "possibly", "indicators suggest".
3. If a verdict says "STRONGLY BLESSED" — say it is strongly blessed. If it says "CHALLENGED" — say it is challenged.
4. The verdicts are the foundation. Your role is to weave them into clear, warm, specific language — not to re-interpret the chart from scratch.
5. Do NOT contradict a CLASSICAL VERDICT or soften it. If it says "marriage after 30" — that is the answer.
6. All other content (non-verdict lines): "indicators suggest" or "strong potential for" — never absolute.

Warm, direct, specific American English. Never add disclaimers, caveats, or "consult a professional". Output ONLY the requested content.`;
...
function wrapHTML(sections,name,type,cd){
  ...
  <div id="${sid}" style="border-bottom:1px solid rgba(0,0,0,.06);margin-bottom:0">
    ...
    <div class="rep-sec-body" style="padding:4px 40px 32px;font-family:'Cormorant Garamond',serif;font-size:19px;line-height:1.78;color:#2a2218;background:#ffffff">${s.html}</div>
  </div>`;}).join('');
...
module.exports = async function handler(req,res){
  ...
  if(body._report){
    ...
    try{
      ...
      const NAVAMSA_START_ENG = [0,9,6,3,0,9,6,3,0,9,6,3];
      const calcNavamsaEng = lon => {
        const si=so(lon), deg=n360(lon)%30, pada=Math.floor(deg/(30/9));
        return (NAVAMSA_START_ENG[si]+pada)%12;
      };
      ...
      try {
        if (VedicRulesEngine) {
          // Build currentTransits from today's chart data (for Step 6 transit rules)
          const currentTransitsForEngine = {};
          if (todayData && todayData.planets) {
            for (const [p, d] of Object.entries(todayData.planets)) {
              if (d.sign !== undefined) currentTransitsForEngine[p] = d.sign;
              else if (d.lon !== undefined) currentTransitsForEngine[p] = so(d.lon);
            }
          }
...
Jupiter Transit: House ${todayData.jupTransit.house} from Moon (${todayData.jupTransit.quality})`;
      ...
      if(reportType==='compat'||reportType==='cosmic'){
        ...
        const compatRulesCtx = [
          r1c ? `P1 MARRIAGE INDICATORS: ${r1c.marriage?.verdict||''}` : '',
          r1c ? `P1 SPOUSE DESCRIPTION: ${r1c.marriage?.spouseDesc?.join(', ')||''}` : '',
          r1c ? `P1 MARRIAGE TYPE: ${r1c.marriage?.type||'standard'}` : '',
          r1c ? `P1 MARRIAGE TIMING: ${r1c.marriage?.timing?'after age '+r1c.marriage.timing:'standard'}` : '',
          r1c ? `P1 MENTAL NATURE: ${r1c.mentalNature?.verdicts?.[0]||''}` : '',
          r1c ? `P1 YOGAS: ${r1c.yogas?.summary||''}` : '',
          r1c ? `P1 PERSONALITY: ${r1c.personality?.verdicts?.[0]||''}` : '',
          r2compat ? `P2 MARRIAGE INDICATORS: ${r2compat.marriage?.verdict||''}` : '',
          r2compat ? `P2 SPOUSE DESCRIPTION: ${r2compat.marriage?.spouseDesc?.join(', ')||''}` : '',
          r2compat ? `P2 MARRIAGE TYPE: ${r2compat.marriage?.type||'standard'}` : '',
          r2compat ? `P2 MARRIAGE TIMING: ${r2compat.marriage?.timing?'after age '+r2compat.marriage.timing:'standard'}` : '',
          r2compat ? `P2 MENTAL NATURE: ${r2compat.mentalNature?.verdicts?.[0]||''}` : '',
          r2compat ? `P2 YOGAS: ${r2compat.yogas?.summary||''}` : '',
          r2compat ? `P2 PERSONALITY: ${r2compat.personality?.verdicts?.[0]||''}` : '',
        ].filter(Boolean).join('\n');
...
// ══════════════════════════════════════════════════════════════════════════════
      ...
      const NEWFEATURES = [
        ...
        // ── VEDIC RULES ENGINE VERDICTS (deterministic, classical texts) ──────
        // These are hard verdicts — Claude uses them as facts, not to re-derive
        rulesResults ? `CLASSICAL VERDICT — MARRIAGE: ${rulesResults.marriage?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — CAREER: ${rulesResults.career?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — WEALTH: ${rulesResults.wealth?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — HEALTH: ${rulesResults.health?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — SIBLINGS: ${rulesResults.siblings?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — FATHER: ${rulesResults.father?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — MOTHER: ${rulesResults.mother?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — CHILDREN: ${rulesResults.children?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — FOREIGN TRAVEL: ${rulesResults.foreignTravel?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — PROPERTY: ${rulesResults.property?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — EDUCATION: ${rulesResults.education?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — SPIRITUALITY: ${rulesResults.spirituality?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — MIND: ${rulesResults.mind?.verdicts?.slice(0,2).join(' | ')||''}` : '',
        rulesResults ? `CLASSICAL YOGAS DETECTED: ${rulesResults.yogas?.summary||''}` : '',
        rulesResults ? `CLASSICAL DASHA READING: ${rulesResults.dasha?.verdict||''}` : '',
        rulesResults ? `CLASSICAL VERDICT — ENEMIES: ${rulesResults.enemies?.verdict||''}` : '',
        rulesResults?.jaiminiKarakas ? `JAIMINI — ATMAKARAKA: ${rulesResults.jaiminiKarakas.verdicts?.[0]?.substring(0,200)||''}` : '',
        rulesResults?.jaiminiKarakas ? `JAIMINI — CAREER (AmK): ${rulesResults.jaiminiKarakas.verdicts?.[1]?.substring(0,150)||''}` : '',
        rulesResults?.jaiminiKarakas ? `JAIMINI — SPOUSE (DK): ${rulesResults.jaiminiKarakas.verdicts?.[2]?.substring(0,150)||''}` : '',
        rulesResults?.upapada ? `JAIMINI — UPAPADA (TRUE MARRIAGE): ${rulesResults.upapada.verdicts?.slice(0,2).join(' | ')||''}` : '',
        rulesResults?.arudhaLagna ? `JAIMINI — PUBLIC IMAGE (AL): ${rulesResults.arudhaLagna.verdicts?.[0]?.substring(0,150)||''}` : '',
        rulesResults?.jaiminiYogas?.verdicts?.length ? `JAIMINI YOGAS: ${rulesResults.jaiminiYogas.verdicts.join(' | ')||''}` : '',
        // Ashtakavarga — compact transit timing (~40 tokens)
        rulesResults?.ashtakavarga ? `ASHTAKAVARGA: ${rulesResults.ashtakavarga.verdicts?.find(v=>v?.includes('ASHTAKAVARGA:'))||''}` : '',
        rulesResults?.ashtakavarga?.trikonaSAV ? `AV TRIKONA: ${rulesResults.ashtakavarga.trikonaSAV} | DUSTHANA: ${rulesResults.ashtakavarga.dusthanaSAV}` : '',
        rulesResults?.ashtakavarga?.verdicts?.find(v=>v?.includes('JUPITER BEST')) || '',
        rulesResults?.ashtakavarga?.verdicts?.find(v=>v?.includes('SATURN FAVORABLE')) || '',
        // Step 4 — secondary areas
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('VEHICLES'))  ? `CLASSICAL VERDICT — VEHICLES: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('VEHICLES'))}` : '',
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('PHYSICAL'))  ? `CLASSICAL VERDICT — APPEARANCE: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('PHYSICAL APPEARANCE:'))}` : '',
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('MARITAL H')) ? `CLASSICAL VERDICT — MARITAL HAPPINESS: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('MARITAL H'))}` : '',
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('HIDDEN E'))  ? `CLASSICAL VERDICT — HIDDEN ENEMIES: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('HIDDEN E'))}` : '',
        rulesResults?.secondary?.verdicts?.find(v=>v?.startsWith('ANCESTRAL')) ? `CLASSICAL VERDICT — ANCESTRAL KARMA: ${rulesResults.secondary.verdicts.find(v=>v.startsWith('ANCESTRAL'))}` : '',
        rulesResults?.femaleHoroscopy?.verdicts?.length ? `CLASSICAL VERDICT — FEMALE: ${rulesResults.femaleHoroscopy.verdicts.slice(0,2).join(' | ')}` : '',
        // Step 5 — Divisional chart confirmations
        rulesResults?.vargas?.verdicts?.find(v=>v?.includes('D10')) ? `D10 CONFIRMATION: ${rulesResults.vargas.verdicts.find(v=>v.includes('D10'))?.substring(0,150)}` : '',
        rulesResults?.vargas?.verdicts?.find(v=>v?.includes('D7'))  ? `D7 CONFIRMATION: ${rulesResults.vargas.verdicts.find(v=>v.includes('D7'))?.substring(0,120)}` : '',
        rulesResults?.vargas?.verdicts?.find(v=>v?.includes('D4'))  ? `D4 CONFIRMATION: ${rulesResults.vargas.verdicts.find(v=>v.includes('D4'))?.substring(0,120)}` : '',
        rulesResults?.vargas?.verdicts?.find(v=>v?.includes('D24')) ? `D24 CONFIRMATION: ${rulesResults.vargas.verdicts.find(v=>v.includes('D24'))?.substring(0,120)}` : '',
        // Step 6 — Current transit verdicts (injected only when currentTransits provided)
        rulesResults?.transits?.verdicts?.[0] ? `CURRENT TRANSIT: ${rulesResults.transits.verdicts[0]?.substring(0,200)}` : '',
        rulesResults?.transits?.verdicts?.[1] ? `CURRENT TRANSIT: ${rulesResults.transits.verdicts[1]?.substring(0,200)}` : '',
        // Step 8 — Prashna verdict (injected in Ask tab only)
        rulesResults?.prashna?.verdict ? `PRASHNA: ${rulesResults.prashna.verdict?.substring(0,150)}` : '',
        // Step 9 — Remedies (injected in all reports — only high priority)
        rulesResults?.remedies?.needed?.filter(r=>r.priority==='HIGH').length > 0 ?
          `REMEDIES NEEDED: ${rulesResults.remedies.needed.filter(r=>r.priority==='HIGH').map(r=>r.planet+' ('+r.reason+')').join(', ')}` : '',
      ].filter(Boolean).join('\n');
      ...
      try {
        if (VedicRulesEngine && clientEngineInput) {
          ...
          const engChart = {
            ...
            dasha:      { maha: ei.mahaName||'', antar: ei.antarName||'', pratyantar: null },
          ...
          const topicPriority = {
            love:['MARRIAGE','MENTAL','YOGAS'],
            marriage:['MARRIAGE','MENTAL','YOGAS'],
            career:['CAREER','WEALTH','YOGAS','DASHA NOW'],
            money:['WEALTH','CAREER','PROPERTY'],
...
${questionType==='pattern'?'\nThis is a WHY question — go deeper on the root pattern in their chart, not just the surface event':''}`;
  ...
  try{
    ...
    return res.status(200).json({lagna:RS[lagnaSign],rashi:RS[moonSign],nakshatra:NK[no(chart.sid.Moon)],nakshatra_pada:po(chart.sid.Moon),planets,ayanamsa:+ayanamsa(JD(yr,mo,dy,h+mi/60-geo.tz)).toFixed(4),dasha_balance:curr?{planet:FN[curr.lord],years_remaining:yearsRemaining}:null,dashas:dashaList,yogas:yogas.map(y=>({name:y.name,description:y.meaning,strength:y.strength,icon:y.strength==='Strong'?'🏆':'⚡'})),
      navamsa:(() => {
        // D9 navamsha — same structure as D1 planets: {sign, house, degree}
        const d9Lagna = calcNavamsa2(chart.lagna);
        return Object.fromEntries(Object.entries(chart.sid).map(([p,lon]) => {
          const d9Sign = calcNavamsa2(lon);
          const d9House = ((d9Sign - d9Lagna + 12) % 12) + 1;
          const degInSign = lon % 30;
          const padaDeg = degInSign % (30/9); // position within this navamsha pada
          const d9Degree = padaDeg * 9;        // scaled to 0-30° within D9 sign
          return [p, {
            sign:      d9Sign,                 // 0-11 (same index as D1)
            signName:  RS[d9Sign],             // 'Taurus' etc
            house:     d9House,                // 1-12 relative to D9 lagna
            degree:    +d9Degree.toFixed(2),   // 0-30° within the D9 sign
          }];
        }));
      })(),
      navamsa_lagna: (() => {
        const d9LagnaSign = calcNavamsa2(chart.lagna);
        const lagnaInSign = chart.lagna % 30;
        const padaDeg = lagnaInSign % (30/9);
        return {
          sign:     d9LagnaSign,
          signName: RS[d9LagnaSign],
          degree:   +(padaDeg * 9).toFixed(2)
        };
      })(),
      ...
      _engineInput: {
        ...
        // D9 navamsha — same object structure as D1 planets
