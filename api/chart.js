...
async function geocode(place){
  ...
  try{
    ...
    if(d.results?.[0]){
      ...
      // Use utc_offset_seconds if available; otherwise estimate from longitude (avoids India +5.5 default)
      const tzRaw = loc.utc_offset_seconds != null ? loc.utc_offset_seconds/3600 : Math.round(loc.longitude/15);
      // Special case: Indian subcontinent → always 5.5
      const isIndia = loc.latitude >= 8 && loc.latitude <= 37 && loc.longitude >= 68 && loc.longitude <= 97;
      return{lat:loc.latitude,lon:loc.longitude,tz:isIndia ? 5.5 : tzRaw};
    }
  }catch(e){}
...
const NAVAMSA_START_ML  = [0,4,8,0,4,8,0,4,8,0,4,8]; // navamsa start by sign
  ...
  if(body._report){
    ...
    try{
      ...
      const _eVargottamaStr = _eVargottama.length
        ? `Vargottama (exceptionally strong): ${_eVargottama.join(', ')}` : '';
      ...
      const _ePariStr = _ePariYogas.length ? `Parivartana Yogas: ${_ePariYogas.join('; ')}` : '';
