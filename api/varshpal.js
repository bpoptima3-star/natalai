// api/chart.js — NatalAI.live (FIXED)
...
function sunLon(j) {
  const t = TC(j),
    L0 = n360(280.46646 + 36000.76983 * t),
    M = n360(357.52911 + 35999.05029 * t),
    Mr = M * R,
    C = (1.914602 - 0.004817 * t) * Math.sin(Mr) + (0.019993 - 1.01e-4 * t) * Math.sin(2 * Mr);
  return n360(L0 + C - 0.00569 - 0.00478 * Math.sin((125.04 - 1934.136 * t) * R));
}

function moonLon(j) {
  const t = TC(j), t2 = t * t, t3 = t2 * t, t4 = t3 * t,
    Lp = n360(218.3164477 + 481267.88123421 * t - 1.5786e-3 * t2 + t3 / 538841 - t4 / 65194000),
    Dv = n360(297.8501921 + 445267.1114034 * t - 1.8819e-3 * t2),
    Mv = n360(357.5291092 + 35999.0502909 * t),
    Mp = n360(134.9633964 + 477198.8675055 * t + 8.7414e-3 * t2),
    Fv = n360(93.2720950 + 483202.0175233 * t),
    E = 1 - 2.516e-3 * t, E2 = E * E;
  ...
  const T = [
    ...
    [2,-1,1,0,4036],[2,0,2,0,3994],[4,0,0,0,3861]
  ];
  let s = 0;
...
const rahuLon = j => {
  const t = TC(j);
  return n360(125.0445479 - 1934.1362608 * t + 2.0754e-3 * t * t);
};
...
function helioXYZ(t, el) {
  ...
  const I = (i0 + di * t) * R, L = n360(L0 + dL * t) * R;
  const w = n360(w0 + dw * t) * R, N = n360(N0 + dN * t) * R;
  const om = w - N, M = n360((L - w) * D) * R;
  const Ev = keplSolve(M, e);
  const xp = a * (Math.cos(Ev) - e), yp = a * Math.sqrt(1 - e * e) * Math.sin(Ev);
...
function computeChart(y, mo, d, h, mi, tz, lat, lon) {
  ...
  const trop = {
    Sun: sunLon(j), Moon: moonLon(j), Mercury: planetLon(j, 'Mercury'),
    Venus: planetLon(j, 'Venus'), Mars: planetLon(j, 'Mars'),
    Jupiter: planetLon(j, 'Jupiter'), Saturn: planetLon(j, 'Saturn'),
...
function getAntardashas(maha, mahaStart, mahaEnd) {
  const totalYrs = mahaEnd - mahaStart, idx = DS.indexOf(maha);
  const NOW = new Date().getFullYear() + new Date().getMonth() / 12;
  let c = mahaStart;
  return DS.map((_, i) => {
    const al = DS[(idx + i) % 9], dur = totalYrs * DY[al] / 120, s = c, e = c + dur;
    c = e;
    return { lord: al, lordName: FN[al], start: s, end: e, curr: s <= NOW && e > NOW };
  });
}

function buildDashaList(dasha) {
  const NOW = new Date().getFullYear() + new Date().getMonth() / 12;
  const curr = dasha.seq.find(s => s.start <= NOW && s.end > NOW);
  if (!curr) return [];
  const idx = dasha.seq.indexOf(curr);
  return dasha.seq.slice(Math.max(0, idx - 1), idx + 3).map(maha => {
    const antars = getAntardashas(maha.lord, maha.start, maha.end);
    return {
      planet: FN[maha.lord], start_year: Math.floor(maha.start), end_year: Math.floor(maha.end), years: DY[maha.lord],
      current: maha.lord === curr.lord,
      antardashas: antars.map(a => ({ planet: FN[a.lord], start: fmtYr(a.start), end: fmtYr(a.end), current: a.curr || false }))
    };
  });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const planets = Object.entries(chart.sid).map(([pName, lon]) => ({
      name: pName, rashi: RS[so(lon)], house: ((so(lon) - lagnaSign + 12) % 12) + 1,
      degrees: fmtDeg(lon), retrograde: ['Rahu', 'Ketu'].includes(pName), status: getPlanetStatus(pName, so(lon))
    }));
...
{ "summary": "2 sentences on what makes this chart unique — name specific placements.", "yogas":[{ "name": "yoga name", "description": "1 sentence practical meaning", "strength": "Strong", "icon": "🏆" }, { "name": "yoga name", "description": "1 sentence", "strength": "Moderate", "icon": "⭐" }], "navamsa":[{ "planet": "Sun", "rashi": "sign" }, { "planet": "Moon", "rashi": "sign" }, { "planet": "Mars", "rashi": "sign" }, { "planet": "Mercury", "rashi": "sign" }, { "planet": "Jupiter", "rashi": "sign" }, { "planet": "Venus", "rashi": "sign" }, { "planet": "Saturn", "rashi": "sign" }, { "planet": "Rahu", "rashi": "sign" }, { "planet": "Ketu", "rashi": "sign" }] }`;
    ...
    try {
      const raw2 = (d2.content?.[0]?.text || '').replace(/```json\s*|\s*```/g, '').trim();
    ...
    const todayPlanets = {
      Sun: n360(sunLon(todayJD) - todayAy), Moon: n360(moonLon(todayJD) - todayAy),
      Mercury: n360(planetLon(todayJD, 'Mercury') - todayAy), Venus: n360(planetLon(todayJD, 'Venus') - todayAy),
      Mars: n360(planetLon(todayJD, 'Mars') - todayAy), Jupiter: n360(planetLon(todayJD, 'Jupiter') - todayAy),
      Saturn: n360(planetLon(todayJD, 'Saturn') - todayAy), Rahu: n360(rahuLon(todayJD) - todayAy),
    };
    const transits = Object.entries(todayPlanets).map(([p, l]) => ({
      planet: p, sign: RS[so(l)], house: ((so(l) - lagnaSign + 12) % 12) + 1, deg: (l % 30).toFixed(1)
    }));

    return res.status(200).json({
      lagna: RS[lagnaSign], rashi: RS[moonSign], nakshatra: NK[no(chart.sid.Moon)], nakshatra_pada: po(chart.sid.Moon),
      planets, dasha_balance: curr ? { planet: FN[curr.lord], years_remaining: yearsRemaining } : null,
      dashas: dashaList, yogas: aiExtra.yogas || [], navamsa: aiExtra.navamsa || [], summary: aiExtra.summary || '',
      _geo: { lat: geo.lat, lon: geo.lon, tz: geo.tz }, transits
    });
  } catch (e) {
