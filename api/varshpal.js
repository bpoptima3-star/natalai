// api/varshpal.js — Varshpal (Solar Return) engine for NatalAI (FIXED)
...
function moonLon(j) {
  const t = TC(j), t2 = t * t, t3 = t2 * t, t4 = t3 * t,
    ...
    Mv = n360(357.5291092 + 35999.0502909 * t), Mp = n360(134.9633964 + 477198.8675055 * t + 8.7414e-3 * t2),
    Fv = n360(93.2720950 + 483202.0175233 * t), E = 1 - 2.516e-3 * t, E2 = E * E;
...
function helioXYZ(t, el) {
  ...
  const om = w - N, M = n360((L - w) * D) * R, Ev = keplSolve(M, e);
...
function computeChart(y, mo, d, h, mi, tz, lat, lon) {
  ...
  const sid = {}; for (const [k, v] of Object.entries(trop)) sid[k] = n360(v - ay);
...
const YOGINI_DUR = [1, 2, 3, 4, 5, 6, 7, 8];

function getMuddaYoginiDasha(varshMoonLon, srJD) {
  const nak = no(varshMoonLon), yIdx = nak % 8, nakLen = 360 / 27, frac = (n360(varshMoonLon) % nakLen) / nakLen;
  const lordDurDays = YOGINI_DUR[yIdx] * 365.25;
  let cursorJD = srJD - frac * lordDurDays;
  const endJD = srJD + 365.25, periods = [];
  for (let i = 0; i < 12; i++) {
    const idx2 = (yIdx + i) % 8, durDays = YOGINI_DUR[idx2] * 365.25, pStart = cursorJD, pEnd = cursorJD + durDays;
    if (pEnd > srJD && pStart < endJD) {
      const sc = JDtoCal(Math.max(pStart, srJD)), ec = JDtoCal(Math.min(pEnd, endJD));
      periods.push({ lord: YOGINI_LORDS[idx2], lordName: YOGINI_NAMES[idx2], yoginiName: YOGINI_KEYS[idx2], startDate: fmtDt(sc), endDate: fmtDt(ec) });
    }
    cursorJD = pEnd; if (cursorJD >= endJD) break;
  }
  return periods;
...
function getSaham(srSid, srLagna) {
  ...
  const lots = {
    Fortune: calc(la, p.Moon, p.Sun), Education: calc(la, p.Mercury, p.Moon),
    Career: calc(la, p.Mars, p.Sun), Love: calc(la, p.Venus, p.Sun), Wellbeing: calc(la, p.Jupiter, p.Saturn)
  };
  ...
  return Object.entries(lots).map(([name, lon]) => ({
    name, sign: RS[so(lon)], lord: SL2[so(lon)], deg: (lon % 30).toFixed(1)
  }));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const srLocalH = srCal.hour + srCal.minute / 60 + tzF;
    ...
    const varshPlanets = Object.entries(srSid).map(([name, lon]) => ({
      ...
      house: ((so(lon) - srLagnaSign + 12) % 12) + 1, deg: (lon % 30).toFixed(1), retrograde: ['Rahu', 'Ketu'].includes(name)
    }));
    ...
    const natalPlanets = Object.entries(natal.sid).map(([name, lon]) => ({
      name, sign: so(lon), signName: RS[so(lon)], house: ((so(lon) - so(nLagna) + 12) % 12) + 1, deg: (lon % 30).toFixed(1), retrograde: ['Rahu', 'Ketu'].includes(name)
    }));
    ...
    return res.status(200).json({
      ...
      muddaDasha, panchadhikari,
      mahadasha: currMaha ? { lordName: FN[currMaha.lord], lord: currMaha.lord, period: `${fmtYr(currMaha.start)} – ${fmtYr(currMaha.end)}`, yearsRemaining: (currMaha.end - (new Date().getFullYear() + new Date().getMonth() / 12)).toFixed(1) } : null,
      antardasha: currAntar ? { lordName: FN[currAntar.lord], lord: currAntar.lord, period: `${fmtYr(currAntar.start)} – ${fmtYr(currAntar.end)}`, nextLordName: antars[antars.indexOf(currAntar) + 1]?.lordName || null, nextPeriod: antars[antars.indexOf(currAntar) + 1] ? `${fmtYr(antars[antars.indexOf(currAntar) + 1].start)} – ${fmtYr(antars[antars.indexOf(currAntar) + 1].end)}` : null } : null,
      muddaYogini, saham,
    });
  } catch (err) {
